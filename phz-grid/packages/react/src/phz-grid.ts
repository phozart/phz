...
import {
  PhzGrid as PhzGridElement,
  type RowAction,
  type ComputedColumnDef,
  type ColumnProfile,
  type RowActionEventDetail,
  type GenerateDashboardEventDetail,
} from '@phozart/grid';
...
export interface PhzGridProps {
  ...
  onDrillThrough?: (detail: { source: GridRowDrillSource; config: DrillThroughConfig; field: string; value: unknown }) => void;
  onCopy?: (detail: { text: string; rowCount: number; colCount: number; source: 'cell' | 'range' | 'rows' }) => void;
  onGenerateDashboard?: (detail: GenerateDashboardEventDetail) => void;
  onVirtualScroll?: (detail: { startIndex: number; endIndex: number; totalCount: number }) => void;
  onRemoteDataLoad?: (detail: { offset: number; count: number; totalCount: number }) => void;
  onRemoteDataError?: (detail: { error: Error; offset: number }) => void;
