...
export interface PhzGridEventMap {
    ...
    'virtual-scroll': CustomEvent<{
        startIndex: number;
        endIndex: number;
        totalCount: number;
    }>;
    'remote-data-load': CustomEvent<{
        offset: number;
        count: number;
        totalCount: number;
    }>;
    'remote-data-error': CustomEvent<{
        error: Error;
        offset: number;
    }>;
    'bulk-delete': CustomEvent<{
        rowIds: (string | number)[];
    }>;
    'admin-settings': CustomEvent<Record<string, never>>;
}
export declare function dispatchGridEvent<K extends keyof PhzGridEventMap>(host: HTMLElement, eventName: K, detail: PhzGridEventMap[K]['detail']): void;
