...
export class ContextMenuController implements ReactiveController {
  ...
  ctxMenuOpen: boolean = false;
  ctxMenuX: number = 0;
  ctxMenuY: number = 0;
  ctxMenuItems: MenuItem[] = [];
  ctxMenuSource: 'header' | 'body' = 'body';
  ctxMenuField: string = '';
  ctxMenuRowId: RowId | null = null;
  ...
  handleBodyContextMenu(e: MouseEvent, row: RowData): void {
    ...
    const td = path.find(el => el instanceof HTMLElement && el.tagName === 'TD' && el.hasAttribute('data-col')) as HTMLElement | undefined;
    if (td) {
      const colIdx = Number(td.getAttribute('data-col'));
      const col = this.host.columnDefs[colIdx];
      if (col) this.ctxMenuField = col.field;
    }
    ...
    items.push(
      { id: 'sep1', label: '', separator: true },
      { id: 'select-row', label: 'Select Row', icon: '\u2713' },
      { id: 'select-all', label: 'Select All Rows', shortcut: 'Ctrl+A' },
      { id: 'sep2', label: '', separator: true },
      { id: 'export-csv', label: 'Export to CSV', icon: '\u21E9' },
      { id: 'export-excel', label: 'Export to Excel', icon: '\u21E9' },
    );
  ...
  handleContextMenuSelect(id: string): void {
    ...
    switch (id) {
      case 'sort-asc': cmds.sort(this.ctxMenuField, 'asc'); break;
      case 'sort-desc': cmds.sort(this.ctxMenuField, 'desc'); break;
      case 'sort-clear': cmds.sort(this.ctxMenuField, null); break;
      case 'filter': cmds.openFilter(this.ctxMenuField); break;
      case 'filter-clear': cmds.removeFilter(this.ctxMenuField); break;
      case 'hide-col': cmds.hideColumn(this.ctxMenuField); break;
      case 'resize-fit': cmds.autoFitColumn(this.ctxMenuField); break;
      case 'copy-cell': cmds.copyCell(this.ctxMenuRowId!, this.ctxMenuField); break;
      case 'copy-row': cmds.copyRow(this.ctxMenuRowId!); break;
      case 'copy-range': cmds.copyCellRange(false); break;
      case 'copy-range-headers': cmds.copyCellRange(true); break;
      case 'copy-selected-rows': cmds.copySelectedRows(false); break;
      case 'copy-selected-rows-headers': cmds.copySelectedRows(true); break;
      case 'select-row': if (this.ctxMenuRowId != null) cmds.selectRow(this.ctxMenuRowId); break;
      case 'select-all': cmds.selectAll(); break;
      case 'group-by': cmds.groupBy(this.ctxMenuField); break;
      case 'ungroup': cmds.ungroupBy(); break;
      case 'visualize': cmds.openChart(this.ctxMenuField); break;
      case 'detect-anomalies': cmds.detectAnomalies(this.ctxMenuField); break;
      case 'column-chooser': cmds.openColumnChooser(); break;
      case 'export-csv': cmds.exportCSV(); break;
      case 'export-excel': cmds.exportExcel(); break;
      default:
        if (id.startsWith('row-action:')) {
          ...
          if (row) cmds.handleRowAction(actionId, row);
        }
        break;
    }
  }
  ...
  private getCellRangeCount(): number {
    if (!this.host.cellRangeAnchor || !this.host.cellRangeEnd) return 0;
