...
export class PhzFilterPopover extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false;
  @property({ type: String }) field = '';
  @property({ type: String }) columnType: string = 'string';
  @property({ attribute: false }) anchorRect: DOMRect | null = null;
  @property({ attribute: false }) values: FilterValueEntry[] = [];

  @state() private searchQuery = '';
  @state() private filteredValues: FilterValueEntry[] = [];
  @state() private showCustomFilter = false;
  @state() private customOperator: FilterOperator = 'contains';
  @state() private customValue = '';
  @state() private customLogic: 'and' | 'or' = 'and';
  @state() private customOperator2: FilterOperator = 'contains';
  @state() private customValue2 = '';
  @state() private posX = 0;
  @state() private posY = 0;
  @state() private sidePanelLeft = true;
  @state() private selectedDays = new Set<number>();
  @state() private expandedYears = new Set<number>();
  @state() private expandedMonths = new Set<string>();
  @state() private showDatePanel = false;
  @state() private focusedValueIndex = -1;
  ...
  private _lastInternalValues: FilterValueEntry[] | null = null;

  private static readonly DAY_NAMES = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
  ...
  show(field: string, anchorRect: DOMRect, values: FilterValueEntry[], columnType?: string): void {
    this.previousFocusElement = (typeof document !== 'undefined' ? document.activeElement as HTMLElement : null) ?? null;
    this.field = field;
    this.anchorRect = anchorRect;
    this.values = values;
    if (columnType) this.columnType = columnType;
  ...
  private getFocusableElements(): HTMLElement[] {
    ...
    return Array.from(
      root.querySelectorAll<HTMLElement>(
        'input, button, select, [tabindex]:not([tabindex="-1"]), [role="option"]'
      )
    ).filter(el => !el.hasAttribute('disabled') && el.offsetParent !== null);
  }

  /** Handle keyboard events within the popover for value list navigation */
  private handlePopoverKeydown(e: KeyboardEvent): void {
  ...
  private positionPopover(): void {
    ...
    if (x + popWidth > vw) x = vw - popWidth - 8;
    if (y + popHeight > vh) y = this.anchorRect.top - popHeight - 4;
    if (x < 4) x = 4;
    if (y < 4) y = 4;
  ...
  private preSearchChecked: Map<unknown, boolean> | null = null;
  ...
  private applySearch(): void {
    ...
    if (!q) {
      ...
      this.filteredValues = this.values.filter(v =>
        v.displayText.toLowerCase().includes(q)
      );
    }
  ...
  private onSearchInput(e: InputEvent): void {
  ...
  private toggleGroupCheck(entries: FilterValueEntry[]): void {
    const allChecked = entries.every(e => e.checked);
    const targetChecked = !allChecked;
    const entryValues = new Set(entries.map(e => e.value));
    this.values = this.values.map(v =>
      entryValues.has(v.value) ? { ...v, checked: targetChecked } : v
    );
    this.applySearch();
  }

  private toggleYearExpand(year: number): void {
  ...
  private toggleMonthExpand(key: string): void {
  ...
  private handleClear(): void {
    this.dispatchEvent(new CustomEvent<FilterApplyEvent>('filter-apply', {
  ...
  private handleApply(): void {
    ...
    const detail: FilterApplyEvent = { field: this.field, selectedValues };
  ...
  private getOperatorsForType(): Array<{ value: FilterOperator; label: string }> {
  ...
  private getActiveDatePartCount(): number {
  ...
  private renderDatePartChips(): TemplateResult {
    return html`
      <div class="phz-filter__dp">
        <!-- Day of Week -->
        <div class="phz-filter__dp-row">
          <span class="phz-filter__dp-label">Day</span>
          ${PhzFilterPopover.DAY_NAMES.map((name, i) => html`
            <button
              class="phz-filter__dp-chip ${this.selectedDays.has(i) ? 'phz-filter__dp-chip--active' : ''}"
              @click="${() => { const s = new Set(this.selectedDays); if (s.has(i)) s.delete(i); else s.add(i); this.selectedDays = s; }}"
            >${name}</button>
          `)}
  ...
  private renderGroupedDateList(): TemplateResult {
    // Build year → month → entries tree from filtered values
    const yearMap = new Map<number, Map<number, FilterValueEntry[]>>();
    const nonDateEntries: FilterValueEntry[] = [];
        ...
        return html`
          <div class="phz-filter__tree-row"
            ...
            <div class="phz-filter__checkbox ${yearAllChecked ? 'phz-filter__checkbox--checked' : ''}"
                 @click="${(e: Event) => { e.stopPropagation(); this.toggleGroupCheck(allYearEntries); }}">
            ...
            return html`
              <div class="phz-filter__tree-row phz-filter__tree-month"
                ...
                <div class="phz-filter__checkbox ${monthAllChecked ? 'phz-filter__checkbox--checked' : ''}"
                     @click="${(e: Event) => { e.stopPropagation(); this.toggleGroupCheck(monthEntries); }}">
                ...
                <span class="phz-filter__tree-label">${PhzFilterPopover.MONTH_NAMES[month]} ${year}</span>
  ...
  private renderDateSidePanel(): TemplateResult {
            ...
            <div class="phz-filter__custom">
              <div class="phz-filter__custom-row">
                <select
                  ...
                  @change="${(e: Event) => { this.customOperator = (e.target as HTMLSelectElement).value as FilterOperator; }}"
                >
                ...
                <input
                  ...
                  placeholder="Value"
                  .value="${this.customValue}"
                  @input="${(e: InputEvent) => { this.customValue = (e.target as HTMLInputElement).value; }}"
                />
              </div>
              ...
              <div class="phz-filter__custom-row">
                <select
                  ...
                  @change="${(e: Event) => { this.customOperator2 = (e.target as HTMLSelectElement).value as FilterOperator; }}"
                >
                  ${operators.map(op => html`<option value="${op.value}">${op.label}</option>`)}
                </select>
                <input
                  ...
                  @input="${(e: InputEvent) => { this.customValue2 = (e.target as HTMLInputElement).value; }}"
                />
              </div>
            </div>
          ` : nothing}
