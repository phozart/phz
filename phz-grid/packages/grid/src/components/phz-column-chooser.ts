...
type ComputedType = ComputedColumnDef['type'];
...
const COMPUTED_TYPES: Array<{ value: ComputedType; label: string }> = [
  { value: 'sum', label: 'Sum' },
...
const NUMERIC_ONLY_TYPES = new Set<ComputedType>(['sum', 'avg', 'max', 'min']);

@customElement('phz-column-chooser')
export class PhzColumnChooser extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false;
  @property({ attribute: false }) columns: ColumnDefinition[] = [];
  @property({ attribute: false }) columnState: ColumnState | null = null;
  @property({ attribute: false }) profiles: ColumnProfile[] = [];
  @property({ attribute: false }) computedColumns: ComputedColumnDef[] = [];
  @property({ attribute: false }) availableFields: Array<{ field: string; header: string; type?: string }> = [];
  @property({ type: String }) userRole: UserRole = 'user';
  @property({ attribute: false }) restrictedFields: Set<string> = new Set();
  @property({ attribute: false }) dateFormats: Record<string, string> = {};

  @state() private localVisibility: Record<string, boolean> = {};
  @state() private localOrder: string[] = [];
  @state() private searchQuery = '';
  @state() private draggedField: string | null = null;
  @state() private profileName = '';
  @state() private sortAlpha: boolean = false;
  @state() private profilesExpanded: boolean = false;
  @state() private computedSectionExpanded: boolean = false;
  @state() private localComputedColumns: ComputedColumnDef[] = [];
  @state() private expandedSettingsField: string | null = null;

  private cleanup: (() => void) | null = null;

  // --- Role-based feature gates ---
  private get canToggle(): boolean { return this.userRole !== 'viewer'; }
  private get canReorder(): boolean { return this.userRole !== 'viewer'; }
  private get canEditSettings(): boolean { return this.userRole === 'editor' || this.userRole === 'admin'; }
  private get canCreateComputed(): boolean { return this.userRole === 'admin'; }
  ...
  private showAll(): void {
    this.localVisibility = Object.fromEntries(
      Object.keys(this.localVisibility).map(k => [k, this.restrictedFields.has(k) ? false : true])
    );
  }
  ...
  private handleApply(): void {
    this.dispatchEvent(new CustomEvent<ColumnChooserChangeEvent>('chooser-apply', {
      detail: {
  ...
  private handleLoadProfile(profile: ColumnProfile): void {
  ...
  private handleDragStart(e: DragEvent, field: string): void {
  ...
  private handleDrop(e: DragEvent, targetField: string): void {
    e.preventDefault();
  ...
  private getColumnType(field: string): string {
    return (this.columns.find(c => c.field === field) as any)?.type ?? 'string';
  }
  ...
  private emitColumnSettingsChange(detail: { field: string; header?: string; type?: string; dateFormat?: string; width?: number }): void {
  ...
  private toggleSettings(field: string): void {
    this.expandedSettingsField = this.expandedSettingsField === field ? null : field;
  }

  // --- Computed Columns Methods ---

  private addComputedColumn(): void {
    const newCc: ComputedColumnDef = {
      name: 'New Column',
  ...
  private removeComputedColumn(index: number): void {
    this.localComputedColumns = this.localComputedColumns.filter((_, i) => i !== index);
    this.emitComputedColumnsChange();
  }

  private updateComputedName(index: number, name: string): void {
  ...
  private updateComputedType(index: number, type: ComputedType): void {
    const updated = [...this.localComputedColumns];
    ...
    if (NUMERIC_ONLY_TYPES.has(type)) {
      const numericFields = new Set(
        this.availableFields.filter(f => f.type === 'number' || f.type === 'bar').map(f => f.field)
      );
  ...
  private toggleComputedSourceField(index: number, field: string): void {
    const updated = [...this.localComputedColumns];
  ...
  private updateComputedFormula(index: number, formula: string): void {
    const updated = [...this.localComputedColumns];
  ...
  private emitComputedColumnsChange(): void {
  ...
  private getFieldsForComputedType(type: ComputedType): Array<{ field: string; header: string }> {
    if (NUMERIC_ONLY_TYPES.has(type)) {
              ...
              <div class="phz-cc__profiles-body">
                <div class="phz-cc__profile-row">
                  <input
                    ...
                    @input="${(e: InputEvent) => { this.profileName = (e.target as HTMLInputElement).value; }}"
                  />
  ...
  private renderColumnItem(field: string): TemplateResult {
    ...
    return html`
      <div
        ...
        @dragstart="${this.canReorder ? (e: DragEvent) => this.handleDragStart(e, field) : nothing}"
        @dragover="${this.canReorder ? this.handleDragOver : nothing}"
        @drop="${this.canReorder ? (e: DragEvent) => this.handleDrop(e, field) : nothing}"
  ...
  private renderColumnSettings(field: string): TemplateResult {
      ...
      <div class="phz-cc__settings">
        ...
        <div class="phz-cc__settings-row">
          ...
          <input
            ...
            @change="${(e: Event) => this.emitColumnSettingsChange({ field, header: (e.target as HTMLInputElement).value })}"
          />
        </div>
        <!-- Type -->
        <div class="phz-cc__settings-row">
          ...
          <select
            ...
            @change="${(e: Event) => this.emitColumnSettingsChange({ field, type: (e.target as HTMLSelectElement).value })}"
          >
        ...
        <div class="phz-cc__settings-row">
          ...
          <input
            ...
            @change="${(e: Event) => this.emitColumnSettingsChange({ field, width: Number((e.target as HTMLInputElement).value) })}"
          />
        </div>
      </div>
    `;
  ...
  private renderComputedItem(cc: ComputedColumnDef, index: number): TemplateResult {
      ...
      <div class="phz-cc__computed-item">
        <div class="phz-cc__computed-header">
          <input
            ...
            @input="${(e: InputEvent) => this.updateComputedName(index, (e.target as HTMLInputElement).value)}"
        ...
        <div class="phz-cc__computed-type-row">
          ...
          <select
            ...
            .value="${cc.type}"
            @change="${(e: Event) => this.updateComputedType(index, (e.target as HTMLSelectElement).value as ComputedType)}"
          >
        ...
        ` : html`
          <textarea
            ...
            @input="${(e: InputEvent) => this.updateComputedFormula(index, (e.target as HTMLTextAreaElement).value)}"
          ></textarea>
        `}
      </div>
    `;
