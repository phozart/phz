...
export function createSharingFlowState(
  artifactId: string,
  visibility: ArtifactVisibility,
  overrides?: Partial<SharingFlowState>,
): SharingFlowState {
  return {
    artifactId,
 ...
 */
export function setTargetVisibility(
  state: SharingFlowState,
  visibility: ArtifactVisibility,
): SharingFlowState {
  return {
    ...state,
...
export function removeShareTarget(
  ...
  if (index < 0 || index >= state.shareTargets.length) return state;
 ...
 */
export function setShareSearchQuery(
  state: SharingFlowState,
  query: string,
): SharingFlowState {
 ...
 */
export function setShareSearchResults(
  state: SharingFlowState,
  results: ShareTarget[],
): SharingFlowState {
 ...
 */
export function setSharingError(state: SharingFlowState, error: unknown): SharingFlowState {
...
export function canSaveSharing(state: SharingFlowState): boolean {
  ...
  if (state.targetVisibility === 'published' && !state.canPublish) return false;

  // If changing to 'shared', must have at least one share target
  if (state.targetVisibility === 'shared' && state.shareTargets.length === 0) return false;
