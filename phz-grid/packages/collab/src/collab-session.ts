...
class CollabSessionImpl implements CollabSession {
  ...
  constructor(config: CollabConfig) {
    if (config.sessionId !== undefined && !VALID_SESSION_ID.test(config.sessionId)) {
      throw new Error(
        '@phozart/collab: Invalid session ID format. Only alphanumeric, hyphen, and underscore allowed (max 128 chars).',
      );
    }
    this.config = config;
  ...
  getYDoc(): YDoc {
    if (!this.doc) throw new Error('@phozart/collab: Not connected');
    return this.doc;
  }
  ...
  private getYGridDocument(): YGridDocument {
    if (!this.doc) throw new Error('@phozart/collab: No Yjs document');
    return {
