...
export function scalePoints(data, minY, maxY, width, height, padding) {
...
export function computeStackedData(allSeries) {
...
export function computeYBounds(allSeries) {
...
let PhzAreaChart = class PhzAreaChart extends LitElement {
    constructor() {
        super(...arguments);
        this.data = [];
        this.stacked = false;
    ...
    handlePointHover(e, seriesName, point) {
        const container = e.currentTarget.closest('.area-container');
        if (!container)
            return;
        ...
        const pos = computeTooltipPosition({ x: e.clientX - rect.left, y: e.clientY - rect.top }, { tooltipWidth: 160, tooltipHeight: 40, viewportWidth: rect.width, viewportHeight: rect.height });
    ...
    render() {
        ...
        // Y axis ticks
        const yTicks = 5;
        const yTickValues = Array.from({ length: yTicks }, (_, i) => bounds.min + (i / (yTicks - 1)) * (bounds.max - bounds.min));
        return html `
          ...
          <svg class="area-svg" viewBox="0 0 ${SVG_WIDTH} ${SVG_HEIGHT}" preserveAspectRatio="xMidYMid meet">
            ...
            ${scaledSeries.map(s => s.points.map(p => svg `
              <circle cx="${p.sx}" cy="${p.sy}" r="3"
                      ...
                      @mouseenter=${(e) => this.handlePointHover(e, s.name, p)}
                      @mouseleave=${() => this.handlePointLeave()}
                      @focus=${(e) => {
            const circle = e.currentTarget;
            const container = circle.closest('.area-container');
            if (!container)
                return;
            const rect = container.getBoundingClientRect();
            this.tooltipContent = formatTooltipContent({
            ...
            const pos = computeTooltipPosition({ x: svgRect.left - rect.left + svgRect.width / 2, y: svgRect.top - rect.top }, { tooltipWidth: 160, tooltipHeight: 40, viewportWidth: rect.width, viewportHeight: rect.height });
            this.tooltipStyle = `top:${pos.top}px;left:${pos.left}px`;
            this.tooltipVisible = true;
        }}
...
], PhzAreaChart.prototype, "data", void 0);
__decorate([
    property({ type: Boolean })
], PhzAreaChart.prototype, "stacked", void 0);
__decorate([
    property({ type: Boolean })
], PhzAreaChart.prototype, "showGrid", void 0);
__decorate([
    property({ type: Boolean })
], PhzAreaChart.prototype, "showAxis", void 0);
__decorate([
    property({ type: Number })
], PhzAreaChart.prototype, "opacity", void 0);
__decorate([
    property({ type: String })
], PhzAreaChart.prototype, "chartTitle", void 0);
__decorate([
    property({ type: Boolean })
], PhzAreaChart.prototype, "loading", void 0);
__decorate([
    property({ type: String })
], PhzAreaChart.prototype, "error", void 0);
__decorate([
    property({ attribute: false })
], PhzAreaChart.prototype, "colors", void 0);
__decorate([
    state()
], PhzAreaChart.prototype, "tooltipContent", void 0);
__decorate([
    state()
], PhzAreaChart.prototype, "tooltipVisible", void 0);
__decorate([
    state()
], PhzAreaChart.prototype, "tooltipStyle", void 0);
PhzAreaChart = __decorate([
    customElement('phz-area-chart')
], PhzAreaChart);
export { PhzAreaChart };
//# sourceMappingURL=phz-area-chart.js.map