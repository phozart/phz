...
export class PhzWidget extends LitElement {
  static styles = [
    widgetBaseStyles,
    css`
      :host { display: block; }
  ...
  @property({ type: Object }) config?: EnhancedWidgetConfig;
  ...
  private renderWidgetContent() {
    if (!this.config) return html`<div class="widget-placeholder">No configuration</div>`;
    ...
    switch (type) {
      case 'kpi-card': {
        if (bindings.type !== 'kpi') return html`<div class="widget-placeholder">Invalid bindings</div>`;
        const kpiDef = this.engine?.kpis.get(bindings.kpiId);
        if (!kpiDef) return html`<div class="widget-placeholder">KPI not found</div>`;
      ...
      case 'kpi-scorecard': {
        if (bindings.type !== 'scorecard') return html`<div class="widget-placeholder">Invalid bindings</div>`;
      ...
      case 'bar-chart': {
        if (bindings.type !== 'chart') return html`<div class="widget-placeholder">Invalid bindings</div>`;
      ...
      case 'trend-line': {
        ...
        if (this.engine) {
          for (const kpi of this.engine.kpis.list()) {
            if ((kpi.id as string) === metricField) { target = kpi.target; break; }
          }
        }
        const provider = this.scoreProvider ?? createDefaultScoreProvider();
        const kpiDef = this.engine?.kpis.list().find(k => (k.id as string) === metricField);
      ...
      case 'status-table': {
        if (bindings.type !== 'status-table') return html`<div class="widget-placeholder">Invalid bindings</div>`;
        const kpiDefs = bindings.kpiIds.map(id => this.engine?.kpis.get(id)).filter(Boolean);
      ...
      case 'drill-link': {
        if (bindings.type !== 'drill-link') return html`<div class="widget-placeholder">Invalid bindings</div>`;
  ...
  render() {
    ...
    if (!this.config) return html`<div class="phz-w-card phz-w-state"><p class="phz-w-state__message">No widget configuration</p></div>`;

    const { appearance } = this.config;
    const { container, titleBar } = appearance;

    const classes = ['widget-container'];
    if (container.shadow !== 'none') classes.push(`widget-container--shadow-${container.shadow}`);
    if (container.border) classes.push('widget-container--bordered');
...
declare global {
  interface HTMLElementTagNameMap { 'phz-widget': PhzWidget; }
}
