...
export function computeWaterfallBounds(bars: WaterfallBar[]): { min: number; max: number } {
...
export function buildWaterfallAccessibleDescription(bars: WaterfallBar[]): string {
  return bars.map(b => {
    if (b.type === 'total') {
...
export class PhzWaterfallChart extends LitElement {
  static styles = [
    widgetBaseStyles,
    css`
      :host { display: block; }
  ...
  @property({ attribute: false }) data: WaterfallDatum[] = [];
  @property({ attribute: false }) colors: Record<string, string> = WATERFALL_COLORS;
  ...
  private handleBarClick(bar: WaterfallBar, index: number) {
  ...
  private handleBarHover(e: MouseEvent, bar: WaterfallBar) {
    const container = (e.currentTarget as SVGElement).closest('.waterfall-container') as HTMLElement;
    if (!container) return;
  ...
  render() {
    ...
    const yToPixel = (v: number) => CHART_DIMS.padding.top + plotH - ((v - yMin) / yRange) * plotH;
            ...
            ${bars.map((bar, i) => {
              ...
              return svg`
                ...
                <rect class="waterfall-bar phz-w-clickable"
                  ...
                  @mouseenter=${(e: MouseEvent) => this.handleBarHover(e, bar)}
                  @mouseleave=${() => this.handleBarLeave()}
                  @focus=${(e: FocusEvent) => {
                    const el = e.currentTarget as SVGElement;
                    const container = el.closest('.waterfall-container') as HTMLElement;
                    if (!container) return;
                    const rect = container.getBoundingClientRect();
