...
export function detectThresholdZone(
  value: number,
...
export class PhzGauge extends LitElement {
  static styles = [
    widgetBaseStyles,
    css`
      :host { display: block; }
  ...
  @property({ attribute: false }) thresholds: GaugeThreshold[] = [];
  @property({ type: String }) label: string = '';
  @property({ type: String }) unit: string = '';
  @property({ type: Boolean }) showValue: boolean = true;
  @property({ type: Number }) startAngle: number = DEFAULT_START_ANGLE;
  @property({ type: Number }) endAngle: number = DEFAULT_END_ANGLE;
