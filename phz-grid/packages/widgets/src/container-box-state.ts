 ...
 */
export function createContainerBoxState(config: ContainerBoxConfig): ContainerBoxState {
  return {
    config,
    childWidgetIds: [],
    collapsed: false,
 ...
 */
export function createDefaultContainerBoxState(
 ...
 */
export function toggleContainerCollapse(state: ContainerBoxState): ContainerBoxState {
 ...
 */
export function addChildWidget(state: ContainerBoxState, widgetId: string): ContainerBoxState {
  if (state.childWidgetIds.includes(widgetId)) return state;
 ...
 */
export function removeChildWidget(state: ContainerBoxState, widgetId: string): ContainerBoxState {
  const filtered = state.childWidgetIds.filter(id => id !== widgetId);
  if (filtered.length === state.childWidgetIds.length) return state;
 ...
 */
export function reorderChildWidget(
  state: ContainerBoxState,
  widgetId: string,
  targetIndex: number,
): ContainerBoxState {
...
export function updateContainerConfig(
  state: ContainerBoxState,
  updates: Partial<ContainerBoxConfig>,
): ContainerBoxState {
