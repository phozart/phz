...
import { initialUnifiedWorkspaceState, navigateToView, navigateBack, canNavigateBack, setWorkspaceViewDirty, toggleWorkspaceDataPanel, setWorkspaceDataPanelTab, getAvailableDataPanelTabs, getActiveWorkspaceView, isAuthoringView, shouldShowDataPanel, shouldShowSelectorBar, openWorkspaceDrawer, closeWorkspaceDrawer, } from './shell/unified-workspace-state.js';
import { VIEW_PANELS, VIEW_TYPE_ICON, legacyPanelToViewType, } from './workspace-config.js';
...
// Component
// ═══════════════════════════════════════════════════════════════════════
let PhzWorkspace = class PhzWorkspace extends LitElement {
    constructor() {
        super(...arguments);
        this.workspaceRole = 'admin';
        this.title = 'Report Administration Tool';
        /** @deprecated Use navigateTo() instead. Kept for backward compat. */
        this.activePanel = '';
        this._ws = initialUnifiedWorkspaceState();
        this._mobileNavOpen = false;
        this._panelCache = new Map();
    }
    static { this.TAG = 'phz-workspace'; }
  ...
  `; }
    ...
    navigateTo(type, label, artifactId) {
        this._ws = navigateToView(this._ws, {
            type,
            label,
            icon: VIEW_TYPE_ICON[type] ?? 'catalog',
    ...
    _renderViewElement(view, panel) {
        ...
        const bindings = getAdapterBindings(view.type === 'report' ? 'authoring-report'
            : view.type === 'dashboard' ? 'authoring-dashboard'
                : view.type, { dataAdapter: this.dataAdapter, workspaceAdapter: this.adapter });
        forwardAdaptersToElement(el, bindings);
        if ('role' in el) {
            el.role = this.workspaceRole;
        }
        if (view.artifactId && 'artifactId' in el) {
            el.artifactId = view.artifactId;
        }
        return el;
    }
    // ── Render ───────────────────────────────────────────────────────
...
], PhzWorkspace.prototype, "adapter", void 0);
__decorate([
    property({ attribute: false })
], PhzWorkspace.prototype, "dataAdapter", void 0);
__decorate([
    property({ type: String, reflect: true, attribute: 'workspace-role' })
], PhzWorkspace.prototype, "workspaceRole", void 0);
__decorate([
    property({ type: String })
], PhzWorkspace.prototype, "title", void 0);
__decorate([
    property({ type: String, reflect: true, attribute: 'active-panel' })
], PhzWorkspace.prototype, "activePanel", void 0);
__decorate([
    state()
], PhzWorkspace.prototype, "_ws", void 0);
__decorate([
    state()
], PhzWorkspace.prototype, "_mobileNavOpen", void 0);
PhzWorkspace = __decorate([
    safeCustomElement('phz-workspace')
], PhzWorkspace);
export { PhzWorkspace };
//# sourceMappingURL=phz-workspace.js.map