...
export function formatPresetValuePreview(value, def) {
...
export function isFilterTypeCompatible(sourceType, targetType) {
    if (sourceType === targetType)
        return true;
    const multiGroup = new Set(['multi_select', 'chip_group']);
    return multiGroup.has(sourceType) && multiGroup.has(targetType);
}
/** Build context menu items for a filter-specific preset card */
export function buildFilterPresetContextItems(preset) {
    return [
 ...
 */
export function resolveDefinitionOptions(def, dataSources, data) {
...
let PhzPresetAdmin = class PhzPresetAdmin extends LitElement {
    constructor() {
        super(...arguments);
  ...
  `]; }
    render() {
        if (this.mode === 'per-filter')
            return this._renderPerFilterMode();
        return this._renderCrossFilterMode();
    }
    // ============================
    // Cross-filter mode (existing)
    // ============================
    _renderCrossFilterMode() {
        return html `
      <div class="phz-pa-root" @contextmenu=${(e) => this._handleBgContextMenu(e)}>
    ...
    _renderPreset(preset) {
      ...
      <div class="phz-pa-card"
           @contextmenu=${(e) => this._handleCardContextMenu(e, preset)}>
    ...
    // Per-filter mode (new)
    // ============================
    _renderPerFilterMode() {
        const activeDefs = (this.definitions ?? []).filter(d => !d.deprecated);
        const comboOpts = activeDefs.map(d => ({ value: d.id, label: d.label }));
      ...
      <div class="phz-pa-root">
        ...
        <div class="phz-pa-body">
          <div class="phz-pa-filter-selector">
            ...
            <phz-combobox
              .options=${comboOpts}
              .value=${this._selectedDefId ?? ''}
              allow-empty
              empty-label="\u2014 Select a filter \u2014"
              @combobox-change=${(e) => {
            const val = e.detail.value;
            this._selectedDefId = val ? val : null;
        }}
            ></phz-combobox>
          </div>
    ...
    _renderFilterPresetCard(preset, def) {
        const chips = formatPresetValuePreview(preset.value, def);
        return html `
      <div class="phz-pa-card"
           @contextmenu=${(e) => this._handleFilterPresetContextMenu(e, preset)}>
        <div class="phz-pa-card-header">
          <span class="phz-pa-card-name">${preset.name}</span>
          <span class="phz-pa-scope-badge phz-pa-scope-badge--${preset.scope}">${preset.scope}</span>
          ${preset.isDefault ? html `<span class="phz-pa-default-badge">Default</span>` : nothing}
    ...
    _renderFilterPresetModal(def) {
      ...
      <div class="phz-pa-modal-backdrop" @click=${() => this._closeModal()}
           @keydown=${(e) => { if (e.key === 'Escape')
            this._closeModal(); }}>
        <div class="phz-pa-modal-panel" @click=${(e) => e.stopPropagation()}
          ...
          <div class="phz-pa-modal-body">
            <div class="phz-pa-field-row">
              ...
              <input class="phz-sc-input" .value=${this._modalName}
                     @input=${(e) => this._modalName = e.target.value}
                  ...
                  ${allOptions.length > 8 ? html `
                    <div class="phz-pa-option-search">
                      <input class="phz-sc-input" type="text"
                             ...
                             @input=${(e) => this._modalSearch = e.target.value}>
                    </div>
                  ` : nothing}
          ...
          <div class="phz-pa-modal-body">
            ...
            <div class="phz-pa-field-row">
              ...
              ` : html `
                <phz-combobox
                  .options=${comboOpts}
                  .value=${this._copyTargetDefId ?? ''}
                  allow-empty
                  empty-label="\u2014 Select target filter \u2014"
                  @combobox-change=${(e) => {
            const val = e.detail.value;
            this._copyTargetDefId = val ? val : null;
        }}
                ></phz-combobox>
              `}
            </div>
    ...
    _saveFilterPreset() {
        ...
        if (this._editingPreset) {
            ...
            this._dispatchEvent('filter-preset-create', {
                filterDefinitionId: this._selectedDefId,
                name,
    ...
    _executeCopy() {
        if (!this._copySourcePreset || !this._copyTargetDefId)
            return;
        this._dispatchEvent('filter-preset-copy', {
            sourcePresetId: this._copySourcePreset.id,
            targetFilterDefinitionId: this._copyTargetDefId,
        });
        this._closeCopyModal();
    }
    _handleFilterPresetContextMenu(e, preset) {
        e.preventDefault();
        e.stopPropagation();
    ...
    _handleCardContextMenu(e, preset) {
        e.preventDefault();
        e.stopPropagation();
    ...
    _handleBgContextMenu(e) {
        ...
        const isCard = path.some(el => el instanceof HTMLElement && el.classList?.contains('phz-pa-card'));
        if (isCard)
            return;
        e.preventDefault();
...
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "sharedPresets", void 0);
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "userPresets", void 0);
__decorate([
    property({ type: String })
], PhzPresetAdmin.prototype, "mode", void 0);
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "definitions", void 0);
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "filterPresets", void 0);
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "dataSources", void 0);
__decorate([
    property({ type: Array })
], PhzPresetAdmin.prototype, "data", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_tab", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_selectedDefId", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_modalOpen", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_editingPreset", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_modalName", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_modalValue", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_modalSearch", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_copyModalOpen", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_copySourcePreset", void 0);
__decorate([
    state()
], PhzPresetAdmin.prototype, "_copyTargetDefId", void 0);
PhzPresetAdmin = __decorate([
    safeCustomElement('phz-preset-admin')
], PhzPresetAdmin);
export { PhzPresetAdmin };
