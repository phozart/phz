...
import { initialUnifiedWorkspaceState, navigateToView, navigateBack, canNavigateBack, setWorkspaceViewDirty, renameWorkspaceView, toggleWorkspaceDataPanel, setWorkspaceDataPanelWidth, setWorkspaceDataPanelTab, getAvailableDataPanelTabs, openWorkspaceDrawer, closeWorkspaceDrawer, setWorkspaceDrawerWidth, addWorkspaceRecentArtifact, getActiveWorkspaceView, isAuthoringView, shouldShowDataPanel, shouldShowSelectorBar, } from '../unified-workspace-state.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeView(type = 'report', label = 'Test', artifactId) {
...
describe('UnifiedWorkspaceState', () => {
    ...
    describe('dataPanelTab', () => {
        ...
        it('rejects filters tab on a report (filters live in editor)', () => {
            let s = initialUnifiedWorkspaceState();
            s = navigateToView(s, makeView('report', 'R1', 'r1'));
            const result = setWorkspaceDataPanelTab(s, 'filters');
            expect(result).toBe(s);
            expect(result.dataPanelTab).toBe('data');
        });
        it('rejects settings tab on a dashboard (settings live in editor)', () => {
            let s = initialUnifiedWorkspaceState();
            s = navigateToView(s, makeView('dashboard', 'D1', 'd1'));
            const result = setWorkspaceDataPanelTab(s, 'settings');
            expect(result).toBe(s);
            expect(result.dataPanelTab).toBe('data');
        });
...
//# sourceMappingURL=unified-workspace-state.test.js.map