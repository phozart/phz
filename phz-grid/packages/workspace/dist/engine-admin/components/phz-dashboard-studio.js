...
const DEFAULT_CONFIGS = {
    'kpi-card': (id) => ({
        id: widgetId(id), type: 'kpi-card', kpiId: '',
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'kpi-scorecard': (id) => ({
        id: widgetId(id), type: 'kpi-scorecard', kpis: [], expandable: false,
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'bar-chart': (id) => ({
        id: widgetId(id), type: 'bar-chart', dataProductId: '',
        metricField: '', dimension: '', rankOrder: 'desc',
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'trend-line': (id) => ({
        id: widgetId(id), type: 'trend-line', dataProductId: '',
        metricField: '', periods: 12, showTarget: true,
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'bottom-n': (id) => ({
        id: widgetId(id), type: 'bottom-n', dataProductId: '',
        metricField: '', dimension: '', n: 5, direction: 'bottom',
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'pivot-table': (id) => ({
        id: widgetId(id), type: 'pivot-table', reportId: '',
        position: { row: 0, col: 0, colSpan: 1, rowSpan: 1 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'data-table': (id) => ({
        id: widgetId(id), type: 'data-table', reportId: '',
        position: { row: 0, col: 0, colSpan: 1, rowSpan: 1 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'status-table': (id) => ({
        id: widgetId(id), type: 'status-table', kpis: [], entityDimension: '',
        position: { row: 0, col: 0 }, size: { rowSpan: 1, colSpan: 1 },
    }),
    'drill-link': (id) => ({
...
/* ─── Component ─── */
let PhzDashboardStudio = class PhzDashboardStudio extends LitElement {
    constructor() {
        super(...arguments);
        /* ── Internal state ── */
        ...
        this.nextId = 1;
        this.scoreProvider = createDefaultScoreProvider();
    }
    static { this.styles = [
        engineAdminStyles,
        css `
    ...
    _initDataModel() {
        ...
        const fields = Object.keys(first).map(k => ({
            name: k,
            type: (typeof first[k] === 'number' ? 'number' : typeof first[k] === 'boolean' ? 'boolean' : 'string'),
        }));
    ...
    removeWidget(id) {
        ...
        this.enhancedWidgets = this.enhancedWidgets.filter(w => w.id !== id);
        this.placements = this.placements.filter(p => p.widgetId !== id);
    ...
    updateWidgetColSpan(id, colSpan) {
        ...
        this.placements = this.placements.map(p => p.widgetId === id ? { ...p, colSpan } : p);
    }
    handleWidgetConfigChange(e) {
        const updatedConfig = e.detail.config;
        const id = updatedConfig.id;
        // Update enhanced configs
        this.enhancedWidgets = this.enhancedWidgets.map(w => w.id === id ? updatedConfig : w);
    ...
    bridgeEnhancedToLegacy(enhanced, existing) {
        const b = enhanced.data.bindings;
        switch (enhanced.type) {
            case 'kpi-card':
                if (b.type === 'kpi')
                    return { ...existing, kpiId: b.kpiId };
                return existing;
            case 'kpi-scorecard':
                if (b.type === 'scorecard')
                    return { ...existing, kpis: b.kpiIds, expandable: false };
                return existing;
            case 'bar-chart':
                if (b.type === 'chart')
                    return {
    ...
    enhancedToLegacy(ew) {
        const config = DEFAULT_CONFIGS[ew.type](ew.id);
        const bridged = this.bridgeEnhancedToLegacy(ew, config);
        const placement = this.placements.find(p => p.widgetId === ew.id);
        return {
            id: ew.id,
            widgetType: ew.type,
            config: bridged,
    ...
    handleDrop(e, targetId) {
        ...
        if (!sourceId || sourceId === targetId)
            return;
        // Reorder widgets
        const srcIdx = this.widgets.findIndex(w => w.id === sourceId);
        ...
        const srcEIdx = this.enhancedWidgets.findIndex(w => w.id === sourceId);
        const tgtEIdx = this.enhancedWidgets.findIndex(w => w.id === targetId);
    ...
    handleResizeStart(e, id) {
        e.preventDefault();
        e.stopPropagation();
        const cell = e.target.closest('.canvas-cell');
        if (!cell)
            return;
        ...
        const onMove = (ev) => {
            ...
            this.placements = this.placements.map(p => p.widgetId === id ? { ...p, heightOverride: newH } : p);
        };
        const onUp = () => {
    ...
    getKPIList() { return this.engine?.kpis.list() ?? []; }
    getReportList() { return this.engine?.reports.list() ?? []; }
    /* ── Publish ── */
    handlePublish() {
        const now = Date.now();
        ...
        if (this.enhancedWidgets.length > 0) {
            ...
            const serialized = serializeDashboard(enhanced, kpis, metrics, fields.map(f => ({ fieldKey: f.name, type: f.type })));
    ...
    /* ── Inline KPI Create ── */
    handleCreateKpi() {
        if (!this.newKpiName || !this.newKpiField)
            return;
        const kpi = {
    ...
    handleCreateMetric() {
        if (!this.newMetricName || !this.newMetricField)
            return;
        const metric = {
            id: ('metric-' + Date.now()),
            name: this.newMetricName,
            dataProductId: 'employees',
    ...
    _openWidgetContextMenu(e, widgetId) {
        e.preventDefault();
        e.stopPropagation();
    ...
    _copyReference(entityType, id, name) {
        let ref = '';
        switch (entityType) {
            case 'fields':
                ref = `[${id}]`;
        ...
        switch (entityType) {
            ...
            case 'metrics': {
                ...
                if (orig) {
                    ...
                    this.engine?.metrics.register(clone);
                    this.requestUpdate();
                }
                break;
            }
            case 'kpis': {
                ...
                if (orig) {
                    ...
                    this.engine?.kpis.register(clone);
                    this.requestUpdate();
                }
                break;
            }
    ...
    _deleteEntity(entityType, id) {
        switch (entityType) {
            ...
            case 'calculatedFields':
                ...
                break;
            case 'metrics':
                this.engine?.metrics.remove(id);
                this.requestUpdate();
                break;
            case 'kpis':
                this.engine?.kpis.remove(id);
                this.requestUpdate();
                break;
        }
    }
    _duplicateWidget(id) {
        ...
        const ew = this.enhancedWidgets.find(w => w.id === id);
        if (!w)
            return;
    ...
    _renderContextMenu() {
        ...
        return html `
      <div class="ctx-backdrop" @click=${() => this._closeContextMenu()} @contextmenu=${(e) => { e.preventDefault(); this._closeContextMenu(); }}></div>
    ...
    renderCanvasWidget(w) {
        const p = this.resolvedWidgets?.get(w.id);
        switch (w.widgetType) {
            case 'kpi-card':
    ...
    render() {
        ...
        const selectedEnhanced = this.enhancedWidgets.find(w => w.id === this.selectedWidgetId);
        ...
        <div class="studio-body">
          ...
          <div class="left-panel">
            <phz-data-model-sidebar
              .fields=${this.dataModelStore?.getFields() ?? fields.map((f) => ({ name: f.name, type: f.type === 'num' ? 'number' : f.type === 'bool' ? 'boolean' : 'string' }))}
          ...
          <phz-data-model-modal
            ...
            @modal-select=${(e) => { this.modalEditId = e.detail.id; }}
          ...
          <div class="canvas">
            <div class="canvas-grid" style="grid-template-columns:repeat(${this.layoutColumns},1fr);">
              ...
              ` : this.widgets.map(w => {
            const placement = this.placements.find(p => p.widgetId === w.id);
            const heightStyle = placement?.heightOverride ? `min-height:${placement.heightOverride}px;` : '';
            return html `
                  <div class="canvas-cell ${w.id === this.selectedWidgetId ? 'canvas-cell--selected' : ''} ${w.id === this.dragOverWidgetId ? 'canvas-cell--drag-over' : ''}"
                       style="grid-column:span ${w.position.colSpan};grid-row:span ${w.position.rowSpan};${heightStyle}"
                       @click=${() => { this.selectedWidgetId = w.id; }}
                       @contextmenu=${(e) => this._openWidgetContextMenu(e, w.id)}
                       @dragover=${(e) => this.handleDragOver(e, w.id)}
                       @dragleave=${this.handleDragLeave}
                       @drop=${(e) => this.handleDrop(e, w.id)}>
                    <div class="canvas-cell__drag" draggable="true"
                         @dragstart=${(e) => this.handleDragStart(e, w.id)}>&#x2630;</div>
                    ${this.renderCanvasWidget(w)}
                    <button class="canvas-cell__remove" @click=${(e) => { e.stopPropagation(); this.removeWidget(w.id); }}>&times;</button>
                    <div class="canvas-cell__resize" @mousedown=${(e) => this.handleResizeStart(e, w.id)}></div>
                  </div>
                `;
        })}
            </div>
              ...
              <div class="dashboard-settings">
                ...
                <div class="prop-group">
                  ...
                  <select class="prop-select" .value=${String(this.layoutColumns)}
                          @change=${(e) => { this.layoutColumns = parseInt(e.target.value) || 3; }}>
...
], PhzDashboardStudio.prototype, "engine", void 0);
__decorate([
    property({ type: Array })
], PhzDashboardStudio.prototype, "data", void 0);
__decorate([
    property({ type: String })
], PhzDashboardStudio.prototype, "dashboardId", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "dashboardName", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "dashboardDescription", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "layoutColumns", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "widgets", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "enhancedWidgets", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "placements", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "selectedWidgetId", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "globalFilters", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "globalFilterValues", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "showKpiForm", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "showMetricForm", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newKpiName", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newKpiField", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newKpiTarget", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newMetricName", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newMetricField", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "newMetricAgg", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "dataModelStore", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "modalType", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "modalEditId", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "resolvedWidgets", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "dragOverWidgetId", void 0);
__decorate([
    state()
], PhzDashboardStudio.prototype, "_ctxMenu", void 0);
PhzDashboardStudio = __decorate([
    safeCustomElement('phz-dashboard-studio')
], PhzDashboardStudio);
export { PhzDashboardStudio };
//# sourceMappingURL=phz-dashboard-studio.js.map