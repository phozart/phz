...
let PhzAdminColumns = class PhzAdminColumns extends LitElement {
    constructor() {
        super(...arguments);
        this.columns = [];
        this.columnTypes = {};
    ...
    get filteredAvailable() {
        ...
        return this.availableFields.filter(f => this.getHeader(f).toLowerCase().includes(q));
    }
    // ── Event emission ──
    emitUpdate() {
    ...
    handleAddSelected() {
        if (this.availableHighlighted.size === 0)
            return;
    ...
    removeFields(fields, keepSettings = false) {
        this.selectedFields = this.selectedFields.filter(f => !fields.includes(f));
        this.availableFields = [...this.availableFields, ...fields].sort((a, b) => this.getHeader(a).localeCompare(this.getHeader(b)));
        this.selectedHighlighted = new Set();
    ...
    handleRemoveSelected() {
        if (this.selectedHighlighted.size === 0)
            return;
    ...
    handleRemoveAll() {
        if (this.selectedFields.length === 0)
            return;
        this.availableFields = [...this.availableFields, ...this.selectedFields].sort((a, b) => this.getHeader(a).localeCompare(this.getHeader(b)));
        this.selectedFields = [];
        this.selectedHighlighted = new Set();
        this.settingsField = null;
        this.emitHideAll();
    ...
    // ── Move up/down ──
    handleMoveUp() {
        if (this.selectedHighlighted.size !== 1)
            return;
    ...
    handleMoveDown() {
        ...
        const field = [...this.selectedHighlighted][0];
        const idx = this.selectedFields.indexOf(field);
        if (idx < 0 || idx >= this.selectedFields.length - 1)
            return;
        const next = [...this.selectedFields];
    ...
    handleDragLeave(_field) {
        this.dragOverField = null;
    }
    handleDrop(targetField, e) {
        e.preventDefault();
        this.dragOverField = null;
    ...
    handleStatusValueRename(field, oldValue, newValue) {
        const current = { ...(this.statusColors || {}) };
    ...
    handleAddThreshold(field) {
        const current = [...(this.barThresholds || [])];
        current.push({ min: 0, color: '#3B82F6' });
        this.emitColumnConfig({ field, barThresholds: current });
    }
    handleRemoveThreshold(field, index) {
        const current = [...(this.barThresholds || [])];
      ...
      <div class="col-settings">
        ...
        <div class="settings-row">
          ...
          <select .value=${colType}
                  @change=${(e) => this.handleColumnTypeChange(field, e.target.value)}>
        ...
        ${showNumberFormat ? html `
          <div class="settings-row">
            ...
            <select .value=${numFmt.display || 'number'}
                    @change=${(e) => this.handleNumberFormatChange(field, 'display', e.target.value)}>
          ...
          ${numFmt.display === 'currency' ? html `
            <div class="settings-row">
              ...
              <input type="text" .value=${numFmt.prefix || ''}
                     ...
                     @change=${(e) => this.handleNumberFormatChange(field, 'prefix', e.target.value)}>
            </div>
          ` : nothing}
        ` : nothing}
        ...
        ${showDateFormat ? html `
          <div class="settings-row">
            ...
            <select .value=${this.dateFormats[field] || 'dd/mm/yyyy'}
                    @change=${(e) => this.handleDateFormatChange(field, e.target.value)}>
              ...
              ${Object.entries(this.statusColors || {}).map(([sv, colors]) => html `
                <div class="status-color-row">
                  <input type="text" .value=${sv} style="width: 80px; padding: 4px 6px; border: 1px solid #D6D3D1; border-radius: 4px; font-size: 11px;"
                         @change=${(e) => this.handleStatusValueRename(field, sv, e.target.value)}>
                  <input type="color" .value=${colors.bg}
                         title="Background"
                         @input=${(e) => this.handleStatusColorChange(field, sv, 'bg', e.target.value)}>
                  <input type="color" .value=${colors.color}
                         title="Text"
                         @input=${(e) => this.handleStatusColorChange(field, sv, 'color', e.target.value)}>
                  <input type="color" .value=${colors.dot}
                         title="Dot"
                         @input=${(e) => this.handleStatusColorChange(field, sv, 'dot', e.target.value)}>
              ...
              ${(this.barThresholds || []).map((t, i) => html `
                <div class="threshold-row">
                  ...
                  <input type="number" .value=${String(t.min)} style="width: 60px;"
                         @change=${(e) => this.handleThresholdChange(field, i, 'min', e.target.value)}>
                  <input type="color" .value=${t.color}
                         @input=${(e) => this.handleThresholdChange(field, i, 'color', e.target.value)}>
                  <button class="remove-entry-btn" title="Remove"
        ...
        ${showLinkTemplate ? html `
          <div class="settings-row">
            ...
            <input type="text" .value=${this.linkTemplates[field] || ''}
                   ...
                   @change=${(e) => this.handleLinkTemplateChange(field, e.target.value)}>
          </div>
    ...
    renderColumnFormatting(field) {
      ...
      <div class="fmt-row">
        ...
        <select .value=${fmt.fontFamily || ''}
          @change=${(e) => this.handleFormattingChange(field, 'fontFamily', e.target.value)}>
          ${FONT_OPTIONS.map(f => {
            let label = f.label;
            if (f.value === '' && isMonoType && !fmt.fontFamily)
      ...
      <div class="fmt-color-row">
        ...
        <input type="color" .value=${fmt.bgColor || '#FFFFFF'}
          @input=${(e) => this.handleFormattingChange(field, 'bgColor', e.target.value)}>
        <input type="text" .value=${fmt.bgColor || ''}
          placeholder="#FFFFFF"
          @change=${(e) => this.handleFormattingChange(field, 'bgColor', e.target.value)}>
      </div>
      <div class="fmt-color-row">
        <label>Text</label>
        <input type="color" .value=${fmt.textColor || '#1C1917'}
          @input=${(e) => this.handleFormattingChange(field, 'textColor', e.target.value)}>
        <input type="text" .value=${fmt.textColor || ''}
          placeholder="#1C1917"
          @change=${(e) => this.handleFormattingChange(field, 'textColor', e.target.value)}>
      </div>
      ...
      ${thresholds.map((t, i) => html `
        <div class="threshold-entry">
          <select .value=${t.operator}
            @change=${(e) => this.handleThresholdEntryChange(field, i, 'operator', e.target.value)}>
          ...
          <input type="text" .value=${t.value}
            placeholder="value"
            @change=${(e) => this.handleThresholdEntryChange(field, i, 'value', e.target.value)}>
          <input type="color" .value=${t.bgColor}
            title="BG"
            @input=${(e) => this.handleThresholdEntryChange(field, i, 'bgColor', e.target.value)}>
          <input type="color" .value=${t.textColor}
            title="Text"
            @input=${(e) => this.handleThresholdEntryChange(field, i, 'textColor', e.target.value)}>
    ...
    handleThresholdEntryChange(field, index, key, value) {
        const fmt = this.columnFormatting[field] || {};
        const thresholds = [...(fmt.colorThresholds || [])];
        if (thresholds[index]) {
    ...
    handleAddThresholdEntry(field) {
        const fmt = this.columnFormatting[field] || {};
        const thresholds = [...(fmt.colorThresholds || [])];
        thresholds.push({ operator: 'gte', value: '', bgColor: '#FEF3C7', textColor: '#92400E' });
        this.handleFormattingChange(field, 'colorThresholds', thresholds);
    }
    handleRemoveThresholdEntry(field, index) {
        const fmt = this.columnFormatting[field] || {};
        const thresholds = [...(fmt.colorThresholds || [])];
          ...
          <div class="panel">
            ...
            : filteredAvail.map(field => html `
                  <div class="panel-item ${this.availableHighlighted.has(field) ? 'panel-item--selected' : ''}"
                       ...
                       @click=${(e) => this.handleAvailableClick(field, e)}
          ...
          <div class="panel">
            ...
            : this.selectedFields.map(field => html `
                  <div class="panel-item ${this.selectedHighlighted.has(field) ? 'panel-item--selected' : ''} ${this.dragOverField === field ? 'panel-item--drag-over' : ''}"
                       ...
                       draggable="true"
                       @click=${(e) => this.handleSelectedClick(field, e)}
                       @dblclick=${() => this.handleRemoveItem(field)}
                       @dragstart=${(e) => this.handleDragStart(field, e)}
                       @dragover=${(e) => this.handleDragOver(field, e)}
                       @dragleave=${() => this.handleDragLeave(field)}
                       @drop=${(e) => this.handleDrop(field, e)}
                    ...
                    <button class="sel-item__gear ${this.settingsField === field ? 'sel-item__gear--active' : ''}"
                            ...
                            @click=${(e) => { e.stopPropagation(); this.handleToggleSettings(field); }}>
                      &#x2699;
                    </button>
                    <button class="sel-item__remove"
                            title="Remove ${this.getHeader(field)}"
                            @click=${(e) => { e.stopPropagation(); this.handleRemoveItem(field); }}>
                      &#x2715;
                    </button>
                  </div>
                `)}
...
__decorate([
    property({ type: Array })
], PhzAdminColumns.prototype, "columns", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "columnTypes", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "statusColors", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "barThresholds", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "dateFormats", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "linkTemplates", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "columnFormatting", void 0);
__decorate([
    property({ attribute: false })
], PhzAdminColumns.prototype, "numberFormats", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "availableFields", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "selectedFields", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "searchQuery", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "availableHighlighted", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "selectedHighlighted", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "dragField", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "dragOverField", void 0);
__decorate([
    state()
], PhzAdminColumns.prototype, "settingsField", void 0);
PhzAdminColumns = __decorate([
    safeCustomElement('phz-admin-columns')
], PhzAdminColumns);
export { PhzAdminColumns };
//# sourceMappingURL=phz-admin-columns.js.map