...
// DuckDB type mapping
// ========================================================================
export function mapDuckDBTypeToDataType(duckType) {
...
export function inferSemanticHint(fieldName, dataType) {
    if (dataType === 'date')
        return 'timestamp';
    if (dataType === 'boolean')
        return undefined;
    const name = fieldName.toLowerCase();
    if (ID_PATTERNS.test(name))
        return 'identifier';
    if (TIMESTAMP_PATTERNS.test(name))
        return 'timestamp';
    if (dataType === 'number') {
        if (PRICE_PATTERNS.test(name))
            return 'currency';
        if (PERCENT_PATTERNS.test(name))
            return 'percentage';
        return 'measure';
    }
    if (dataType === 'string') {
        if (CATEGORY_PATTERNS.test(name))
            return 'category';
        return 'dimension';
    }
    return undefined;
...
export function buildAggregationSelectSQL(field, fn, alias) {
...
// Window function SQL
// ========================================================================
export function buildWindowFunctionSQL(spec) {
...
export function buildDataAdapterQuery(query) {
    ...
    if (query.sort?.length) {
        const sortParts = query.sort.map(s => `${quoteId(s.field)} ${s.direction === 'desc' ? 'DESC' : 'ASC'}`);
...
//# sourceMappingURL=duckdb-data-adapter.js.map