 ...
 */
export function mapFilterOperator(op) {
    switch (op) {
        // Direct 1:1 mappings
        case 'equals':
        case 'notEquals':
 ...
 */
export function resolveTemporalFilter(field, operator, config, now = new Date()) {
    switch (operator) {
        case 'lastN': {
            const n = config.n ?? 7;
...
function subtractTime(date, n, unit) {
...
function periodStart(date, unit) {
    const result = new Date(date);
    switch (unit) {
        case 'days':
 ...
 */
export function filterValuesToQueryFilters(filters, now) {
    const result = [];
    for (const filter of filters) {
        ...
        const temporal = resolveTemporalFilter(filter.field, filter.operator, filter.value ?? {}, now);
        if (temporal) {
 ...
 */
export function injectFiltersIntoQuery(query, filters) {
    if (filters.length === 0)
        return query;
    return {
        ...query,
        filters: [...(Array.isArray(query.filters) ? query.filters : []), ...filters],
    };
}
//# sourceMappingURL=filter-query-bridge.js.map