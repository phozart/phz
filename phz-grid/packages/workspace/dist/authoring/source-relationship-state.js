...
// Initial state
// ========================================================================
export function initialSourceRelationshipState() {
    return { relationships: [], validationErrors: [], suggestedRelationships: [] };
}
// ========================================================================
// CRUD
// ========================================================================
export function addSourceRelationship(state, rel) {
    // No duplicate pairs
    const exists = state.relationships.some(r => (r.leftSourceId === rel.leftSourceId && r.rightSourceId === rel.rightSourceId) ||
        (r.leftSourceId === rel.rightSourceId && r.rightSourceId === rel.leftSourceId));
    if (exists)
        return state;
    return { ...state, relationships: [...state.relationships, rel] };
}
export function removeSourceRelationship(state, id) {
    return { ...state, relationships: state.relationships.filter(r => r.id !== id) };
}
export function updateSourceRelationship(state, id, updates) {
...
// Edit flow
// ========================================================================
export function startEditRelationship(state, id) {
    const rel = state.relationships.find(r => r.id === id);
    if (!rel)
        return state;
    return { ...state, editingRelationshipId: id, editingDraft: { ...rel } };
}
export function commitEditRelationship(state) {
    if (!state.editingRelationshipId || !state.editingDraft)
        return state;
    const relId = state.editingRelationshipId;
    const draft = state.editingDraft;
    const relationships = state.relationships.map(r => r.id === relId ? { ...r, ...draft } : r);
    return { ...state, relationships, editingRelationshipId: undefined, editingDraft: undefined };
}
export function cancelEditRelationship(state) {
 ...
 */
export function autoDetectRelationships(schemas) {
    if (schemas.length < 2)
        return [];
    const results = [];
    ...
    for (let i = 0; i < schemas.length; i++) {
        for (let j = i + 1; j < schemas.length; j++) {
            ...
            for (const fa of a.fields) {
                const match = b.fields.find(fb => !usedB.has(fb.name) &&
            ...
            for (const fa of a.fields) {
                if (joinKeys.some(k => k.leftField === fa.name))
                    continue;
                const match = b.fields.find(fb => !usedB.has(fb.name) &&
                    fb.dataType === fa.dataType &&
...
export function applySuggestedRelationships(state) {
    let next = state;
...
// Validation
// ========================================================================
export function validateRelationships(relationships, availableSlotIds) {
    const errors = [];
 ...
 * @internal
 */
export function _resetRelationshipCounter() {
    relCounter = 0;
}
//# sourceMappingURL=source-relationship-state.js.map