...
import { createWorkbenchState, setWorkbenchSources, setWorkbenchSourcesLoading, setWorkbenchSchema, setWorkbenchSchemaLoading, setWorkbenchFieldSearch, addFieldToWorkbench, removeFieldFromWorkbench, autoPlaceWorkbenchField, cycleAggregation, setPreviewMode, setPreviewLoading, setPreviewResult, setPreviewError, workbenchToExploreQuery, hasWorkbenchQuery, getFilteredFieldsByCategory, pushWorkbenchSnapshot, undoWorkbench, redoWorkbench, canUndoWorkbench, canRedoWorkbench, } from './data-workbench-orchestrator.js';
// ========================================================================
...
let PhzDataWorkbench = class PhzDataWorkbench extends LitElement {
    constructor() {
        // ── Properties ──
        super(...arguments);
        // ── Internal State ──
        this._state = createWorkbenchState();
  ...
  `; }
    ...
    async _loadSources() {
        if (!this.adapter || this._sourcesLoaded)
            return;
        ...
        try {
            ...
            this._state = setWorkbenchSources(this._state, sources);
    ...
    async _fetchPreview() {
        ...
        try {
            ...
            this._state = setPreviewResult(this._state, result);
        }
        catch (err) {
    ...
    _renderFieldCategory(category, fields) {
        if (fields.length === 0)
            return nothing;
        return html `
    ...
    _renderShelf(zone, label, entries, type) {
        return html `
      <div class="wb-shelf"
           role="region"
           aria-label="${label} shelf"
           @dragover=${(e) => { e.preventDefault(); e.currentTarget.classList.add('wb-shelf--drag-over'); }}
           @dragleave=${(e) => { e.currentTarget.classList.remove('wb-shelf--drag-over'); }}
           @drop=${(e) => {
            e.preventDefault();
            e.currentTarget.classList.remove('wb-shelf--drag-over');
            const fieldName = e.dataTransfer?.getData('text/plain');
            if (fieldName && this._state.schema) {
                const field = this._state.schema.fields.find(f => f.name === fieldName);
                if (field)
                    this._onFieldClickToZone(field, zone);
            }
        }}>
        <span class="wb-shelf__label">${label}</span>
        ${entries.length === 0
            ? html `<span class="wb-shelf__placeholder">Drop fields here</span>`
            : entries.map((entry) => this._renderChip(zone, entry, type))}
      </div>
    `;
    }
    _renderChip(zone, entry, type) {
    ...
    _renderSQLPreview() {
        if (!this._state.selectedSourceId)
            return nothing;
        const query = workbenchToExploreQuery(this._state);
        const dataQuery = exploreQueryToDataQuery(query, this._state.selectedSourceId);
...
], PhzDataWorkbench.prototype, "adapter", void 0);
__decorate([
    property({ type: String })
], PhzDataWorkbench.prototype, "sourceId", void 0);
__decorate([
    state()
], PhzDataWorkbench.prototype, "_state", void 0);
PhzDataWorkbench = __decorate([
    safeCustomElement('phz-data-workbench')
], PhzDataWorkbench);
export { PhzDataWorkbench };
//# sourceMappingURL=phz-data-workbench.js.map