 ...
 */
export function handleDashboardFieldAdd(state, fieldName, metadata) {
    ...
    if (isMeasure) {
        ...
        if (widget.dataConfig.measures.some(m => m.field === fieldName))
            return state;
        const newMeasures = [
        ...
        // Dimension
        if (widget.dataConfig.dimensions.some(d => d.field === fieldName))
            return state;
        const newDimensions = [
 ...
 */
export function handleDashboardFieldRemove(state, fieldName) {
    if (!state.selectedWidgetId)
        return state;
    const widget = state.widgets.find(w => w.id === state.selectedWidgetId);
    if (!widget)
    ...
    if (newDimensions.length === widget.dataConfig.dimensions.length &&
 ...
 */
export function autoCreateWidgetForField(metadata, existingWidgets, availableFields) {
    const field = {
        name: metadata.name,
 ...
 */
export function createWidgetFromPalette(state, widgetType, position, manifest, sourceSlotId) {
 ...
 */
export function getSelectedWidgetSource(state) {
...
//# sourceMappingURL=dashboard-editor-wiring.js.map