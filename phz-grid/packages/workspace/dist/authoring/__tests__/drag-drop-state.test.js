...
import { initialDragDropState, startDrag, hoverTarget, cancelDrag, computeValidTargets, executeDrop, } from '../drag-drop-state.js';
import { initialDashboardEditorState, addWidget, _resetWidgetCounter, } from '../dashboard-editor-state.js';
describe('DragDropState', () => {
    ...
    describe('startDrag', () => {
        it('sets dragging source', () => {
            const source = { type: 'field-palette', field: 'revenue', dataType: 'number' };
    ...
    describe('hoverTarget', () => {
        it('sets hovering target', () => {
            const target = { type: 'canvas-cell', row: 1, col: 2 };
    ...
    describe('computeValidTargets', () => {
        ...
        it('field-palette: includes all three data zones per widget', () => {
            ...
            const source = { type: 'field-palette', field: 'revenue', dataType: 'number' };
            const targets = computeValidTargets(source, dashState);
            const dataZones = targets.filter(t => t.type === 'widget-data-zone' && t.widgetId === wid);
        ...
        it('widget-library: includes only canvas', () => {
            const source = { type: 'widget-library', widgetType: 'bar-chart' };
            const targets = computeValidTargets(source, dashState);
        ...
        it('existing-widget: includes canvas and swap targets (excluding self)', () => {
            ...
            const source = { type: 'existing-widget', widgetId: sourceWidgetId };
            const targets = computeValidTargets(source, dashState);
            const types = targets.map(t => t.type);
            expect(types).toContain('canvas-cell');
            expect(types).toContain('widget-swap');
            // Should not include self as swap target
            const swaps = targets.filter(t => t.type === 'widget-swap');
        ...
        it('filter-chip: includes only filter bar', () => {
            const source = { type: 'filter-chip', filterId: 'f-1' };
            const targets = computeValidTargets(source, dashState);
            expect(targets).toHaveLength(1);
    ...
    describe('executeDrop', () => {
        ...
        it('field on canvas: string field creates bar-chart with dimension', () => {
            const source = { type: 'field-palette', field: 'category', dataType: 'string' };
            const target = { type: 'canvas-cell', row: 0, col: 0 };
            const result = executeDrop(dashState, source, target);
        ...
        it('field on canvas: date field creates line-chart with dimension', () => {
            const source = { type: 'field-palette', field: 'created_at', dataType: 'date' };
            const target = { type: 'canvas-cell', row: 0, col: 0 };
            const result = executeDrop(dashState, source, target);
        ...
        it('field on widget-data-zone dimensions: adds to dimensions', () => {
            dashState = addWidget(dashState, 'bar-chart');
            const wid = dashState.widgets[0].id;
            const source = { type: 'field-palette', field: 'region', dataType: 'string' };
            const target = { type: 'widget-data-zone', widgetId: wid, zone: 'dimensions' };
            const result = executeDrop(dashState, source, target);
        ...
        it('field on widget-data-zone measures: adds measure with sum aggregation', () => {
            ...
            const wid = dashState.widgets[0].id;
            const source = { type: 'field-palette', field: 'revenue', dataType: 'number' };
            const target = { type: 'widget-data-zone', widgetId: wid, zone: 'measures' };
            const result = executeDrop(dashState, source, target);
            expect(result.widgets[0].dataConfig.measures).toEqual([
                { field: 'revenue', aggregation: 'sum' },
            ]);
        ...
        it('field on widget-data-zone filters: adds filter with eq operator', () => {
            dashState = addWidget(dashState, 'bar-chart');
            const wid = dashState.widgets[0].id;
            const source = { type: 'field-palette', field: 'status', dataType: 'string' };
            const target = { type: 'widget-data-zone', widgetId: wid, zone: 'filters' };
            const result = executeDrop(dashState, source, target);
        ...
        it('field on widget-data-zone: returns unchanged for non-existent widget', () => {
            const source = { type: 'field-palette', field: 'x', dataType: 'string' };
            const target = { type: 'widget-data-zone', widgetId: 'no-such', zone: 'dimensions' };
            const result = executeDrop(dashState, source, target);
            expect(result).toBe(dashState);
        });
        it('widget-library on canvas: creates widget at target position', () => {
            const source = { type: 'widget-library', widgetType: 'pie-chart' };
            const target = { type: 'canvas-cell', row: 3, col: 4 };
            const result = executeDrop(dashState, source, target);
            expect(result.widgets).toHaveLength(1);
        ...
        it('existing-widget on canvas: moves widget', () => {
            dashState = addWidget(dashState, 'bar-chart');
            const wid = dashState.widgets[0].id;
            const source = { type: 'existing-widget', widgetId: wid };
            const target = { type: 'canvas-cell', row: 5, col: 5 };
            const result = executeDrop(dashState, source, target);
        ...
        it('existing-widget on widget-swap: swaps positions', () => {
            ...
            const source = { type: 'existing-widget', widgetId: w1.id };
            const target = { type: 'widget-swap', widgetId: w2.id };
            const result = executeDrop(dashState, source, target);
        ...
        it('existing-widget on widget-swap: returns unchanged for non-existent source', () => {
            dashState = addWidget(dashState, 'bar-chart');
            const source = { type: 'existing-widget', widgetId: 'no-such' };
            const target = { type: 'widget-swap', widgetId: dashState.widgets[0].id };
            const result = executeDrop(dashState, source, target);
            expect(result).toBe(dashState);
        });
...
//# sourceMappingURL=drag-drop-state.test.js.map