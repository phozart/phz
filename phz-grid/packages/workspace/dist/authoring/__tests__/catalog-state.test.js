...
import { initialCatalogState, filterArtifacts, extractTags, sortArtifacts, getArtifactsByStatus, } from '../catalog-state.js';
// ---------------------------------------------------------------------------
// Test fixtures
// ---------------------------------------------------------------------------
const artifacts = [
...
describe('filterArtifacts — typeFilter', () => {
    ...
    it('filters to only reports when typeFilter is "report"', () => {
        const state = { ...initialCatalogState(artifacts), typeFilter: 'report' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(2);
        expect(result.every(a => a.type === 'report')).toBe(true);
    });
    it('filters to only dashboards when typeFilter is "dashboard"', () => {
        const state = { ...initialCatalogState(artifacts), typeFilter: 'dashboard' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(2);
    ...
    it('returns empty when typeFilter matches no artifacts', () => {
        ...
        const state = { ...initialCatalogState(onlyKpis), typeFilter: 'report' };
        const result = filterArtifacts(state);
...
describe('filterArtifacts — search', () => {
    it('filters by name (case-insensitive)', () => {
        const state = { ...initialCatalogState(artifacts), search: 'sales' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(1);
        expect(result[0].id).toBe('1');
    });
    it('filters by description (case-insensitive)', () => {
        const state = { ...initialCatalogState(artifacts), search: 'c-suite' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(1);
        expect(result[0].id).toBe('2');
    });
    it('matches partial strings in name', () => {
        const state = { ...initialCatalogState(artifacts), search: 'report' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(2);
        expect(result.map(a => a.id).sort()).toEqual(['1', '3']);
    });
    it('returns nothing when search matches no artifacts', () => {
        const state = { ...initialCatalogState(artifacts), search: 'nonexistent' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(0);
    });
    it('handles empty search string (no filtering)', () => {
        const state = { ...initialCatalogState(artifacts), search: '' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(5);
    });
    it('matches artifacts without description — does not crash', () => {
        const state = { ...initialCatalogState(artifacts), search: 'operations' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(1);
...
describe('filterArtifacts — selectedTags', () => {
    it('filters by single tag (type-based)', () => {
        const state = { ...initialCatalogState(artifacts), selectedTags: ['report'] };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(2);
        expect(result.every(a => a.type === 'report')).toBe(true);
    });
    it('filters by multiple tags (union — artifact type must be in selected tags)', () => {
        const state = { ...initialCatalogState(artifacts), selectedTags: ['report', 'kpi'] };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(3);
    });
    it('returns all artifacts when selectedTags is empty', () => {
        const state = { ...initialCatalogState(artifacts), selectedTags: [] };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(5);
    });
    it('returns empty when selectedTags match no artifact types', () => {
        const state = { ...initialCatalogState(artifacts), selectedTags: ['metric'] };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(0);
    });
});
...
describe('filterArtifacts — sorting', () => {
    it('sorts by name ascending', () => {
        const state = { ...initialCatalogState(artifacts), sortBy: 'name', sortDir: 'asc' };
        const result = filterArtifacts(state);
    ...
    it('sorts by name descending', () => {
        const state = { ...initialCatalogState(artifacts), sortBy: 'name', sortDir: 'desc' };
        const result = filterArtifacts(state);
        const names = result.map(a => a.name);
        expect(names).toEqual([
            'Sales Report',
    ...
    it('sorts by updatedAt ascending', () => {
        const state = { ...initialCatalogState(artifacts), sortBy: 'updatedAt', sortDir: 'asc' };
        const result = filterArtifacts(state);
        const updatedAts = result.map(a => a.updatedAt);
        expect(updatedAts).toEqual([500, 1000, 2000, 3000, 4000]);
    });
    it('sorts by createdAt descending', () => {
        const state = { ...initialCatalogState(artifacts), sortBy: 'createdAt', sortDir: 'desc' };
        const result = filterArtifacts(state);
        const createdAts = result.map(a => a.createdAt);
        expect(createdAts).toEqual([4000, 3000, 2000, 1000, 500]);
    });
    it('sorts by createdAt ascending', () => {
        const state = { ...initialCatalogState(artifacts), sortBy: 'createdAt', sortDir: 'asc' };
        const result = filterArtifacts(state);
        const createdAts = result.map(a => a.createdAt);
...
describe('edge cases', () => {
    ...
    it('search matches description but not name', () => {
        const state = { ...initialCatalogState(artifacts), search: 'monthly revenue' };
        const result = filterArtifacts(state);
        expect(result).toHaveLength(1);
        expect(result[0].id).toBe('4');
    });
});
//# sourceMappingURL=catalog-state.test.js.map