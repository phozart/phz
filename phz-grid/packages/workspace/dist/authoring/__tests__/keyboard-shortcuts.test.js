...
import { matchShortcut, createShortcutHandler, formatShortcut, DEFAULT_AUTHORING_SHORTCUTS, } from '../keyboard-shortcuts.js';
...
describe('keyboard-shortcuts', () => {
    describe('matchShortcut', () => {
        it('matches Ctrl+Z to undo on other platform', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', ctrlKey: true }), 'other');
            expect(action).toBe('undo');
        });
        it('matches Ctrl+Shift+Z to redo on other platform', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', ctrlKey: true, shiftKey: true }), 'other');
            expect(action).toBe('redo');
        });
        it('matches Ctrl+Y to redo on other platform', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'y', ctrlKey: true }), 'other');
            expect(action).toBe('redo');
        });
        it('matches Ctrl+S to save', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 's', ctrlKey: true }), 'other');
            expect(action).toBe('save');
        });
        it('returns null for unbound key', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'q', ctrlKey: true }), 'other');
            expect(action).toBeNull();
        });
        it('returns null for plain letter key with no modifier', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z' }), 'other');
            expect(action).toBeNull();
        });
        it('on mac platform uses metaKey instead of ctrlKey for Cmd+Z', () => {
            // metaKey (Cmd) should match
            const withMeta = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', metaKey: true }), 'mac');
            expect(withMeta).toBe('undo');
            // ctrlKey should NOT match on mac
            const withCtrl = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', ctrlKey: true }), 'mac');
            expect(withCtrl).toBeNull();
        });
        it('on mac platform Cmd+Shift+Z maps to redo', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', metaKey: true, shiftKey: true }), 'mac');
            expect(action).toBe('redo');
        });
        it('does not match when extra modifiers are pressed (Ctrl+Shift+S does not match Ctrl+S)', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 's', ctrlKey: true, shiftKey: true }), 'other');
            expect(action).toBeNull();
        });
        it('does not match when extra alt modifier is pressed', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'z', ctrlKey: true, altKey: true }), 'other');
            expect(action).toBeNull();
        });
        it('Delete maps to delete-widget', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'Delete' }), 'other');
            expect(action).toBe('delete-widget');
        });
        it('Backspace maps to delete-widget', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'Backspace' }), 'other');
            expect(action).toBe('delete-widget');
        });
        it('Escape maps to deselect', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'Escape' }), 'other');
            expect(action).toBe('deselect');
        });
        it('Ctrl+A maps to select-all-widgets', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'a', ctrlKey: true }), 'other');
            expect(action).toBe('select-all-widgets');
        });
        it('Ctrl+D maps to duplicate-widget', () => {
            const action = matchShortcut(DEFAULT_AUTHORING_SHORTCUTS, ev({ key: 'd', ctrlKey: true }), 'other');
    ...
    describe('formatShortcut', () => {
        it('formats Ctrl+Z on other platform', () => {
            const binding = { key: 'z', ctrl: true, action: 'undo', label: 'Undo' };
            expect(formatShortcut(binding, 'other')).toBe('Ctrl+Z');
        });
        it('formats Ctrl+Shift+Z on other platform', () => {
            const binding = { key: 'z', ctrl: true, shift: true, action: 'redo', label: 'Redo' };
        ...
        it('formats Alt modifier on other platform', () => {
            const binding = { key: 'n', alt: true, action: 'new', label: 'New' };
        ...
        it('formats multi-character key (Delete) without uppercasing', () => {
            const binding = { key: 'Delete', action: 'delete-widget', label: 'Delete Widget' };
            expect(formatShortcut(binding, 'other')).toBe('Delete');
        });
        it('formats Escape key', () => {
            const binding = { key: 'Escape', action: 'deselect', label: 'Deselect' };
            expect(formatShortcut(binding, 'other')).toBe('Escape');
        });
        it('formats all modifiers combined on other platform', () => {
            const binding = { key: 'k', ctrl: true, shift: true, alt: true, action: 'special', label: 'Special' };
        ...
        it('defaults to other platform when no platform specified', () => {
            const binding = { key: 's', ctrl: true, action: 'save', label: 'Save' };
...
//# sourceMappingURL=keyboard-shortcuts.test.js.map