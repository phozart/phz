...
import { getWidgetMenu, getCanvasMenu, getChartSegmentMenu, getDashboardContextMenu, } from '../dashboard-context-menu.js';
import { initialDashboardEditorState, addWidget, updateWidgetConfig, _resetWidgetCounter, } from '../dashboard-editor-state.js';
describe('DashboardContextMenu', () => {
    ...
    describe('getDashboardContextMenu', () => {
        it('dispatches widget target to getWidgetMenu', () => {
            const target = { type: 'widget', widgetId: state.widgets[0].id };
        ...
        it('dispatches canvas target to getCanvasMenu', () => {
            const target = { type: 'canvas', position: { row: 0, col: 0 } };
            const menu = getDashboardContextMenu(state, target);
            const ids = menu.map(m => m.id);
        ...
        it('dispatches kpi-value target to getWidgetMenu', () => {
            ...
            const target = { type: 'kpi-value', widgetId: kpiId };
            const menu = getDashboardContextMenu(state, target);
            const ids = menu.map(m => m.id);
            expect(ids).toContain('configure');
            expect(ids).toContain('morph-to');
...
//# sourceMappingURL=dashboard-context-menu.test.js.map