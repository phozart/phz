...
import { getColumnHeaderMenu, getCellMenu, getSelectionMenu, getContextMenu, } from '../report-context-menu.js';
import { initialReportEditorState, addColumn, setSorting, setGrouping, pinColumn, } from '../report-editor-state.js';
function makeState() {
...
function nonSeparatorItems(items) {
...
describe('ReportContextMenu', () => {
    ...
    describe('getContextMenu — dispatcher', () => {
        it('dispatches column-header target to getColumnHeaderMenu', () => {
            const target = { type: 'column-header', field: 'name' };
            const items = getContextMenu(makeState(), target);
            expect(findItem(items, 'sort-asc')).toBeDefined();
            expect(findItem(items, 'hide-column')).toBeDefined();
        });
        it('dispatches cell target to getCellMenu', () => {
            const target = { type: 'cell', field: 'name', rowIndex: 0, value: 'test' };
            const items = getContextMenu(makeState(), target);
        ...
        it('dispatches selection target to getSelectionMenu', () => {
            const target = { type: 'selection', fields: ['name'], rowIndices: [0, 1] };
            const items = getContextMenu(makeState(), target);
...
//# sourceMappingURL=report-context-menu.test.js.map