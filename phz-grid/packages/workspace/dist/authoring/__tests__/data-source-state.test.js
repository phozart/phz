...
import { createDataSourceState, setSources, selectSource, setSchema, setSchemaLoading, setFieldSearch, addField, removeField, reorderFields, setFieldStats, setError, clearError, filteredDimensions, filteredMeasures, allFilteredFields, } from '../data-source-state.js';
// ── Mock data ──────────────────────────────────────────────────────
const mockSources = [
...
const mockSchema = {
    ...
    name: 'Sales Data',
    fields: [
        { name: 'order_id', dataType: 'string', nullable: false, semanticHint: 'identifier' },
        { name: 'customer_name', dataType: 'string', nullable: false, semanticHint: 'dimension', cardinality: 'medium' },
        { name: 'region', dataType: 'string', nullable: false, semanticHint: 'dimension', cardinality: 'low' },
        { name: 'product_category', dataType: 'string', nullable: false, semanticHint: 'category', cardinality: 'low' },
        { name: 'order_date', dataType: 'date', nullable: false, semanticHint: 'timestamp' },
        { name: 'revenue', dataType: 'number', nullable: false, semanticHint: 'measure', unit: { type: 'currency', currencyCode: 'USD' } },
        { name: 'quantity', dataType: 'number', nullable: false, semanticHint: 'measure' },
        { name: 'discount_pct', dataType: 'number', nullable: true, semanticHint: 'percentage' },
    ],
};
...
const ambiguousSchema = {
    ...
    fields: [
        ...
        { name: 'name', dataType: 'string', nullable: false, cardinality: 'high' },
        { name: 'category', dataType: 'string', nullable: false, cardinality: 'low' },
...
describe('data-source-state', () => {
    ...
    describe('setSchema — field classification', () => {
        it('classifies fields by semanticHint', () => {
            ...
            expect(state.dimensions.map(f => f.name)).toEqual(expect.arrayContaining(['customer_name', 'region', 'product_category']));
            expect(state.dimensions).toHaveLength(3);
            // Measures: measure + currency + percentage
            expect(state.measures.map(f => f.name)).toEqual(expect.arrayContaining(['revenue', 'quantity', 'discount_pct']));
...
//# sourceMappingURL=data-source-state.test.js.map