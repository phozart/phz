...
export class PhzAdminFilters extends LitElement {
  ...
  @property({ type: Object }) presets: Record<string, FilterPreset> = {};
  @property({ type: String }) activePreset?: string;

  private handleApply(name: string) {
  ...
  private handleDuplicate(name: string) {
    this.dispatchEvent(new CustomEvent('presets-change', {
