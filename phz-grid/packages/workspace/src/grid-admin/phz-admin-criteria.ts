...
export class PhzAdminCriteria extends LitElement {
  static styles = [
    adminBaseStyles,
    css`
      :host { display: block; }
  ...
  @property({ attribute: false }) availableDefinitions: CriteriaDefinitionItem[] = [];
  @property({ attribute: false }) bindings: CriteriaBindingItem[] = [];

  @state() private searchQuery: string = '';
  @state() private editingBindingId: string | null = null;

  /** Definitions not yet bound */
  private get unboundDefinitions(): CriteriaDefinitionItem[] {
    ...
    if (!this.searchQuery.trim()) return defs;
  ...
  private _addBinding(def: CriteriaDefinitionItem) {
    const newBinding: CriteriaBindingItem = {
  ...
  private _removeBinding(filterDefinitionId: string) {
    this.bindings = (this.bindings ?? [])
      .filter(b => b.filterDefinitionId !== filterDefinitionId)
      .map((b, i) => ({ ...b, order: i + 1 }));
    if (this.editingBindingId === filterDefinitionId) this.editingBindingId = null;
    this._emitChange();
  }

  private _moveBinding(filterDefinitionId: string, direction: -1 | 1) {
    const idx = (this.bindings ?? []).findIndex(b => b.filterDefinitionId === filterDefinitionId);
    if (idx < 0) return;
    const newIdx = idx + direction;
    if (newIdx < 0 || newIdx >= (this.bindings ?? []).length) return;
  ...
  private _toggleVisibility(filterDefinitionId: string) {
    this.bindings = (this.bindings ?? []).map(b =>
      b.filterDefinitionId === filterDefinitionId
        ? { ...b, visible: !b.visible }
        : b,
    );
    this._emitChange();
  ...
  private _toggleConfig(filterDefinitionId: string) {
    this.editingBindingId = this.editingBindingId === filterDefinitionId
      ? null
      : filterDefinitionId;
  }

  private _updateLabelOverride(filterDefinitionId: string, value: string) {
    this.bindings = (this.bindings ?? []).map(b =>
      b.filterDefinitionId === filterDefinitionId
                ...
                <div class="config-overlay">
                  ...
                  <div class="phz-admin-field">
                    ...
                    <input class="phz-admin-input" type="text"
                           ...
                           @input=${(e: InputEvent) => this._updateLabelOverride(b.filterDefinitionId, (e.target as HTMLInputElement).value)}
                           placeholder="${b.label}" />
                  </div>
                  <div class="phz-admin-field">
...
declare global {
  interface HTMLElementTagNameMap { 'phz-admin-criteria': PhzAdminCriteria; }
}
