...
type SettingsTab = 'layout' | 'behaviour' | 'styling';

@safeCustomElement('phz-admin-table-settings')
export class PhzAdminTableSettings extends LitElement {
  static styles = [
    adminBaseStyles,
    css`
      :host { display: block; }
  ...
  private renderToggle(label: string, section: string, key: keyof TableSettings, disabled = false) {
      ...
      <div class="toggle-row ${disabled ? 'disabled' : ''}">
        ...
        <label class="phz-admin-toggle">
          <input type="checkbox" .checked=${checked} ?disabled=${disabled}
            @change=${(e: Event) => this.emit(section, key, (e.target as HTMLInputElement).checked)}>
  ...
  private renderBtnGroup(section: string, key: keyof TableSettings, options: { value: string; label: string }[]) {
    const current = this.s[key] as string;
    return html`
      <div class="btn-group">
        ${options.map(o => html`
        ...
        <div class="settings-content">
          ...
          <div class="setting-row">
            ...
            <input type="range" min="0" max="24" step="1"
              ...
              @input=${(e: Event) => this.emit('container', 'containerRadius', Number((e.target as HTMLInputElement).value))}>
            <input type="number" min="0" max="24"
              .value=${String(this.s.containerRadius ?? 8)}
              style="width:56px;flex:0"
              @change=${(e: Event) => this.emit('container', 'containerRadius', Number((e.target as HTMLInputElement).value))}>
        ...
        <div class="settings-content">
          ...
          <div class="setting-row">
            ...
            <input type="text" .value=${this.s.titleText}
              ...
              @input=${(e: Event) => this.emit('titleBar', 'titleText', (e.target as HTMLInputElement).value)}>
          </div>
          <div class="setting-row">
            <label>Subtitle</label>
            <input type="text" .value=${this.s.subtitleText}
              placeholder="Subtitle..."
              @input=${(e: Event) => this.emit('titleBar', 'subtitleText', (e.target as HTMLInputElement).value)}>
          </div>
          <div class="setting-row">
            <label>Font Family</label>
            <select .value=${this.s.titleFontFamily}
              @change=${(e: Event) => this.emit('titleBar', 'titleFontFamily', (e.target as HTMLSelectElement).value)}>
          ...
          <div class="setting-row">
            ...
            <input type="number" .value=${String(this.s.titleFontSize)} min="10" max="32"
              @change=${(e: Event) => this.emit('titleBar', 'titleFontSize', Number((e.target as HTMLInputElement).value))}>
            <span style="font-size:12px;color:#78716C">px</span>
          </div>
          <div class="setting-row">
            <label>Subtitle Size</label>
            <input type="number" .value=${String(this.s.subtitleFontSize)} min="10" max="24"
              @change=${(e: Event) => this.emit('titleBar', 'subtitleFontSize', Number((e.target as HTMLInputElement).value))}>
            <span style="font-size:12px;color:#78716C">px</span>
          </div>
          <div class="setting-row">
            <label>Background</label>
            <input type="color" .value=${this.s.titleBarBg || '#1C1917'}
              @input=${(e: Event) => this.emit('titleBar', 'titleBarBg', (e.target as HTMLInputElement).value)}>
            <input type="text" .value=${this.s.titleBarBg || ''} style="width:80px;flex:0"
              placeholder="#RRGGBB"
              @change=${(e: Event) => this.emit('titleBar', 'titleBarBg', (e.target as HTMLInputElement).value)}>
          </div>
          <div class="setting-row">
            <label>Text Color</label>
            <input type="color" .value=${this.s.titleBarText || '#FEFDFB'}
              @input=${(e: Event) => this.emit('titleBar', 'titleBarText', (e.target as HTMLInputElement).value)}>
            <input type="text" .value=${this.s.titleBarText || ''} style="width:80px;flex:0"
              placeholder="#RRGGBB"
              @change=${(e: Event) => this.emit('titleBar', 'titleBarText', (e.target as HTMLInputElement).value)}>
          </div>
          <div class="setting-row">
            <label>Icon</label>
            <input type="text" .value=${this.s.titleIcon}
              ...
              @input=${(e: Event) => this.emit('titleBar', 'titleIcon', (e.target as HTMLInputElement).value)}>
          </div>
        </div>
      </details>
    `;
          ...
          ${isPaginate ? html`
            <div class="setting-row">
              ...
              <select .value=${String(this.s.pageSize)}
                @change=${(e: Event) => this.emit('gridOptions', 'pageSize', Number((e.target as HTMLSelectElement).value))}>
          ...
          ${sortEnabled ? html`
            <div class="setting-row">
              ...
              <select .value=${this.s.defaultSortField}
                @change=${(e: Event) => this.emit('gridOptions', 'defaultSortField', (e.target as HTMLSelectElement).value)}>
              ...
              <select .value=${this.s.defaultSortDirection}
                @change=${(e: Event) => this.emit('gridOptions', 'defaultSortDirection', (e.target as HTMLSelectElement).value)}>
   ...
   */
  private _getFieldLevelMap(): Map<string, number> {
   ...
   */
  private _emitGroupLevels(fieldLevelMap: Map<string, number>) {
    // Find max level
    let maxLevel = 0;
    ...
    while (levels.length > 0 && levels[levels.length - 1].length === 0) levels.pop();
              ...
              ${this.s.groupTotals ? html`
                <div class="setting-row" style="margin-top:6px;">
                  ...
                  <select .value=${this.s.groupTotalsFn || 'sum'}
                    @change=${(e: Event) => this.emit('gridOptions', 'groupTotalsFn', (e.target as HTMLSelectElement).value)}>
  ...
  private renderGroupTotalsOverrides() {
    if ((this.columnFields ?? []).length === 0) return nothing;
              ...
              <div class="setting-row">
                ...
                <select .value=${currentVal}
                  @change=${(e: Event) => {
                    ...
                    if (val) {
                      updated[f] = val as 'sum' | 'avg' | 'min' | 'max' | 'count' | 'none';
                    } else {
                      delete updated[f];
                    }
          ...
          ${show ? html`
            ...
            <div class="setting-row">
              ...
              <select .value=${this.s.aggregationFn || 'sum'}
                @change=${(e: Event) => this.emit('gridOptions', 'aggregationFn', (e.target as HTMLSelectElement).value)}>
                <option value="sum">Sum</option>
                <option value="avg">Average</option>
                <option value="count">Count</option>
                <option value="min">Min</option>
        ...
        <div class="settings-content">
          <div class="setting-row">
            ...
            <select .value=${this.s.selectionMode || 'single'}
              @change=${(e: Event) => this.emit('gridOptions', 'selectionMode', (e.target as HTMLSelectElement).value)}>
          ...
          <div class="setting-row">
            ...
            <select .value=${this.s.editMode || 'none'}
              @change=${(e: Event) => this.emit('gridOptions', 'editMode', (e.target as HTMLSelectElement).value)}>
          ...
          <div class="setting-row">
            ...
            <input type="number" .value=${String(this.s.virtualScrollThreshold || 0)} min="0" step="100"
              ...
              @change=${(e: Event) => this.emit('gridOptions', 'virtualScrollThreshold', Number((e.target as HTMLInputElement).value))}>
          ...
          ${isVirtual || hasThreshold ? html`
            <div class="setting-row">
              ...
              <select .value=${String(this.s.fetchPageSize || 100)}
                @change=${(e: Event) => this.emit('gridOptions', 'fetchPageSize', Number((e.target as HTMLSelectElement).value))}>
            ...
            <div class="setting-row">
              ...
              <input type="number" .value=${String(this.s.prefetchPages ?? 2)} min="0" max="5"
                @change=${(e: Event) => this.emit('gridOptions', 'prefetchPages', Number((e.target as HTMLInputElement).value))}>
            </div>
          ` : nothing}
    ...
    return html`
      <div class="group-editor">
            ...
            <div class="group-entry-header">
              <input type="text" .value=${group.header} placeholder="Group header..."
                @change=${(e: Event) => {
                  ...
                  updated[gi] = { ...updated[gi], header: (e.target as HTMLInputElement).value };
        ...
        <button class="add-group-btn" @click=${() => {
          const updated = [...groups, { header: 'Group ' + (groups.length + 1), children: [] as string[] }];
          this.emit('gridOptions', 'columnGroups', updated);
        }}>+ Add Group</button>
      </div>
    `;
        ...
        <div class="settings-content">
          ...
          <div class="setting-row">
            ...
            <input type="color" .value=${this.s.gridLineColor || '#E7E5E4'}
              @input=${(e: Event) => this.emit('display', 'gridLineColor', (e.target as HTMLInputElement).value)}>
            <input type="text" .value=${this.s.gridLineColor || '#E7E5E4'} style="width:80px;flex:0"
              placeholder="#RRGGBB"
              @change=${(e: Event) => this.emit('display', 'gridLineColor', (e.target as HTMLInputElement).value)}>
          </div>
          <div class="setting-row">
          ...
          <div class="setting-row">
            ...
            <input type="color" .value=${this.s.bandingColor || '#FAFAF9'}
              @input=${(e: Event) => this.emit('display', 'bandingColor', (e.target as HTMLInputElement).value)}>
            <input type="text" .value=${this.s.bandingColor || '#FAFAF9'} style="width:80px;flex:0"
              placeholder="#RRGGBB"
              @change=${(e: Event) => this.emit('display', 'bandingColor', (e.target as HTMLInputElement).value)}>
          </div>
        ...
        <div class="settings-content">
          <div class="setting-row">
            ...
            <select .value=${this.s.fontFamily}
              @change=${(e: Event) => this.emit('typography', 'fontFamily', (e.target as HTMLSelectElement).value)}>
          ...
          <div class="setting-row">
            ...
            <input type="number" .value=${String(this.s.fontSize)} min="10" max="20"
              @change=${(e: Event) => this.emit('typography', 'fontSize', Number((e.target as HTMLInputElement).value))}>
            <span style="font-size:12px;color:#78716C">px</span>
          </div>
          <div class="setting-row">
            <label>Style</label>
  ...
  private renderTypeAlignRow(label: string, key: keyof TableSettings) {
    const current = this.s[key] as string;
    return html`
      <div class="setting-row">
        <label>${label}</label>
        <div class="btn-group">
          ${(['left', 'center', 'right'] as const).map(a => html`
            <button class="${current === a ? 'active' : ''}"
              @click=${() => this.emit('typography', key as string, a)}>
              ${a === 'left' ? '\u2190' : a === 'center' ? '\u2194' : '\u2192'}
            </button>
          `)}
        </div>
  ...
  private renderSectionColorsSection() {
    const colorRow = (label: string, bgKey: keyof TableSettings, textKey: keyof TableSettings) => html`
      <div class="color-row">
        <label>${label}</label>
        <div class="color-pair">
          <span>BG</span>
          <input type="color" .value=${String(this.s[bgKey])}
            @input=${(e: Event) => this.emit('colors', bgKey as string, (e.target as HTMLInputElement).value)}>
        </div>
        <div class="color-pair">
          <span>Text</span>
          <input type="color" .value=${String(this.s[textKey])}
            @input=${(e: Event) => this.emit('colors', textKey as string, (e.target as HTMLInputElement).value)}>
        </div>
      </div>
    `;
