...
export class PhzAdminFormatting extends LitElement {
  static styles = [
    adminBaseStyles,
    css`
  ...
  @property({ type: Array }) rules: ConditionalFormattingRule[] = [];
  @property({ type: Array }) fields: string[] = [];

  @state() private editingRuleId: string | null = null;
  private _prevRuleCount = 0;
  ...
  private handleRemoveRule(ruleId: string) {
    if (this.editingRuleId === ruleId) this.editingRuleId = null;
    this.dispatchEvent(new CustomEvent('rules-change', {
      bubbles: true, composed: true,
  ...
  private handleToggleEdit(ruleId: string) {
    this.editingRuleId = this.editingRuleId === ruleId ? null : ruleId;
  }

  private emitUpdate(ruleId: string, updates: Record<string, unknown>) {
    this.dispatchEvent(new CustomEvent('rules-change', {
      bubbles: true, composed: true,
  ...
  private handleFieldChange(rule: ConditionalFormattingRule, value: string) {
    this.emitUpdate(rule.id, { field: value });
  }

  private handleOperatorChange(rule: ConditionalFormattingRule, value: string) {
    this.emitUpdate(rule.id, { operator: value });
  }

  private handleValueChange(rule: ConditionalFormattingRule, value: string) {
    const numVal = Number(value);
    this.emitUpdate(rule.id, { value: value !== '' && !isNaN(numVal) ? numVal : value });
  }

  private handleValue2Change(rule: ConditionalFormattingRule, value: string) {
    const numVal = Number(value);
    this.emitUpdate(rule.id, { value2: value !== '' && !isNaN(numVal) ? numVal : value });
  }

  private handleBgColorChange(rule: ConditionalFormattingRule, value: string) {
    this.emitUpdate(rule.id, { backgroundColor: value });
  }

  private handleTextColorChange(rule: ConditionalFormattingRule, value: string) {
    this.emitUpdate(rule.id, { color: value });
  }

  private handleFontWeightChange(rule: ConditionalFormattingRule, value: string) {
    this.emitUpdate(rule.id, { fontWeight: value });
  }

  private renderEditor(rule: ConditionalFormattingRule) {
      ...
      <div class="rule-editor">
        <div class="editor-row">
          ...
          <select .value=${rule.field}
                  @change=${(e: Event) => this.handleFieldChange(rule, (e.target as HTMLSelectElement).value)}>
        ...
        <div class="editor-row">
          ...
          <select .value=${rule.condition.operator}
                  @change=${(e: Event) => this.handleOperatorChange(rule, (e.target as HTMLSelectElement).value)}>
            ${Object.entries(OPERATOR_LABELS).map(([key, label]) =>
              html`<option value=${key} ?selected=${key === rule.condition.operator}>${label}</option>`
            )}
          </select>
        </div>
        ...
        ${showValue ? html`
          <div class="editor-row">
            ...
            <input type="text" .value=${String(rule.condition.value ?? '')}
                   @input=${(e: Event) => this.handleValueChange(rule, (e.target as HTMLInputElement).value)}>
          </div>
        ` : nothing}
        ...
        ${showValue2 ? html`
          <div class="editor-row">
            ...
            <input type="text" .value=${String(rule.condition.value2 ?? '')}
                   @input=${(e: Event) => this.handleValue2Change(rule, (e.target as HTMLInputElement).value)}>
          </div>
        ` : nothing}
        ...
        <div class="editor-row">
          ...
          <input type="color" .value=${rule.style.backgroundColor || '#ffffff'}
                 @input=${(e: Event) => this.handleBgColorChange(rule, (e.target as HTMLInputElement).value)}>
        ...
        <div class="editor-row">
          ...
          <input type="color" .value=${rule.style.color || '#000000'}
                 @input=${(e: Event) => this.handleTextColorChange(rule, (e.target as HTMLInputElement).value)}>
        ...
        <div class="editor-row">
          ...
          <select .value=${rule.style.fontWeight || 'normal'}
                  @change=${(e: Event) => this.handleFontWeightChange(rule, (e.target as HTMLSelectElement).value)}>
