...
export function diagnoseCORSError(error: unknown, url: string): CORSDiagnosis {
  ...
  const resolutions: CORSResolution[] = ['download-manually', 'configure-server'];
  ...
  return {
    ...
    url,
    message:
      `CORS policy blocked the request to ${url}. ` +
