...
import type { FieldMetadata, TimeIntelligenceConfig, TimeGranularity } from '../data-adapter.js';

// ========================================================================
// Filter UI Type Inference (O.5)
// ========================================================================
...
export function inferFilterUIType(
  field: FieldMetadata,
...
// ========================================================================
// Date Filter Options from TimeIntelligence (O.5a)
// ========================================================================
...
const PERIOD_GRANULARITY: Record<string, TimeGranularity | 'day'> = {
  'today': 'day',
  'yesterday': 'day',
  'this-week': 'week',
  'last-week': 'week',
...
export function buildDateFilterOptions(config: TimeIntelligenceConfig): DateFilterOption[] {
  const availableGranularities = new Set<string>(config.granularities);

  return config.relativePeriods
    .filter(period => {
      const required = PERIOD_GRANULARITY[period.id];
      if (!required) return true; // custom periods always included
