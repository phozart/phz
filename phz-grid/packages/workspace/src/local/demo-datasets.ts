...
function seededValue(arr: string[], idx: number): string {
  return arr[idx % arr.length];
}

function generateSalesRow(idx: number): unknown[] {
  const day = (idx % 28) + 1;
...
function generateInventoryRow(idx: number): unknown[] {
  return [
...
function generateEmployeeRow(idx: number): unknown[] {
  const month = (idx % 12) + 1;
  return [
...
export function generateSampleRows(dataset: SampleDataset, count: number): unknown[][] {
  const rows: unknown[][] = [];
