...
export class PhzGridCreator extends LitElement {
  static styles = css`
    :host { display: block; }
  ...
  @state()
  private _state: WizardState = createWizardState();
        ...
        return html`
          <div class="phz-admin-field" style="margin-bottom: 16px;">
            ...
            <input type="text" style="width: 100%; padding: 8px 12px; border: 1px solid #D6D3D1; border-radius: 8px; font-size: 14px;"
                   ...
                   @input=${(e: InputEvent) => { this._state.draft.name = (e.target as HTMLInputElement).value; this.requestUpdate(); }}
          ...
          <div class="phz-admin-field">
            ...
            <textarea style="width: 100%; padding: 8px 12px; border: 1px solid #D6D3D1; border-radius: 8px; font-size: 14px; min-height: 80px; resize: vertical;"
                      ...
                      @input=${(e: InputEvent) => { this._state.draft.description = (e.target as HTMLTextAreaElement).value; this.requestUpdate(); }}
  ...
  private _create() {
    ...
    this.dispatchEvent(
      new CustomEvent('grid-definition-create', {
        detail: payload,
        bubbles: true,
        composed: true,
      }),
...
declare global {
  interface HTMLElementTagNameMap { 'phz-grid-creator': PhzGridCreator; }
}
