...
export function generateSkeletonHTML(node: LayoutNode): string {
  switch (node.kind) {
    case 'widget': {
...
export function generateLoadingHTML(state: LoadingState): string {
...
export function generateStaleIndicatorHTML(widgetId: string, freshnessStatus: 'fresh' | 'stale' | 'unknown'): string {
  if (freshnessStatus !== 'stale') return '';
...
export function generateFreshnessBadgeHTML(widgetId: string, quality: DataQualityInfo | undefined): string {
  if (!quality) return '';
...
export function generateQualityWarningHTML(widgetId: string, quality: DataQualityInfo | undefined): string {
  if (!quality) return '';
  if (!quality.issues || quality.issues.length === 0) return '';
  ...
  const issueItems = quality.issues.map(issue =>
    `<li class="phz-quality-issue" data-severity="${issue.severity}"${issue.field ? ` data-field="${issue.field}"` : ''}>${issue.message}</li>`
  ).join('\n');
...
export function generateStaleDimmingCSS(widgetId: string): string {
