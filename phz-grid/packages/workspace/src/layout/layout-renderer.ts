...
export function renderLayoutToCSS(node: LayoutNode): LayoutRenderResult {
  const widgetIds = flattenLayoutWidgets(node);
  const { css, html } = renderNode(node);
  return { css, html, widgetIds };
}

function renderNode(node: LayoutNode): { css: string; html: string } {
  switch (node.kind) {
...
function renderWidgetSlot(node: WidgetSlot): { css: string; html: string } {
  const style: string[] = [];
...
function renderAutoGrid(node: AutoGridLayout): { css: string; html: string } {
...
function renderTabs(node: TabsLayout): { css: string; html: string } {
...
function renderSections(node: SectionsLayout): { css: string; html: string } {
...
function renderFreeformLayout(node: FreeformLayout): { css: string; html: string } {
...
export function generateResponsiveCSS(
  widgetId: string,
  behavior: WidgetResponsiveBehavior | undefined,
): string {
  if (!behavior) return '';
  ...
  if (behavior.compactBehavior.hideLegend) rules.push('    --phz-legend-display: none;');
  if (behavior.compactBehavior.hideAxisLabels) rules.push('    --phz-axis-labels-display: none;');
  if (behavior.compactBehavior.hideDataLabels) rules.push('    --phz-data-labels-display: none;');
  if (behavior.compactBehavior.simplifyToSingleValue) rules.push('    --phz-simplify: single-value;');
  if (behavior.compactBehavior.collapseToSummary) rules.push('    --phz-collapse: summary;');
  rules.push('  }');
  rules.push('}');
...
export function computeContainerBreakpoints(layout: AutoGridLayout): { compact: number; mobile: number } {
