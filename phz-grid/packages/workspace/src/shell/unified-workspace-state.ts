...
const CATALOG_VIEW: WorkspaceView = {
  type: 'catalog',
  label: 'Catalog',
  icon: 'catalog',
};
...
export function initialUnifiedWorkspaceState(): UnifiedWorkspaceState {
  return {
...
export function navigateToView(
  ...
  if (isSameView(state.activeView, view)) return state;
  ...
  let next: UnifiedWorkspaceState = {
    ...state,
    activeView: { ...view },
    isDirty: false,
...
export function canNavigateBack(state: UnifiedWorkspaceState): boolean {
  return state.navigationHistory.length > 0;
}

/** Marks the current view as dirty (unsaved changes) or clean. */
export function setWorkspaceViewDirty(
  state: UnifiedWorkspaceState,
  isDirty: boolean,
): UnifiedWorkspaceState {
  if (state.isDirty === isDirty) return state;
  return { ...state, isDirty };
}

/** Updates the active view's label (e.g., after user renames an artifact). */
export function renameWorkspaceView(
  state: UnifiedWorkspaceState,
  newLabel: string,
): UnifiedWorkspaceState {
...
export function toggleWorkspaceDataPanel(
  state: UnifiedWorkspaceState,
): UnifiedWorkspaceState {
...
export function setWorkspaceDataPanelTab(
  state: UnifiedWorkspaceState,
  tab: DataPanelTab,
): UnifiedWorkspaceState {
  if (tab === state.dataPanelTab) return state;
  const available = getAvailableDataPanelTabs(state);
  if (!available.includes(tab)) return state;
 ...
 */
export function getAvailableDataPanelTabs(
  _state: UnifiedWorkspaceState,
): DataPanelTab[] {
  return ['data'];
}

/** Sets data panel width, clamped between 200 and 480. */
export function setWorkspaceDataPanelWidth(
  state: UnifiedWorkspaceState,
  width: number,
): UnifiedWorkspaceState {
...
export function setWorkspaceDrawerWidth(
  state: UnifiedWorkspaceState,
  width: number,
): UnifiedWorkspaceState {
 ...
 */
export function addWorkspaceRecentArtifact(
  state: UnifiedWorkspaceState,
  artifact: Omit<RecentArtifact, 'openedAt'>,
): UnifiedWorkspaceState {
  const entry: RecentArtifact = { ...artifact, openedAt: Date.now() };
...
export function isAuthoringView(state: UnifiedWorkspaceState): boolean {
...
export function shouldShowDataPanel(state: UnifiedWorkspaceState): boolean {
  return !isAuthoringView(state);
}

/** Whether the workspace selector bar should be visible (hidden for authoring views). */
export function shouldShowSelectorBar(state: UnifiedWorkspaceState): boolean {
  return !isAuthoringView(state);
}
...
function isAuthoringViewType(type: WorkspaceViewType): boolean {
...
function isSameView(a: WorkspaceView, b: WorkspaceView): boolean {
  if (a.type !== b.type) return false;
  if (a.artifactId || b.artifactId) return a.artifactId === b.artifactId;
  return true;
}
