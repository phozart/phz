...
import { resolveActivePanel, type NavItem, type BreadcrumbEntry } from './shell-utils.js';
import {
  type BreadcrumbBarState,
  initialBreadcrumbBarState,
  pushBreadcrumb,
  popToBreadcrumb,
  getBreadcrumbs,
  getCollapsedBreadcrumbs,
} from './breadcrumb-bar-state.js';
import { icon, NAV_ICONS, type IconName } from '../styles/icons.js';
import {
  getNavItemsForRole,
  getShellConfig,
  type WorkspaceRole,
...
export class PhzWorkspaceShell extends LitElement {
  ...
  @property({ type: String, reflect: true }) role: WorkspaceRole = 'admin';
  @property({ type: String }) title: string = 'Workspace';
  ...
  @property({ type: String, attribute: 'save-status' }) saveStatus: 'idle' | 'dirty' | 'saving' | 'saved' | 'error' = 'idle';

  /** Whether undo action is available. */
  @property({ type: Boolean, attribute: 'can-undo' }) canUndo = false;

  /** Whether redo action is available. */
  @property({ type: Boolean, attribute: 'can-redo' }) canRedo = false;

  @state() private activePanel: string = '';
  @state() private _mobileNavOpen: boolean = false;
  @state() private _breadcrumbState: BreadcrumbBarState = initialBreadcrumbBarState();

  private getResolvedNavItems(): RoleNavItem[] {
    if (this.navItems) {
      return this.navItems.map(n => ({ ...n, section: 'CONTENT' as const }));
    }
    return getNavItemsForRole(this.role);
  }

  private _emitUndo(): void {
    this.dispatchEvent(new CustomEvent('undo-action', { bubbles: true, composed: true }));
  }

  private _emitRedo(): void {
    this.dispatchEvent(new CustomEvent('redo-action', { bubbles: true, composed: true }));
  }

  private _getSaveLabel(): string {
  ...
  private _handleBreadcrumbClick(index: number) {
  ...
  private renderNavIcon(iconKey: string): unknown {
    const iconName = (NAV_ICONS[iconKey] ?? iconKey) as IconName;
  ...
  private renderSidebar() {
    ...
    const grouped = new Map<SidebarSection, RoleNavItem[]>();
    ...
    return html`
      <div class="shell-sidebar__logo">
        <span class="shell-sidebar__logo-icon">${unsafeHTML(icon('grid' as IconName, 20, '#3B82F6'))}</span>
      ...
      <header class="shell-header">
        ...
        <div class="shell-toolbar-group">
          <button class="shell-toolbar-btn"
                  ...
                  aria-label="Undo"
                  title="Undo (Ctrl+Z)">
            ${unsafeHTML(icon('back' as IconName, 16, 'currentColor'))}
          </button>
          <button class="shell-toolbar-btn"
            ...
            ${unsafeHTML(icon('forward' as IconName, 16, 'currentColor'))}
          </button>
        </div>
      </header>
