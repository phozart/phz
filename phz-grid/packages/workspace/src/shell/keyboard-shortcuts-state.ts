...
export const DEFAULT_CONTEXTUAL_SHORTCUTS: ContextualShortcut[] = [
  // Global
...
export function initialKeyboardShortcutsState(
  shortcuts?: ContextualShortcut[],
...
export function getActiveShortcuts(
  ...
  return state.shortcuts.filter(
    s => s.context === state.activeContext || s.context === 'global',
  );
}
...
export function matchContextualShortcut(
  state: KeyboardShortcutsState,
  event: KeyEvent,
): ContextualShortcut | undefined {
  ...
  return active.find(s => {
    ...
    if (ctrl !== hasCtrlOrMeta) return false;
...
export function setCustomBinding(
  ...
  if (!shortcut || !shortcut.customizable) return state;
...
export function clearCustomBinding(
  state: KeyboardShortcutsState,
  shortcutId: string,
): KeyboardShortcutsState {
  const customBindings = new Map(state.customBindings);
...
export function clearAllCustomBindings(
  state: KeyboardShortcutsState,
): KeyboardShortcutsState {
...
export function recordKeyCombination(
  state: KeyboardShortcutsState,
  event: KeyEvent,
): KeyboardShortcutsState {
  if (!state.recordingShortcutId) return state;

  // Ignore bare modifier keys
  if (['Control', 'Shift', 'Alt', 'Meta'].includes(event.key)) return state;
...
export function getShortcutsByGroup(
  state: KeyboardShortcutsState,
): Map<string, ContextualShortcut[]> {
  const active = getActiveShortcuts(state);
  const groups = new Map<string, ContextualShortcut[]>();
...
export function formatContextualShortcut(
  state: KeyboardShortcutsState,
  shortcutId: string,
): string {
  const shortcut = state.shortcuts.find(s => s.id === shortcutId);
