...
const SUB_TAB_LABELS: Record<DataPanelTab, string> = {
  data: 'Data',
};

@safeCustomElement('phz-workspace-data-panel')
export class PhzWorkspaceDataPanel extends LitElement {
  ...
  @property({ type: String }) activeTab: DataPanelTab = 'data';
  @property({ type: Array }) availableTabs: DataPanelTab[] = ['data'];
  ...
  private _handleTabClick(tab: DataPanelTab) {
  ...
  private _renderDataContent(): unknown {
    ...
    if (this.dataAdapter) {
      (el as unknown as Record<string, unknown>).adapter = this.dataAdapter;
    }
    return el;
  }
