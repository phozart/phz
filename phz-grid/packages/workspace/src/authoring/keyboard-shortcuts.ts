...
export interface ShortcutBinding {
  key: string;
  ctrl?: boolean;     // Ctrl on Windows/Linux, Cmd on macOS
  shift?: boolean;
  alt?: boolean;
  action: string;
  label: string;
...
export const DEFAULT_AUTHORING_SHORTCUTS: ShortcutBinding[] = [
...
export function createShortcutHandler(
  bindings?: ShortcutBinding[],
  platform?: 'mac' | 'other',
): (event: ShortcutMatchEvent) => string | null {
  const b = bindings ?? DEFAULT_AUTHORING_SHORTCUTS;
  const p = platform ?? 'other';
  return (event) => matchShortcut(b, event, p);
}

export function formatShortcut(binding: ShortcutBinding, platform: 'mac' | 'other' = 'other'): string {
  const parts: string[] = [];
  if (binding.ctrl) parts.push(platform === 'mac' ? '\u2318' : 'Ctrl');
