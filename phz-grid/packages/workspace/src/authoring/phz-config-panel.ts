...
import type { EnhancedConfigPanelState, EnhancedConfigSection, FormattingRuleEntry, OverlayEntry } from './enhanced-config-state.js';
import {
...
} from './enhanced-config-state.js';
import type { ShadowLevel } from './phz-shadow-picker.js';
...
const ENHANCED_SECTIONS: { key: EnhancedConfigSection; label: string }[] = [
  { key: 'data', label: 'Data' },
...
export class PhzConfigPanel extends LitElement {
  ...
  @property({ type: Object }) config?: WidgetConfigPanelState;

  /** Enhanced 5-section accordion config */
  @property({ type: Object }) enhancedConfig?: EnhancedConfigPanelState;
  ...
  private _emit(config: WidgetConfigPanelState): void {
  ...
  private _setTab(tab: WidgetConfigPanelState['activeTab']): void {
    if (!this.config) return;
    this._emit(setActiveTab(this.config, tab));
  }

  private _updateTitle(e: Event): void {
    if (!this.config) return;
    this._emit(updateStyleConfig(this.config, { title: (e.target as HTMLInputElement).value }));
  }

  private _updateSubtitle(e: Event): void {
    if (!this.config) return;
    this._emit(updateStyleConfig(this.config, { subtitle: (e.target as HTMLInputElement).value }));
  }
  ...
  private _emitEnhanced(config: EnhancedConfigPanelState): void {
  ...
  override render() {
    ...
    return html`
      <div class="tabs">
        ${(['data', 'style', 'filters'] as const).map(tab => html`
        ...
        return html`
          ...
          <div class="toggle-row">
            ...
            <input type="checkbox" .checked=${this.config.showLegend ?? false}
              @change=${(e: Event) => this._emit(updateStyleConfig(this.config!, { showLegend: (e.target as HTMLInputElement).checked }))}
              aria-label="Show legend" />
          </div>
          <div class="toggle-row">
            <span>Show Labels</span>
            <input type="checkbox" .checked=${this.config.showLabels ?? false}
              @change=${(e: Event) => this._emit(updateStyleConfig(this.config!, { showLabels: (e.target as HTMLInputElement).checked }))}
  ...
  private _renderDataSection(ec: EnhancedConfigPanelState) {
    return html`
  ...
  private _isAccordionOpen(ec: EnhancedConfigPanelState, id: string): boolean {
    return ec.expandedAccordions.includes(id);
  }

  private _renderAccordion(ec: EnhancedConfigPanelState, id: string, title: string, content: unknown) {
  ...
  private _renderAppearanceSection(ec: EnhancedConfigPanelState) {
    return html`
  ...
  private _renderContainerAccordion(ec: EnhancedConfigPanelState) {
    return this._renderAccordion(ec, 'container', 'Container', html`
      <div class="style-field">
        <phz-shadow-picker label="Shadow"
          .value=${ec.container.shadow}
          @shadow-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedContainer(ec, { shadow: e.detail.shadow as ShadowLevel }))}>
        </phz-shadow-picker>
      </div>
      ...
      <div class="style-field">
        <phz-slider-input label="Border Radius"
          ...
          @value-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedContainer(ec, { borderRadius: e.detail.value }))}>
        </phz-slider-input>
      </div>
      ...
      <div class="style-field">
        <phz-color-picker label="Background"
          ...
          @color-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedContainer(ec, { background: e.detail.color }))}>
        </phz-color-picker>
      </div>
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${ec.container.border ?? false}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedContainer(ec, { border: (e.target as HTMLInputElement).checked }))}
      ...
      ${ec.container.border ? html`
        <div class="style-field">
          <phz-color-picker label="Border Color"
            ...
            @color-changed=${(e: CustomEvent) =>
              this._emitEnhanced(updateEnhancedContainer(ec, { borderColor: e.detail.color }))}>
          </phz-color-picker>
        </div>
      ` : nothing}
    `);
  ...
  private _renderTitleBarAccordion(ec: EnhancedConfigPanelState) {
    return this._renderAccordion(ec, 'titlebar', 'Title Bar', html`
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${ec.titleBar.show}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedTitleBar(ec, { show: (e.target as HTMLInputElement).checked }))}
      ...
      ${ec.titleBar.show ? html`
        <div class="style-field">
          ...
          <input class="style-input" .value=${ec.titleBar.title ?? ''}
            @input=${(e: Event) =>
              this._emitEnhanced(updateEnhancedTitleBar(ec, { title: (e.target as HTMLInputElement).value }))}
        ...
        <div class="style-field">
          ...
          <input class="style-input" .value=${ec.titleBar.subtitle ?? ''}
            @input=${(e: Event) =>
              this._emitEnhanced(updateEnhancedTitleBar(ec, { subtitle: (e.target as HTMLInputElement).value }))}
        ...
        <div class="style-field">
          <phz-slider-input label="Font Size"
            ...
            @value-changed=${(e: CustomEvent) =>
              this._emitEnhanced(updateEnhancedTitleBar(ec, { fontSize: e.detail.value }))}>
          </phz-slider-input>
        </div>
        ...
        <div class="style-field">
          ...
          <select class="select-input"
            @change=${(e: Event) =>
              this._emitEnhanced(updateEnhancedTitleBar(ec, { fontWeight: Number((e.target as HTMLSelectElement).value) }))}
        ...
        <div class="style-field">
          <phz-color-picker label="Text Color"
            ...
            @color-changed=${(e: CustomEvent) =>
              this._emitEnhanced(updateEnhancedTitleBar(ec, { color: e.detail.color }))}>
          </phz-color-picker>
        </div>
      ` : nothing}
    `);
  ...
  private _renderChartAccordion(ec: EnhancedConfigPanelState) {
    if (!ec.chart) return nothing;
    ...
    return this._renderAccordion(ec, 'chart', 'Chart', html`
      <div class="style-field">
        <phz-slider-input label="Height"
          ...
          @value-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedChart(ec, { height: e.detail.value }))}>
        </phz-slider-input>
      </div>
      ...
      <div class="style-field">
        <phz-slider-input label="Padding"
          ...
          @value-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedChart(ec, { padding: e.detail.value }))}>
        </phz-slider-input>
      </div>
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${chart.xAxis?.show ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedChart(ec, { xAxis: { ...chart.xAxis, show: (e.target as HTMLInputElement).checked } }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${chart.yAxis?.show ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedChart(ec, { yAxis: { ...chart.yAxis, show: (e.target as HTMLInputElement).checked } }))}
        ...
        <select class="select-input"
          @change=${(e: Event) => {
            ...
            if (pos === 'none') {
              ...
              this._emitEnhanced(updateEnhancedChart(ec, { legend: { show: true, position: pos as 'top' | 'bottom' | 'left' | 'right' } }));
            }
          }}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${chart.dataLabels?.show ?? false}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedChart(ec, { dataLabels: { ...chart.dataLabels, show: (e.target as HTMLInputElement).checked, position: chart.dataLabels?.position ?? 'outside' } }))}
      ...
      <div class="style-field">
        ...
        <select class="select-input"
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedChart(ec, { palette: (e.target as HTMLSelectElement).value }))}
  ...
  private _renderKpiAccordion(ec: EnhancedConfigPanelState) {
    if (!ec.kpi) return nothing;
    const kpi = ec.kpi;
    ...
    return this._renderAccordion(ec, 'kpi', 'KPI', html`
      <div class="style-field">
        <phz-slider-input label="Value Size"
          ...
          @value-changed=${(e: CustomEvent) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { valueSize: e.detail.value }))}>
        </phz-slider-input>
      </div>
      ...
      <div class="style-field">
        ...
        <select class="select-input"
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { layout: (e.target as HTMLSelectElement).value as 'vertical' | 'horizontal' }))}
      ...
      <div class="style-field">
        ...
        <select class="select-input"
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { alignment: (e.target as HTMLSelectElement).value as 'left' | 'center' | 'right' }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${kpi.showTrend ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { showTrend: (e.target as HTMLInputElement).checked }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${kpi.showTarget ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { showTarget: (e.target as HTMLInputElement).checked }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${kpi.showSparkline ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedKpi(ec, { showSparkline: (e.target as HTMLInputElement).checked }))}
  ...
  private _renderScorecardAccordion(ec: EnhancedConfigPanelState) {
    if (!ec.scorecard) return nothing;
    ...
    return this._renderAccordion(ec, 'scorecard', 'Scorecard', html`
      <div class="style-field">
        ...
        <select class="select-input"
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedScorecard(ec, { density: (e.target as HTMLSelectElement).value as 'comfortable' | 'compact' | 'dense' }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${sc.rowBanding ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedScorecard(ec, { rowBanding: (e.target as HTMLInputElement).checked }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${sc.stickyHeader ?? true}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedScorecard(ec, { stickyHeader: (e.target as HTMLInputElement).checked }))}
  ...
  private _renderFormatSection(ec: EnhancedConfigPanelState) {
    return html`
  ...
  private _renderOverlaysSection(ec: EnhancedConfigPanelState) {
  ...
  private _renderBehaviorSection(ec: EnhancedConfigPanelState) {
    return html`
      <div class="style-field">
        <label class="style-label">Click Action</label>
        <select class="select-input"
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedBehaviour(ec, { onClick: (e.target as HTMLSelectElement).value as 'none' | 'filter-others' | 'open-detail' | 'custom-url' }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${ec.behaviour.exportPng}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedBehaviour(ec, { exportPng: (e.target as HTMLInputElement).checked }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${ec.behaviour.exportCsv}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedBehaviour(ec, { exportCsv: (e.target as HTMLInputElement).checked }))}
      ...
      <div class="toggle-row">
        ...
        <input type="checkbox" .checked=${ec.behaviour.autoRefresh}
          @change=${(e: Event) =>
            this._emitEnhanced(updateEnhancedBehaviour(ec, { autoRefresh: (e.target as HTMLInputElement).checked }))}
      ...
      ${ec.behaviour.autoRefresh ? html`
        <div class="style-field">
          <phz-slider-input label="Refresh Interval"
            ...
            @value-changed=${(e: CustomEvent) =>
              this._emitEnhanced(updateEnhancedBehaviour(ec, { refreshInterval: e.detail.value }))}>
          </phz-slider-input>
        </div>
      ` : nothing}
    `;
