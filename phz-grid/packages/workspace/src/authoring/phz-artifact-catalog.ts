...
export class PhzArtifactCatalog extends LitElement {
  @property({ type: Array }) artifacts: ArtifactMeta[] = [];

  @state() private _state: CatalogState = initialCatalogState();
  ...
  private _onTypeFilter(type?: 'report' | 'dashboard'): void {
    this._state = { ...this._state, typeFilter: type };
  }

  private _onSort(e: Event): void {
    const value = (e.target as HTMLSelectElement).value;
    const [sortBy, sortDir] = value.split('-') as [CatalogState['sortBy'], CatalogState['sortDir']];
    this._state = { ...this._state, sortBy, sortDir };
  }

  private _onToggleTag(tag: string): void {
  ...
  private _onArtifactClick(artifact: ArtifactMeta): void {
          ...
          <div class="artifact-grid">
            ${filtered.map(a => html`
              <div class="artifact-card" @click=${() => this._onArtifactClick(a)}
                   ...
                   @keydown=${(e: KeyboardEvent) => e.key === 'Enter' && this._onArtifactClick(a)}>
