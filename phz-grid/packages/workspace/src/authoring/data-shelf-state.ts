...
export interface DataShelfState {
  ...
  draggedField?: { name: string; dataType: string; sourceZone?: DataShelfZone };
 ...
 * - filters → filters
 */
export function initialDataShelfFromWidget(
  widgetId: string,
  dataConfig: {
  ...
  const rows: DataShelfField[] = dataConfig.dimensions.length > 0
    ? [{ name: dataConfig.dimensions[0].field, dataType: 'string', alias: dataConfig.dimensions[0].alias }]
    : [];

  const columns: DataShelfField[] = dataConfig.dimensions.slice(1).map(d => ({
    name: d.field,
    dataType: 'string' as const,
    alias: d.alias,
  }));

  const values: DataShelfMeasure[] = dataConfig.measures.map(m => ({
    name: m.field,
    aggregation: m.aggregation as DataShelfMeasure['aggregation'],
    alias: m.alias,
  }));

  const filters: DataShelfFilter[] = dataConfig.filters.map(f => ({
    field: f.field,
    operator: f.operator as DataShelfFilter['operator'],
    value: f.value,
  }));
 ...
 */
export function getDefaultDataShelfAggregation(
  dataType: string,
): DataShelfMeasure['aggregation'] {
 ...
 */
export function addFieldToDataShelf(
  state: DataShelfState,
  zone: DataShelfZone,
  field: DataShelfField,
): DataShelfState {
  switch (zone) {
    case 'rows':
    ...
    case 'values': {
      const measure: DataShelfMeasure = {
        name: field.name,
 ...
 */
export function removeFieldFromDataShelf(
  state: DataShelfState,
  zone: DataShelfZone,
  fieldName: string,
): DataShelfState {
 ...
 */
export function moveFieldBetweenDataShelves(
  state: DataShelfState,
  from: DataShelfZone,
  to: DataShelfZone,
  fieldName: string,
  ...
  let fieldInfo: { name: string; dataType: DataShelfField['dataType']; alias?: string } | undefined;
  ...
  // Add to target
  const shelfField: DataShelfField = {
    name: fieldInfo.name,
 ...
 */
export function reorderDataShelfField(
  state: DataShelfState,
  zone: DataShelfZone,
  fieldName: string,
  newIndex: number,
 ...
 */
export function setDataShelfAggregation(
  state: DataShelfState,
  fieldName: string,
  aggregation: DataShelfMeasure['aggregation'],
): DataShelfState {
  const idx = state.values.findIndex(m => m.name === fieldName);
  if (idx === -1) return state;
 ...
 */
export function startDataShelfDrag(
  state: DataShelfState,
 ...
 */
export function setDataShelfHoveredZone(
  state: DataShelfState,
  zone: DataShelfZone | undefined,
): DataShelfState {
 ...
 */
export function endDataShelfDrag(state: DataShelfState): DataShelfState {
 ...
 */
export function toggleDataShelfDropZones(state: DataShelfState): DataShelfState {
 ...
 */
export function setDataShelfSuggestion(
  state: DataShelfState,
 ...
 * - filters → filters
 */
export function applyDataShelfToWidget(state: DataShelfState): {
  dimensions: Array<{ field: string; alias?: string }>;
  measures: Array<{ field: string; aggregation: string; alias?: string }>;
  filters: Array<{ field: string; operator: string; value: unknown }>;
} {
