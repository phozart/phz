...
export function initialHierarchyEditorState(): HierarchyEditorState {
  return {
    hierarchies: [],
    editingId: undefined,
...
export function addHierarchy(
  state: HierarchyEditorState,
...
export function commitEdit(state: HierarchyEditorState): HierarchyEditorState {
  if (!state.editingId || !state.editingDraft) return state;

  return {
    ...state,
    hierarchies: state.hierarchies.map(h =>
      h.id === state.editingId ? state.editingDraft! : h,
    ),
    editingId: undefined,
    editingDraft: undefined,
  };
 ...
 */
export function autoDetectDateHierarchy(
  state: HierarchyEditorState,
  fields: FieldMetadata[],
): HierarchyEditorState {
  const dateField = fields.find(f => f.dataType === 'date');
  if (!dateField) return state;
