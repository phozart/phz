...
import {
  suggestTemplatesForSource,
  applyTemplate,
  saveAsTemplate,
  _resetTemplateWidgetCounter,
} from '../template-selection.js';
import type { TemplateSuggestion } from '../template-selection.js';
...
const minimalSchema: DataSourceSchema = {
  id: 'minimal-ds',
  name: 'Minimal Data',
  fields: [
...
function makeZeroScoreTemplate(): TemplateDefinition {
  return makeTemplate({
...
describe('TemplateSelection', () => {
  ...
  describe('applyTemplate', () => {
    ...
    it('sets layout from template', () => {
      const layout = { kind: 'auto-grid' as const, minItemWidth: 300, gap: 8, children: [] };
