...
import type { WidgetManifest, FilterValue } from '../../types.js';

// --- Fixtures ---

function makeWidget(overrides?: Partial<DashboardWidgetState>): DashboardWidgetState {
  return {
    id: 'w-1',
    type: 'bar-chart',
...
function makeFilter(id: string, field = 'region'): FilterValue {
  return {
    filterId: id,
    field,
    operator: 'equals',
...
describe('WidgetConfigPanelState', () => {
  ...
  describe('addWidgetFilter', () => {
    ...
    it('replaces existing filter with same filterId', () => {
      ...
      const filter2: FilterValue = { ...filter1, value: 'EU', label: 'region: EU' };
