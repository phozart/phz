...
describe('canProceed', () => {
  ...
  it('returns false at choose-source when dataSourceId is not set', () => {
    const state: CreationFlowState = { step: 'choose-source', name: '', artifactType: 'dashboard' };
    expect(canProceed(state)).toBe(false);
  });

  it('returns true at choose-source when dataSourceId is set', () => {
    const state: CreationFlowState = { step: 'choose-source', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    expect(canProceed(state)).toBe(true);
  });

  it('returns false at choose-template when templateId is not set', () => {
    const state: CreationFlowState = { step: 'choose-template', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    expect(canProceed(state)).toBe(false);
  });

  it('returns true at choose-template when templateId is "blank"', () => {
    const state: CreationFlowState = { step: 'choose-template', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'blank' };
    expect(canProceed(state)).toBe(true);
  });

  it('returns true at choose-template when templateId is a real id', () => {
    const state: CreationFlowState = { step: 'choose-template', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'tmpl-executive' };
    expect(canProceed(state)).toBe(true);
  });

  it('returns false at configure when name is empty', () => {
    const state: CreationFlowState = { step: 'configure', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    expect(canProceed(state)).toBe(false);
  });
  ...
  it('returns true at configure when name is set', () => {
    const state: CreationFlowState = { step: 'configure', name: 'My Dashboard', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    expect(canProceed(state)).toBe(true);
  });

  it('returns false at done', () => {
    const state: CreationFlowState = { step: 'done', name: 'Test', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    expect(canProceed(state)).toBe(false);
  });
});
...
describe('nextStep', () => {
  ...
  it('skips choose-template for report — goes directly to configure', () => {
    const state: CreationFlowState = { step: 'choose-source', name: '', artifactType: 'report', dataSourceId: 'ds-1' };
    const next = nextStep(state);
    expect(next.step).toBe('configure');
  });

  it('advances from choose-template to configure for dashboard', () => {
    const state: CreationFlowState = { step: 'choose-template', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'tmpl-1' };
    const next = nextStep(state);
    expect(next.step).toBe('configure');
  });

  it('advances from configure to done', () => {
    const state: CreationFlowState = { step: 'configure', name: 'Test', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'tmpl-1' };
    const next = nextStep(state);
  ...
  it('preserves all state properties when advancing', () => {
    const state: CreationFlowState = { step: 'choose-source', name: 'Draft', artifactType: 'dashboard', dataSourceId: 'ds-1' };
    const next = nextStep(state);
...
describe('prevStep', () => {
  ...
  it('skips choose-template going backwards for report — goes from configure to choose-source', () => {
    const state: CreationFlowState = { step: 'configure', name: '', artifactType: 'report', dataSourceId: 'ds-1' };
    const prev = prevStep(state);
    expect(prev.step).toBe('choose-source');
  });

  it('goes back from configure to choose-template for dashboard', () => {
    const state: CreationFlowState = { step: 'configure', name: '', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'tmpl-1' };
    const prev = prevStep(state);
  ...
  it('preserves state properties when going back', () => {
    const state: CreationFlowState = { step: 'configure', name: 'My Report', artifactType: 'dashboard', dataSourceId: 'ds-1', templateId: 'tmpl-1' };
    const prev = prevStep(state);
...
describe('finishCreation', () => {
  ...
  it('returns null when artifactType is missing', () => {
    const state: CreationFlowState = { step: 'done', name: 'Test', dataSourceId: 'ds-1' };
    expect(finishCreation(state)).toBeNull();
  });

  it('returns null when dataSourceId is missing', () => {
    const state: CreationFlowState = { step: 'done', name: 'Test', artifactType: 'report' };
    expect(finishCreation(state)).toBeNull();
  });

  it('returns null when name is empty', () => {
    const state: CreationFlowState = { step: 'done', name: '', artifactType: 'report', dataSourceId: 'ds-1' };
    expect(finishCreation(state)).toBeNull();
  });
...
describe('state immutability', () => {
  ...
  it('prevStep does not mutate the original state', () => {
    const original: CreationFlowState = { step: 'choose-source', name: '', artifactType: 'dashboard' };
    prevStep(original);
