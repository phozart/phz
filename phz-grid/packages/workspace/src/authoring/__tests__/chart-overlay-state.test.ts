...
import {
  ...
  cancelEditOverlay,
  reorderOverlays,
} from '../chart-overlay-state.js';
import type { ChartOverlay, ReferenceLine, ChartThresholdBand } from '@phozart/phz-engine';
...
describe('addOverlay', () => {
  it('adds an overlay to the list', () => {
    const state = initialChartOverlayState();
    const overlay: ReferenceLine = { id: 'r1', type: 'reference-line', axis: 'y', value: 50 };
  ...
  it('does not mutate original state', () => {
    const state = initialChartOverlayState();
    const overlay: ChartOverlay = { id: 'r1', type: 'average-line' };
...
describe('startEditOverlay', () => {
  it('sets the editing overlay ID and creates a draft copy', () => {
    const overlay: ChartOverlay = { id: 'o1', type: 'reference-line', label: 'Target' };
