...
import {
  type CrossFilterRule,
  type CrossFilterRuleState,
  type CrossFilterFieldMapping,
...
function makeRule(overrides: Partial<CrossFilterRule> = {}): CrossFilterRule {
  return {
    id: 'r-1',
    sourceWidgetId: 'w-a',
    targetWidgetId: 'w-b',
...
describe('CrossFilterRuleState', () => {
  ...
  describe('autoSuggestFieldMapping', () => {
    it('matches fields by name (case-insensitive)', () => {
      const source = [
        { name: 'Region', dataType: 'string' as const, nullable: false },
    ...
    it('falls back to dataType + cardinality matching', () => {
      const source = [
        { name: 'sales_region', dataType: 'string' as const, nullable: false, cardinality: 'low' as const },
      ];
      const target = [
        { name: 'area_code', dataType: 'string' as const, nullable: false, cardinality: 'low' as const },
      ];
      const mappings = autoSuggestFieldMapping(source, target);
    ...
    it('returns empty array when no matches found', () => {
      const source = [
        { name: 'foo', dataType: 'string' as const, nullable: false, cardinality: 'high' as const },
      ];
      const target = [
        { name: 'bar', dataType: 'number' as const, nullable: false, cardinality: 'low' as const },
      ];
      const mappings = autoSuggestFieldMapping(source, target);
