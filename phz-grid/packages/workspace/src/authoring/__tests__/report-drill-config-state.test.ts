...
import {
  ...
  validateDrillConfig,
  type DrillAction,
  type ReportDrillConfigState,
} from '../report-drill-config-state.js';

function makeDrillAction(id: string): DrillAction {
  return {
    id,
    label: `Drill ${id}`,
...
describe('ReportDrillConfigState', () => {
  ...
  describe('updateDrillAction', () => {
    ...
    it('preserves the id even if updates try to override it', () => {
      let s = addDrillAction(initialReportDrillConfigState(), makeDrillAction('d-1'));
      s = updateDrillAction(s, 'd-1', { id: 'hacked' } as Partial<DrillAction>);
      expect(s.drillActions[0].id).toBe('d-1');
    });
  });
