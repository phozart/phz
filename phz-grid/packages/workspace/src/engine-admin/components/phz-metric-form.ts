...
export class PhzMetricForm extends LitElement {
  ...
  @property({ type: Object }) metric?: MetricDef;
  ...
  @state() private _name = '';
  @state() private _mode: FormulaMode = 'simple';
  @state() private _field = '';
  @state() private _aggregation = 'avg';
  @state() private _expression?: ExpressionNode;
  @state() private _format: MetricFormat = { type: 'number', decimals: 1 };
  ...
  willUpdate(changed: Map<string, unknown>) {
    if (changed.has('metric') && this.metric) {
      ...
      if (this.metric.formula.type === 'simple') {
        ...
        this._expression = (this.metric.formula as any).expression;
      }
    }
  }

  private _handleExpressionChange(e: CustomEvent) {
  ...
  private _handleSave() {
    ...
    const formula = this._mode === 'simple'
      ? { type: 'simple' as const, field: this._field, aggregation: this._aggregation as any }
      : { type: 'expression' as const, expression: this._expression! };

    const metric: MetricDef = {
      id,
      name: this._name,
      dataProductId: 'default' as any,
      formula,
      format: this._format,
    };
      ...
      ${this._mode === 'simple' ? html`
        <div class="phz-ea-field">
          ...
          <select class="phz-ea-select" .value=${this._field}
            @change=${(e: Event) => { this._field = (e.target as HTMLSelectElement).value; }}>
        ...
        <div class="phz-ea-field">
          ...
          <select class="phz-ea-select" .value=${this._aggregation}
            @change=${(e: Event) => { this._aggregation = (e.target as HTMLSelectElement).value; }}>
        ...
        <div class="format-row">
          <div class="phz-ea-field" style="flex:1;">
            ...
            <select class="phz-ea-select" .value=${this._format.type}
              @change=${(e: Event) => { this._format = { ...this._format, type: (e.target as HTMLSelectElement).value as any }; }}>
          ...
          <div class="phz-ea-field" style="flex:1;">
            ...
            <input class="phz-ea-input" type="number" min="0" max="6"
              ...
              @input=${(e: Event) => { this._format = { ...this._format, decimals: Number((e.target as HTMLInputElement).value) }; }}>
          </div>
        </div>
        <div class="format-row">
          <div class="phz-ea-field" style="flex:1;">
            ...
            <input class="phz-ea-input" .value=${this._format.prefix ?? ''}
              @input=${(e: Event) => { this._format = { ...this._format, prefix: (e.target as HTMLInputElement).value || undefined }; }}
              placeholder="e.g. $">
          </div>
          <div class="phz-ea-field" style="flex:1;">
            <label class="phz-ea-label">Suffix</label>
            <input class="phz-ea-input" .value=${this._format.suffix ?? ''}
              @input=${(e: Event) => { this._format = { ...this._format, suffix: (e.target as HTMLInputElement).value || undefined }; }}
              placeholder="e.g. %">
          </div>
        </div>
      </div>
...
declare global {
  interface HTMLElementTagNameMap { 'phz-metric-form': PhzMetricForm; }
}
