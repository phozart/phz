...
const OUTPUT_TYPES: Array<{ value: CalculatedFieldOutputType; label: string }> = [
...
export class PhzCalculatedFieldForm extends LitElement {
  static styles = [
    engineAdminStyles,
    css`
      :host { display: block; }
  ...
  @property({ type: Object }) calculatedField?: CalculatedFieldDef;
  ...
  @state() private _name = '';
  @state() private _outputType: CalculatedFieldOutputType = 'number';
  ...
  private _getPreviewResults(): Array<{ row: Record<string, unknown>; result: unknown }> {
    if (!this._expression || this.previewData.length === 0) return [];
  ...
  private _handleSave() {
    if (!this._name || !this._expression) return;
...
declare global {
  interface HTMLElementTagNameMap { 'phz-calculated-field-form': PhzCalculatedFieldForm; }
}
