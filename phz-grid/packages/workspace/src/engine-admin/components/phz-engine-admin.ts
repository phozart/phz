...
type AdminTab = 'kpi' | 'dashboard' | 'metric' | 'report' | 'pivot' | 'data' | 'selection';

@safeCustomElement('phz-engine-admin')
export class PhzEngineAdmin extends LitElement {
  static styles = [
    engineAdminStyles,
    css`
      :host { display: block; }
  ...
  @property({ type: Array }) selectionFields: SelectionFieldDef[] = [];

  @state() private activeTab: AdminTab = 'kpi';
...
declare global {
  interface HTMLElementTagNameMap { 'phz-engine-admin': PhzEngineAdmin; }
}
