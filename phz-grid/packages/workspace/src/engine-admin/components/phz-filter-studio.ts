...
import type {
  FilterDefinition, FilterDefinitionId, SelectionFieldType, SessionBehavior,
  SelectionFieldOption, TreeNode, DateRangeFieldConfig, NumericRangeFieldConfig,
  SearchFieldConfig, FieldPresenceConfig,
...
const TYPE_CATALOG: TypeCatalogItem[] = [
...
const OPTIONS_TYPES: SelectionFieldType[] = ['single_select', 'multi_select', 'chip_group'];
...
export class PhzFilterStudio extends LitElement {
  static styles = [
    engineAdminStyles,
    css`
  ...
  @property({ type: Object }) definition?: FilterDefinition;
  ...
  @state() private selectedType: SelectionFieldType = 'single_select';
  @state() private draft: Partial<FilterDefinition> = {};
  @state() private _treeSourceDsId = '';
  @state() private _treeLevels: TreeLevelConfig[] = [{ field: '' }, { field: '' }];
  @state() private _importColumn = '';
  @state() private _validationError = '';
  @state() private _selectedDataSourceId = '';
  @state() private _selectedValueField = '';
  @state() private _labelTemplate = '';

  /* ── Draft helpers ── */

  private _options: SelectionFieldOption[] = [];
  private _treeOptions: TreeNode[] = [];
  ...
  private _updateDraft<K extends keyof FilterDefinition>(key: K, value: FilterDefinition[K]) {
    this.draft = { ...this.draft, [key]: value };
  }

  private _selectType(type: SelectionFieldType) {
  ...
  private _handleStudioDataSourceChange(dsId: string) {
    this._selectedDataSourceId = dsId;
    this._selectedValueField = '';
    this._labelTemplate = '';
    if (dsId && !this.draft.label) {
      const ds = this.dataSources.find(d => d.id === dsId);
      if (ds) this._updateDraft('label', ds.name);
    }
  }

  private _renderStudioTemplatePreview() {
    if (!this._labelTemplate || !this._selectedDataSourceId) return nothing;
  ...
  private _handleSave() {
    ...
    const now = Date.now();
    const id: FilterDefinitionId = this.definition?.id ?? filterDefinitionId(label.toLowerCase().replace(/\s+/g, '_'));
    ...
    let treeSource: TreeSourceConfig | undefined;
    let treeOptions: TreeNode[] | undefined;
  ...
  private _updateOption(idx: number, field: 'value' | 'label', val: string) {
    this._options = this._options.map((o, i) => i === idx ? { ...o, [field]: val } : o);
    this.requestUpdate();
  }

  private _removeOption(idx: number) {
  ...
  private _removeTreeLevel(idx: number) {
    if (this._treeLevels.length <= 2) return;
    this._treeLevels = this._treeLevels.filter((_, i) => i !== idx);
  }

  private _updateTreeLevel(idx: number, patch: Partial<TreeLevelConfig>) {
    this._treeLevels = this._treeLevels.map((l, i) => i === idx ? { ...l, ...patch } : l);
  }

  private _getTreeSourceColumns(): string[] {
    if (this._treeSourceDsId) {
      const ds = this.dataSources.find(d => d.id === this._treeSourceDsId);
      if (ds) return ds.columns;
    }
    return this._getDataColumns();
  }

  private _getTreeSourceRows(): Record<string, unknown>[] {
    if (this._treeSourceDsId) {
      const ds = this.dataSources.find(d => d.id === this._treeSourceDsId);
      if (ds?.sampleRows?.length) return ds.sampleRows;
    }
    return this.data;
  }

  private _computeTreePreview(): TreeNode[] {
    const validLevels = this._treeLevels.filter(l => l.field);
  ...
  private _getDataColumns(): string[] {
    if (this._selectedDataSourceId) {
      const ds = this.dataSources.find(d => d.id === this._selectedDataSourceId);
      if (ds) return ds.columns;
    }
    if (this.data.length === 0) return this.availableColumns;
      ...
      <div class="section">
        ...
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input ${this._validationError ? 'phz-ea-input--error' : ''}"
                 ...
                 @input=${(e: Event) => {
                   this._updateDraft('label', (e.target as HTMLInputElement).value);
                   if (this._validationError) this._validationError = '';
                 }}
                 @keydown=${(e: KeyboardEvent) => { if (e.key === 'Enter') this._handleSave(); }}
        ...
        ${this.dataSources.length > 0 && this.selectedType !== 'tree_select' ? html`
          <div class="phz-ea-field">
            ...
            <phz-combobox
              ...
              allow-empty
              empty-label="\u2014 None \u2014"
              @combobox-change=${(e: CustomEvent) => this._handleStudioDataSourceChange((e.detail as { value: string }).value)}
            ></phz-combobox>
          </div>
          ${this._selectedDataSourceId ? html`
            <div class="phz-ea-field">
              ...
              <phz-combobox
                ...
                allow-empty
                empty-label="\u2014 Select \u2014"
                @combobox-change=${(e: CustomEvent) => this._selectedValueField = (e.detail as { value: string }).value}
              ></phz-combobox>
            </div>
            <div class="phz-ea-field">
              <label class="phz-ea-label">Label Template</label>
              <input class="phz-ea-input" .value=${this._labelTemplate}
                     @input=${(e: Event) => this._labelTemplate = (e.target as HTMLInputElement).value}
  ...
  private _renderTypeConfig() {
    ...
    if (OPTIONS_TYPES.includes(t)) return this._renderOptionsConfig();
    if (t === 'text') return this._renderTextConfig();
    if (t === 'search') return this._renderSearchConfig();
    if (t === 'date_range') return this._renderDateRangeConfig();
    if (t === 'numeric_range') return this._renderNumericRangeConfig();
    if (t === 'tree_select') return this._renderTreeConfig();
    if (t === 'field_presence') return this._renderFieldPresenceConfig();
    return nothing;
  }
          ...
          <div class="import-bar">
            <div class="phz-ea-field">
              ...
              <phz-combobox
                ...
                @combobox-change=${(e: CustomEvent) => this._importColumn = (e.detail as { value: string }).value}
              ></phz-combobox>
            </div>
        ...
        ${this._options.map((opt, idx) => html`
          <div class="option-row">
            <input class="phz-ea-input" .value=${opt.value} placeholder="Value"
                   @input=${(e: Event) => this._updateOption(idx, 'value', (e.target as HTMLInputElement).value)}
                   style="flex:1">
            <input class="phz-ea-input" .value=${opt.label} placeholder="Label"
                   @input=${(e: Event) => this._updateOption(idx, 'label', (e.target as HTMLInputElement).value)}
                   style="flex:1">
  ...
  private _renderTextConfig() {
    const placeholder = (this.draft.metadata as Record<string, unknown> | undefined)?.placeholder as string ?? '';
    return html`
      <div class="section">
        ...
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" .value=${placeholder}
                 ...
                 @input=${(e: Event) => this._updateDraft('metadata', { ...(this.draft.metadata ?? {}), placeholder: (e.target as HTMLInputElement).value } as never)}>
        </div>
      </div>
    `;
  }
  ...
  private _renderSearchConfig() {
    ...
    return html`
      <div class="section">
        ...
        <div class="phz-ea-field">
          <label class="phz-ea-label">Placeholder</label>
          <input class="phz-ea-input" .value=${(this.draft.metadata as Record<string, unknown> | undefined)?.placeholder as string ?? ''}
                 placeholder="Search..."
                 @input=${(e: Event) => this._updateDraft('metadata', { ...(this.draft.metadata ?? {}), placeholder: (e.target as HTMLInputElement).value } as never)}>
        </div>
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" type="number" .value=${String(cfg.minChars ?? 2)} min="1"
                 @input=${(e: Event) => this._updateDraft('searchConfig', { ...cfg, minChars: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" type="number" .value=${String(cfg.debounceMs ?? 300)} min="0" step="50"
                 @input=${(e: Event) => this._updateDraft('searchConfig', { ...cfg, debounceMs: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" type="number" .value=${String(cfg.maxSuggestions ?? 10)} min="1"
                 @input=${(e: Event) => this._updateDraft('searchConfig', { ...cfg, maxSuggestions: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
  ...
  private _renderDateRangeConfig() {
    const cfg: DateRangeFieldConfig = this.draft.dateRangeConfig ?? {};
      ...
      <div class="section">
        ...
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" type="number" .value=${String(cfg.fiscalYearStartMonth ?? 1)} min="1" max="12"
                 @input=${(e: Event) => this._updateDraft('dateRangeConfig', { ...cfg, fiscalYearStartMonth: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
        ...
        <div class="phz-ea-field" style="margin-top:10px">
          ...
          <input class="phz-ea-input" type="date" .value=${cfg.minDate ?? ''}
                 @input=${(e: Event) => this._updateDraft('dateRangeConfig', { ...cfg, minDate: (e.target as HTMLInputElement).value })}>
        </div>
        <div class="phz-ea-field">
          <label class="phz-ea-label">Max Date</label>
          <input class="phz-ea-input" type="date" .value=${cfg.maxDate ?? ''}
                 @input=${(e: Event) => this._updateDraft('dateRangeConfig', { ...cfg, maxDate: (e.target as HTMLInputElement).value })}>
        </div>
      </div>
    `;
  }
  ...
  private _renderNumericRangeConfig() {
    const cfg: NumericRangeFieldConfig = this.draft.numericRangeConfig ?? {};
    return html`
      <div class="section">
        ...
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" type="number" .value=${String(cfg.min ?? 0)}
                 @input=${(e: Event) => this._updateDraft('numericRangeConfig', { ...cfg, min: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
          <label class="phz-ea-label">Max</label>
          <input class="phz-ea-input" type="number" .value=${String(cfg.max ?? 100)}
                 @input=${(e: Event) => this._updateDraft('numericRangeConfig', { ...cfg, max: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
          <label class="phz-ea-label">Step</label>
          <input class="phz-ea-input" type="number" .value=${String(cfg.step ?? 1)} min="0.01" step="any"
                 @input=${(e: Event) => this._updateDraft('numericRangeConfig', { ...cfg, step: Number((e.target as HTMLInputElement).value) })}>
        </div>
        <div class="phz-ea-field">
          <label class="phz-ea-label">Unit</label>
          <input class="phz-ea-input" .value=${cfg.unit ?? ''} placeholder="e.g. USD, kg, %"
                 @input=${(e: Event) => this._updateDraft('numericRangeConfig', { ...cfg, unit: (e.target as HTMLInputElement).value })}>
        </div>
        <div class="toggle-row">
        ...
        ${this.dataSources.length > 0 ? html`
          <div class="phz-ea-field">
            ...
            <phz-combobox
              ...
              @combobox-change=${(e: CustomEvent) => { this._treeSourceDsId = (e.detail as { value: string }).value; }}
            ></phz-combobox>
          </div>
        ` : nothing}
            ...
            <div class="tree-build-panel" style="margin-bottom:8px">
              ...
              <div class="phz-ea-field" style="margin-bottom:6px">
                ...
                <phz-combobox
                  ...
                  allow-empty
                  empty-label="\u2014 select column \u2014"
                  @combobox-change=${(e: CustomEvent) => this._updateTreeLevel(idx, { field: (e.detail as { value: string }).value })}
                ></phz-combobox>
              </div>
              <div class="phz-ea-field" style="margin-bottom:0">
                ...
                <input class="phz-ea-input" .value=${level.labelTemplate ?? ''}
                       ...
                       @input=${(e: Event) => this._updateTreeLevel(idx, { labelTemplate: (e.target as HTMLInputElement).value || undefined })}>
              </div>
            </div>
          `;
        })}
  ...
  private _countTreeNodes(nodes: TreeNode[]): number {
    let count = 0;
    for (const n of nodes) {
      count++;
      if (n.children?.length) count += this._countTreeNodes(n.children);
    }
    return count;
  }

  private _renderFieldPresenceConfig() {
    const cfg: FieldPresenceConfig = this.draft.fieldPresenceConfig ?? { fields: [] };
    return html`
      <div class="section">
        ...
        <div class="phz-ea-field">
          ...
          <input class="phz-ea-input" .value=${(cfg.fields ?? []).join(', ')}
                 @input=${(e: Event) => {
                   const fields = (e.target as HTMLInputElement).value.split(',').map(s => s.trim()).filter(Boolean);
  ...
  private _renderPreviewControl() {
    ...
    if (OPTIONS_TYPES.includes(t)) return this._renderOptionsPreview(label);
    if (t === 'text') return this._renderTextPreview(label);
    if (t === 'search') return this._renderSearchPreview(label);
    if (t === 'date_range') return this._renderDateRangePreview(label);
    if (t === 'numeric_range') return this._renderNumericPreview(label);
    if (t === 'tree_select') return this._renderTreePreview(label);
    if (t === 'field_presence') return this._renderPresencePreview(label);
    if (t === 'period_picker') return this._renderPeriodPreview(label);

    return html`<div class="preview-empty">Select a filter type</div>`;
  }

  private _renderOptionsPreview(label: string) {
  ...
  private _renderTextPreview(label: string) {
    const placeholder = (this.draft.metadata as Record<string, unknown> | undefined)?.placeholder as string || 'Enter text...';
    return html`
      <div class="preview-card">
        <div class="preview-label">${label}</div>
  ...
  private _renderSearchPreview(label: string) {
    const cfg = this.draft.searchConfig ?? {};
    const mode = cfg.matchMode ?? 'contains';
    const multi = cfg.multiValue ?? false;
    const placeholder = multi ? 'Space-separated values...' : 'Search...';
    const hints: string[] = [mode === 'beginsWith' ? 'prefix' : 'substring'];
    if (multi) hints.push('multi-value');
    return html`
      <div class="preview-card">
        <div class="preview-label">${label}</div>
  ...
  private _renderDateRangePreview(label: string) {
  ...
  private _renderNumericPreview(label: string) {
  ...
  private _renderTreePreview(label: string) {
    const preview = this._computeTreePreview();
  ...
  private _renderTreePreviewNodes(nodes: TreeNode[], depth: number): unknown {
  ...
  private _renderPresencePreview(label: string) {
  ...
  private _renderPeriodPreview(label: string) {
    return html`
      <div class="preview-card">
        <div class="preview-label">${label}</div>
        <div class="chip-group">
...
declare global {
  interface HTMLElementTagNameMap { 'phz-filter-studio': PhzFilterStudio; }
}
