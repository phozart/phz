...
const AGGREGATION_MAP: Array<{ pattern: RegExp; aggregation: KPIAggregation }> = [
...
const PERIOD_MAP: Array<{ pattern: RegExp; period: KPIPeriod }> = [
...
function parseNumber(text: string): { value: number; unit?: KPIUnit } | null {
  const match = text.match(NUMBER_PATTERN);
  if (!match) return null;

  let value = parseFloat(match[1].replace(/,/g, ''));
  let unit: KPIUnit | undefined;

  const suffix = match[2]?.toUpperCase();
  if (suffix === 'K') value *= 1_000;
  else if (suffix === 'M') value *= 1_000_000;
  else if (suffix === 'B') value *= 1_000_000_000;
  else if (suffix === '%') unit = 'percent';

  // Check for dollar sign
  if (text.includes('$')) unit = 'currency';
...
function inferDirection(text: string, comparison?: KPIComparison): KPIDirection {
  if (comparison === 'below') return 'lower_is_better';
  if (LOWER_IS_BETTER_HINTS.test(text)) return 'lower_is_better';
...
function extractFieldHint(text: string, threshold?: number): string {
  let cleaned = text;
...
function generateName(text: string): string {
  ...
  if (!cleaned) return 'KPI';
  return cleaned.charAt(0).toUpperCase() + cleaned.slice(1);
}

// --- Main Parser ---

export function parseKPIDescription(text: string): ParsedKPIDescription {
  // Aggregation
  let aggregation: KPIAggregation = 'sum';
  ...
  // Comparison
  let comparison: KPIComparison | undefined;
