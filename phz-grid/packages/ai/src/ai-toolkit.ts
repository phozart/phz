...
function sanitizeForPrompt(input: string, maxLength = MAX_NL_QUERY_LENGTH): string {
  return input
    .slice(0, maxLength)
    .replace(/[\x00-\x1f]/g, '')
...
class AIToolkitImpl implements AIToolkit {
  ...
  async inferSchema(sampleData: unknown[], options?: InferSchemaOptions): Promise<ColumnDefinition[]> {
    ...
    try {
      const result = await this.config.provider.generateStructuredOutput<{ columns: Array<{ field: string; type: string; header: string }> }>(
        prompt,
        {
          type: 'object',
          properties: {
      ...
      return result.columns.map((c) => ({
        field: c.field,
        header: c.header,
        type: c.type as 'string' | 'number' | 'boolean' | 'date' | 'custom',
        sortable: true,
        filterable: true,
      }));
    } catch {
  ...
  async validateSchema(schema: ColumnDefinition[], data: unknown[]): Promise<SchemaValidationResult> {
    const errors: Array<{ row: number; column: string; error: string }> = [];
    const warnings: Array<{ row: number; column: string; warning: string }> = [];
    let checkedCells = 0;
    let validCells = 0;
  ...
  async executeNaturalLanguageQuery(query: string, options?: NLQueryOptions): Promise<AIQueryResult> {
...
Return ONLY a SELECT query, no explanation. Never generate DROP, DELETE, INSERT, UPDATE, ALTER, CREATE, TRUNCATE, EXEC, GRANT, or REVOKE statements.`;
  ...
  async explainQuery(sql: string): Promise<string> {
    const result = await this.config.provider.generateCompletion(
      `Explain this SQL query in plain English:\n${sanitizeForPrompt(sql, 2000)}`,
      { temperature: 0.3, maxTokens: 300 },
    );
    return result.text.trim();
  }

  async suggestQueries(context?: string): Promise<string[]> {
...
Suggest 5 useful analytical queries a user might want to run. Return as a JSON array of strings.`;
    ...
    try {
      const result = await this.config.provider.generateStructuredOutput<{ queries: string[] }>(
        prompt,
        {
          type: 'object',
  ...
  async detectAnomalies(column: string, options?: AnomalyDetectionOptions): Promise<AnomalyResult[]> {
    if (!this.grid) return [];
    const data = this.grid.getData();
    const values = data.map((row) => (row as Record<string, unknown>)[column]);
  ...
  async suggestDataTypes(sampleData: unknown[]): Promise<DataTypeSuggestion[]> {
    if (sampleData.length === 0) return [];
    const first = sampleData[0] as Record<string, unknown>;
    const suggestions: DataTypeSuggestion[] = [];

    for (const [column, value] of Object.entries(first)) {
      const currentType = typeof value;
      const allValues = sampleData.map((r) => (r as Record<string, unknown>)[column]);
  ...
  async detectDuplicates(columns?: string[]): Promise<DuplicateResult[]> {
    if (!this.grid) return [];
    const data = this.grid.getData();
    const keys = columns ?? Object.keys((data[0] ?? {}) as Record<string, unknown>).filter((k) => k !== '__id');
    const seen = new Map<string, Array<{ rowId: string; row: Record<string, unknown> }>>();
    ...
    for (const [, entries] of seen) {
      if (entries.length > 1) {
        ...
        for (const k of keys) values[k] = entries[0].row[k];
        duplicates.push({
  ...
  async summarize(options?: SummarizeOptions): Promise<string> {
    if (!this.grid) return '';
    const data = this.grid.getData();
    const columns = options?.columns ?? Object.keys((data[0] ?? {}) as Record<string, unknown>).filter((k) => k !== '__id');
...
Sample data: ${JSON.stringify(this.redactSample(sample.slice(0, 10)))}`;
  ...
  async generateInsights(columns?: string[]): Promise<Insight[]> {
    if (!this.grid) return [];
    const data = this.grid.getData();
    const cols = columns ?? Object.keys((data[0] ?? {}) as Record<string, unknown>).filter((k) => k !== '__id');
...
Return insights as JSON array with objects having: type, title, description, columns, confidence`;
    ...
    try {
      const result = await this.config.provider.generateStructuredOutput<{ insights: Insight[] }>(
        prompt,
        {
          type: 'object',
          properties: {
  ...
  async suggestFilters(input: string): Promise<FilterSuggestion[]> {
    const columns = this.getGridColumns();
...
Valid operators: equals, notEquals, contains, startsWith, greaterThan, lessThan, between, isNull, isNotNull`;
    ...
    try {
      const result = await this.config.provider.generateStructuredOutput<{ filters: FilterSuggestion[] }>(
        prompt,
        {
          type: 'object',
          properties: {
  ...
  async autoCompleteValue(column: string, partial: string): Promise<string[]> {
    if (!this.grid) return [];
    const data = this.grid.getData();
    const values = new Set<string>();
    for (const row of data) {
      const v = String((row as Record<string, unknown>)[column] ?? '');
  ...
  private redactSample(data: unknown[]): unknown[] {
    ...
    return data.map(row => {
      ...
      for (const field of redactSet) {
        if (field in r) r[field] = '[REDACTED]';
      }
      return r;
    });
  }
  ...
  private getGridColumns(): ColumnDefinition[] {
    if (!this.grid) return [];
    const state = this.grid.getState();
  ...
  private buildInferSchemaPrompt(sample: unknown[], options?: InferSchemaOptions): string {
...
Return columns with: field, type (string/number/boolean/date/custom), header (human-readable name).`;
  ...
  private heuristicInferSchema(sample: unknown[]): ColumnDefinition[] {
    if (sample.length === 0) return [];
    const first = sample[0] as Record<string, unknown>;
    return Object.keys(first).map((field) => {
      const values = sample.map((r) => (r as Record<string, unknown>)[field]);
  ...
  private inferColumnType(values: unknown[]): 'string' | 'number' | 'boolean' | 'date' {
  ...
  private suggestType(column: string, values: unknown[], currentType: string): DataTypeSuggestion | null {
    const nonNull = values.filter((v) => v !== null && v !== undefined);
    ...
    if (currentType === 'string') {
      ...
      if (dateCount / nonNull.length > 0.9) {
        return {
          ...
          examples: nonNull.slice(0, 3).map((v) => ({ value: v, parsedValue: new Date(v as string).toISOString() })),
        };
      }
