...
export interface GeneratedKPIConfig {
  ...
  dataSource: { scoreEndpoint: string; trendEndpoint?: string };
  dimensions: string[];
}
...
function matchField(hint: string, fields: KPIGeneratorField[]): KPIGeneratorField | undefined {
  ...
  let bestMatch: KPIGeneratorField | undefined;
  let bestScore = 0;
...
export function generateKPIConfig(
  description: string,
  schema: KPIGeneratorSchema,
): GeneratedKPIConfig {
