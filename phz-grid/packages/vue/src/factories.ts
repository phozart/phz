 ...
 */
import type { GridApi, ColumnDefinition, RowId, CellPosition, FilterOperator } from '@phozart/phz-core';
import type {
  PhzGridProps,
  UseGridReturn,
...
export function createPhzGridComponent(vue: VueRuntime): any {
  return vue.defineComponent({
    ...
    setup(props: any, { emit, slots }: any) {
      ...
      const gridApi = vue.ref<GridApi | null>(null);

      function syncAllProperties(el: any) {
        el.data = props.data;
        el.columns = props.columns;
        if (props.pageSizeOptions != null) el.pageSizeOptions = props.pageSizeOptions;
        if (props.showToolbar != null) el.showToolbar = props.showToolbar;
        if (props.showDensityToggle != null) el.showDensityToggle = props.showDensityToggle;
        if (props.showColumnEditor != null) el.showColumnEditor = props.showColumnEditor;
        if (props.showAdminSettings != null) el.showAdminSettings = props.showAdminSettings;
        if (props.showPagination != null) el.showPagination = props.showPagination;
        if (props.showCheckboxes != null) el.showCheckboxes = props.showCheckboxes;
        if (props.showRowActions != null) el.showRowActions = props.showRowActions;
        if (props.showSelectionActions != null) el.showSelectionActions = props.showSelectionActions;
        if (props.showEditActions != null) el.showEditActions = props.showEditActions;
        if (props.showCopyActions != null) el.showCopyActions = props.showCopyActions;
        if (props.rowBanding != null) el.rowBanding = props.rowBanding;
        if (props.statusColors != null) el.statusColors = props.statusColors;
        if (props.barThresholds != null) el.barThresholds = props.barThresholds;
        if (props.dateFormats != null) el.dateFormats = props.dateFormats;
        if (props.numberFormats != null) el.numberFormats = props.numberFormats;
        if (props.columnStyles != null) el.columnStyles = props.columnStyles;
        if (props.hoverHighlight != null) el.hoverHighlight = props.hoverHighlight;
        if (props.compactNumbers != null) el.compactNumbers = props.compactNumbers;
        if (props.autoSizeColumns != null) el.autoSizeColumns = props.autoSizeColumns;
        if (props.aggregation != null) el.aggregation = props.aggregation;
        if (props.groupBy != null) el.groupBy = props.groupBy;
        if (props.groupByLevels != null) el.groupByLevels = props.groupByLevels;
        if (props.groupTotals != null) el.groupTotals = props.groupTotals;
        if (props.groupTotalsFn != null) el.groupTotalsFn = props.groupTotalsFn;
        if (props.conditionalFormattingRules != null) el.conditionalFormattingRules = props.conditionalFormattingRules;
        if (props.columnGroups != null) el.columnGroups = props.columnGroups;
        if (props.copyHeaders != null) el.copyHeaders = props.copyHeaders;
        if (props.copyFormatted != null) el.copyFormatted = props.copyFormatted;
      }
      ...
      vue.onMounted(() => {
        ...
        el.addEventListener('grid-ready', (e: CustomEvent) => {
        ...
        el.addEventListener('sort-change', (e: CustomEvent) => emit('sort-change', e.detail));
        el.addEventListener('filter-change', (e: CustomEvent) => emit('filter-change', e.detail));
        el.addEventListener('edit-commit', (e: CustomEvent) => emit('edit-commit', e.detail));
        el.addEventListener('cell-click', (e: CustomEvent) => emit('cell-click', e.detail));
      });

      vue.watch(() => props.data, (newData: unknown[]) => {
        const el = gridRef.value as any;
        if (el) el.data = newData;
      });

      vue.watch(() => props.columns, (newCols: ColumnDefinition[]) => {
        const el = gridRef.value as any;
        if (el) el.columns = newCols;
      });
...
export function createUseGrid(vue: VueRuntime) {
  return function useGrid(): UseGridReturn {
    const gridInstance = vue.ref<GridApi | null>(null);
...
export function createUseGridSelection(vue: VueRuntime) {
  return function useGridSelection(gridInstance?: { value: GridApi | null }): UseGridSelectionReturn {
    const selectedRows = vue.ref<RowId[]>([]);
    const selectedCells = vue.ref<CellPosition[]>([]);

    if (gridInstance) {
      vue.watch(() => gridInstance.value, (grid: GridApi | null) => {
        if (!grid) return;
        const sync = () => {
...
export function createUseGridSort(vue: VueRuntime) {
  return function useGridSort(gridInstance?: { value: GridApi | null }): UseGridSortReturn {
    const sortState = vue.ref(null);

    if (gridInstance) {
      vue.watch(() => gridInstance.value, (grid: GridApi | null) => {
        if (!grid) return;
        sortState.value = grid.getSortState() as any;
        grid.on('sort:change', () => { sortState.value = grid.getSortState() as any; });
      });
    }
...
export function createUseGridFilter(vue: VueRuntime) {
  return function useGridFilter(gridInstance?: { value: GridApi | null }): UseGridFilterReturn {
    const filterState = vue.ref(null);

    if (gridInstance) {
      vue.watch(() => gridInstance.value, (grid: GridApi | null) => {
        if (!grid) return;
        filterState.value = grid.getFilterState() as any;
        grid.on('filter:change', () => { filterState.value = grid.getFilterState() as any; });
      });
    }

    return {
      filterState,
      addFilter: (f, o, v) => gridInstance?.value?.addFilter(f, o as FilterOperator, v),
...
export function createUseGridEdit(vue: VueRuntime) {
  return function useGridEdit(gridInstance?: { value: GridApi | null }): UseGridEditReturn {
    const editState = vue.ref(null);
    const isDirty = vue.ref(false);
    const dirtyRows = vue.ref<RowId[]>([]);

    if (gridInstance) {
      vue.watch(() => gridInstance.value, (grid: GridApi | null) => {
        if (!grid) return;
        const sync = () => {
          editState.value = grid.getEditState() as any;
