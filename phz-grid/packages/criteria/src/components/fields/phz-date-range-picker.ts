...
export class PhzDateRangePicker extends LitElement {
  ...
  @property({ type: Object }) config: DateRangeFieldConfig = {};
  @property({ type: Object }) value: DateRangeValue | null = null;
  @property({ type: Boolean }) disabled = false;
  /** Render the panel inline (always open, no popup/backdrop). Useful for previews. */
  @property({ type: Boolean }) inline = false;

  @state() private _open = false;
  @state() private _granularity: DateGranularity = 'day';
  @state() private _viewYear = new Date().getFullYear();
  @state() private _viewMonth = new Date().getMonth();
  @state() private _selecting: 'start' | 'end' = 'start';
  @state() private _tempStart: string | null = null;
  @state() private _tempEnd: string | null = null;
  @state() private _showComparison = false;
  @state() private _compType: ComparisonType = 'previous_period';
  @state() private _panelStyle = '';

  // --- Config helpers ---

  private get _weekStart(): WeekStartDay { return this.config.weekStartDay ?? 'monday'; }
  private get _fiscal(): number { return this.config.fiscalYearStartMonth ?? 1; }
  private get _weekNumbering() { return this.config.weekNumbering ?? 'iso'; }
  private get _minDate(): string | undefined { return this.config.minDate; }
  private get _maxDate(): string | undefined { return this.config.maxDate; }

  /** Check if a date string (YYYY-MM-DD) is within the allowed min/max range */
  private _isDateDisabled(dateStr: string): boolean {
    if (this._minDate && dateStr < this._minDate) return true;
    if (this._maxDate && dateStr > this._maxDate) return true;
    return false;
  }

  /** Check if a period (start-end) is entirely outside the allowed range */
  private _isPeriodDisabled(startStr: string, endStr: string): boolean {
    if (this._minDate && endStr < this._minDate) return true;
    if (this._maxDate && startStr > this._maxDate) return true;
    return false;
  }

  private get _granularities(): DateGranularity[] {
    return this.config.availableGranularities ?? ALL_GRANULARITIES;
  }

  private get _presets(): DatePresetDef[] {
    return getAvailablePresets(this.config);
  }

  private get _groupedPresets(): Map<DatePresetGroup, DatePresetDef[]> {
    const map = new Map<DatePresetGroup, DatePresetDef[]>();
  ...
  private _positionPanel() {
    this.updateComplete.then(() => {
      const trigger = this.renderRoot.querySelector('.phz-sc-dp-trigger') as HTMLElement;
      if (!trigger) return;
  ...
  private _selectPreset(presetId: BuiltinDatePresetId) {
  ...
  private _selectWeek(weekStartDate: Date) {
    ...
    if (this._selecting === 'start') {
      ...
      let re = e;
      if (re < rs) { rs = s; re = this._tempEnd ?? e; }
      else { rs = this._tempStart!; }
  ...
  private _selectMonth(year: number, month: number) {
    ...
    if (this._selecting === 'start') {
      ...
      let rs = this._tempStart!;
      let re = e;
      if (s < rs) { rs = s; re = this._tempEnd ?? e; }
      this._confirmCustomRange(rs, re, 'month');
    }
  }

  private _selectQuarter(year: number, quarter: number) {
  ...
  private _confirmCustomRange(start: string, end: string, granularity: DateGranularity) {
  ...
  private _fireChange(range: DateRangeValue | null) {
    this.value = range;
  ...
  private _prevYear() { this._viewYear--; this.requestUpdate(); }
  private _nextYear() { this._viewYear++; this.requestUpdate(); }

  // --- Calendar helpers ---

  private _getDaysInMonth(y: number, m: number) { return new Date(y, m + 1, 0).getDate(); }

  private _getFirstDayOffset(y: number, m: number): number {
    const day = new Date(y, m, 1).getDay(); // 0=Sun
    if (this._weekStart === 'monday') return (day + 6) % 7;
    return day;
  }
  ...
  private _renderTrigger() {
    ...
    if (this._open && this._tempStart) {
      ...
      if (this.value.comparisonStartDate && this.value.comparisonEndDate) {
        const compRange: DateRangeValue = { startDate: this.value.comparisonStartDate, endDate: this.value.comparisonEndDate };
  ...
  private _renderDayCalendar() {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (disabled) classes.push('phz-sc-calendar-day--disabled');
      else if (this._isSelected(dateStr)) classes.push('phz-sc-calendar-day--selected');
      else if (this._isInRange(dateStr)) classes.push('phz-sc-calendar-day--in-range');
      if (isToday) classes.push('phz-sc-calendar-day--today');
      cells.push(html`
  ...
  private _renderWeekCalendar() {
    ...
    for (let i = 0; i < firstDayOffset; i++) allDays.push(null);
    for (let d = 1; d <= days; d++) allDays.push(d);
      ...
      <div class="phz-sc-calendar">
        ...
        <div class="phz-sc-calendar-grid phz-sc-calendar-grid--weeks">
          ...
          ${weeks.map(week => {
            ...
            if (!firstRealDay) return nothing;
            ...
            return html`
              ...
              ${week.map(day => {
                if (day === null) return html`<div class="phz-sc-calendar-day phz-sc-calendar-day--other-month"></div>`;
                const dateStr = `${y}-${pad2(m + 1)}-${pad2(day)}`;
                const isToday = dateStr === todayStr;
                const classes = ['phz-sc-calendar-day'];
                if (weekDisabled) classes.push('phz-sc-calendar-day--disabled');
                else if (isSelectedWeek) classes.push('phz-sc-calendar-day--selected');
                else if (isInRangeWeek) classes.push('phz-sc-calendar-day--in-range');
                if (isToday) classes.push('phz-sc-calendar-day--today');
      ...
      <div>
        ...
        <div class="phz-sc-dp-period-grid phz-sc-dp-period-grid--months">
          ${MONTH_SHORT.map((name, idx) => {
            ...
            if (disabled) classes.push('phz-sc-dp-period-cell--disabled');
            else if (isSelected) classes.push('phz-sc-dp-period-cell--selected');
            else if (inRange) classes.push('phz-sc-dp-period-cell--in-range');
        ...
        <div class="phz-sc-dp-comp-toggle">
          <label class="phz-sc-toggle-switch" style="width:32px; height:18px">
            <input type="checkbox" .checked=${this._showComparison}
              @change=${(e: Event) => { this._showComparison = (e.target as HTMLInputElement).checked; }} />
