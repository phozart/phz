...
export class PhzFilterDrawer extends LitElement {
  ...
  @property({ type: Number }) minWidth = 320;
  @property({ type: Number }) maxWidth = 0; // 0 = 90vw
  @property({ type: Boolean }) showBackdrop = true;
  @property({ type: Boolean }) resizable = true;
  @property({ type: String }) drawerTitle = 'Filters';
  /** Whether the pin button is shown */
  @property({ type: Boolean }) pinnable = false;
  ...
  @state() private _transitionDone = false;

  @query('.phz-sc-drawer-panel') private _panel?: HTMLElement;
