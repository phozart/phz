...
@customElement('phz-filter-configurator')
export class PhzFilterConfigurator extends LitElement {
  ...
  @property({ attribute: 'artefact-id' }) artefactId: string = '';
  @property({ attribute: 'artefact-name' }) artefactName: string = '';
  ...
  @state() private _hasPicker = false;
  @state() private _pickerChecked = false;
  @state() private _pickerSelected: Set<string> = new Set();
  ...
  private _renderBindingCard(binding: FilterBinding, idx: number) {
    const def = (this.definitions ?? []).find(d => d.id === binding.filterDefinitionId);
    const label = binding.labelOverride || def?.label || (binding.filterDefinitionId as string);
      ...
      <div class="phz-fc-card">
        <div class="phz-fc-card-collapsed"
          ...
          <button class="phz-fc-toggle ${binding.visible ? 'phz-fc-toggle--on' : 'phz-fc-toggle--off'}"
                  @click=${(e: Event) => { e.stopPropagation(); this._toggleVisibility(binding); }}
                  aria-label="${binding.visible ? 'Hide' : 'Show'} filter"
                  title="${binding.visible ? 'Visible' : 'Hidden'}"></button>
          <button class="phz-fc-card-remove"
                  @click=${(e: Event) => { e.stopPropagation(); this._removeBinding(binding); }}
  ...
  private _renderExpandedForm(binding: FilterBinding, def?: FilterDefinition) {
    return html`
      <div class="phz-fc-card-expanded">
        ...
        <div class="phz-fc-form-row">
          ...
          ${(this.availableColumns ?? []).length > 0 ? html`
            <phz-combobox
              ...
              allow-empty
              @combobox-change=${(e: CustomEvent) => this._updateBinding(binding, {
                dataFieldOverride: (e.detail as { value: string }).value || undefined,
              })}
            ></phz-combobox>
          ` : html`
            <input class="phz-sc-input"
                   .value=${binding.dataFieldOverride || ''}
                   @change=${(e: Event) => this._updateBinding(binding, {
                     dataFieldOverride: (e.target as HTMLInputElement).value || undefined,
                   })}
        ...
        <div class="phz-fc-form-row">
          ...
          <input class="phz-sc-input"
                 ...
                 @change=${(e: Event) => this._updateBinding(binding, {
                   labelOverride: (e.target as HTMLInputElement).value || undefined,
                 })}
        ...
        ${def && HAS_SELECTION_MODE.includes(def.type) ? html`
          <div class="phz-fc-form-row">
            ...
            <div class="phz-fc-segmented">
              ${(['single', 'multiple', 'none'] as CriteriaSelectionMode[]).map(m => html`
  ...
  private _renderPickerDrawer() {
    const boundIds = new Set((this.bindings ?? []).map(b => b.filterDefinitionId as string));
    const unboundDefs = (this.definitions ?? []).filter(d => !d.deprecated && !boundIds.has(d.id as string));
  ...
  private _renderFallbackPicker(defs: FilterDefinition[]) {
    if (defs.length === 0) {
      ...
      <div class="phz-fc-picker-list">
        ${defs.map(def => html`
          <label class="phz-fc-picker-item">
            <input type="checkbox"
                   .checked=${this._pickerSelected.has(def.id as string)}
                   @change=${(e: Event) => {
                     const checked = (e.target as HTMLInputElement).checked;
                     const newSet = new Set(this._pickerSelected);
                     if (checked) newSet.add(def.id as string);
                     else newSet.delete(def.id as string);
  ...
  private _removeBinding(binding: FilterBinding) {
  ...
  private _toggleVisibility(binding: FilterBinding) {
    this._updateBinding(binding, { visible: !binding.visible });
  }

  private _updateBinding(binding: FilterBinding, patch: Partial<FilterBinding>) {
  ...
  private _updateBarConfig(binding: FilterBinding, patch: Partial<FilterBarFieldConfig>) {
