...
let PhzTreeSelect = class PhzTreeSelect extends LitElement {
    constructor() {
        super(...arguments);
        this.nodes = [];
        this.value = [];
  ...
  `]; }
    ...
    _getCheckState(node) {
        ...
        if (checkedCount === 0)
            return 'none';
        if (checkedCount === leafValues.length)
            return 'all';
        return 'some';
    }
    _toggleNode(node) {
    ...
    _toggleExpand(nodeValue, e) {
        e.stopPropagation();
        const expanded = new Set(this._expanded);
        if (expanded.has(nodeValue))
            expanded.delete(nodeValue);
        else
            expanded.add(nodeValue);
    ...
    _matchesSearch(node) {
        ...
        if (node.label.toLowerCase().includes(q))
            return true;
        if (node.children)
            return node.children.some(c => this._matchesSearch(c));
        return false;
    }
    ...
    _renderNode(node, isChild = false) {
        if (!this._matchesSearch(node))
            return nothing;
      ...
      <div
        ...
        @click=${(e) => { e.stopPropagation(); this._toggleNode(node); }}
      >
        ${showExpand ? html `
          <span
            class="phz-sc-ts-expand ${hasChildren ? (expanded ? 'phz-sc-ts-expand--open' : '') : 'phz-sc-ts-expand--hidden'}"
            @click=${hasChildren ? (e) => this._toggleExpand(node.value, e) : nothing}
    ...
    _renderTreeContent() {
      ...
      ${!this._isSingleMode ? html `
        <div class="phz-sc-ts-actions">
          <button class="phz-sc-ts-action-btn" @click=${(e) => { e.stopPropagation(); this._selectAll(); }}>Select All</button>
          <span class="phz-sc-ts-action-sep"></span>
          <button class="phz-sc-ts-action-btn" @click=${(e) => { e.stopPropagation(); this._clearAll(); }}>Clear All</button>
        </div>
      ` : html `
        <div class="phz-sc-ts-actions">
          <button class="phz-sc-ts-action-btn" @click=${(e) => { e.stopPropagation(); this._clearAll(); }}>Clear</button>
        </div>
      `}
      <div class="phz-sc-ts-tree-wrap">
        ${hasMatches
            ? html `
              ...
              ${shouldTruncate ? html `
                <button class="phz-sc-ts-show-all" @click=${(e) => { e.stopPropagation(); this._showAllItems = true; }}>
        ...
        ${this.showExpandButton ? html `
          <button class="phz-sc-ts-expand-btn" @click=${(e) => { e.stopPropagation(); this._onExpandRequest(); }}>
            Expand to full view
          </button>
        ` : nothing}
      </div>
...
__decorate([
    property({ type: Array })
], PhzTreeSelect.prototype, "nodes", void 0);
__decorate([
    property({ type: Array })
], PhzTreeSelect.prototype, "value", void 0);
__decorate([
    property({ type: Boolean })
], PhzTreeSelect.prototype, "disabled", void 0);
__decorate([
    property({ type: Number })
], PhzTreeSelect.prototype, "maxVisibleItems", void 0);
__decorate([
    property({ type: Array })
], PhzTreeSelect.prototype, "inactiveItems", void 0);
__decorate([
    property({ type: Boolean })
], PhzTreeSelect.prototype, "showExpandButton", void 0);
__decorate([
    property({ type: Boolean })
], PhzTreeSelect.prototype, "inline", void 0);
__decorate([
    property({ type: String })
], PhzTreeSelect.prototype, "selectionMode", void 0);
__decorate([
    state()
], PhzTreeSelect.prototype, "_open", void 0);
__decorate([
    state()
], PhzTreeSelect.prototype, "_search", void 0);
__decorate([
    state()
], PhzTreeSelect.prototype, "_expanded", void 0);
__decorate([
    state()
], PhzTreeSelect.prototype, "_showAllItems", void 0);
PhzTreeSelect = __decorate([
    customElement('phz-tree-select')
], PhzTreeSelect);
export { PhzTreeSelect };
//# sourceMappingURL=phz-tree-select.js.map