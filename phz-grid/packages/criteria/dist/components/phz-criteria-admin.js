...
let PhzCriteriaAdmin = class PhzCriteriaAdmin extends LitElement {
    constructor() {
        super(...arguments);
        this.criteriaConfig = { fields: [] };
        ...
        this._expandedFieldIdx = -1;
        this._showColumnPicker = false;
        this._showDefinitionPicker = false;
  ...
  `]; }
    connectedCallback() {
        ...
        if (!this._deprecationWarned) {
            console.warn('[phz-criteria-admin] Deprecated: use <phz-filter-designer> for definition management ' +
    ...
    get _resolvedFields() {
        ...
        if (this._isRegistryBacked && this.filterBindings.length > 0) {
            const defMap = new Map(this.filterDefinitions.map(d => [d.id, d]));
            return this.filterBindings
                .filter(b => !this.artefactId || b.artefactId === this._artId)
                .sort((a, b) => a.order - b.order)
                .map(binding => {
                const def = defMap.get(binding.filterDefinitionId);
                ...
                return { field, definition: def, binding };
            })
                .filter((r) => r !== null);
        }
    ...
    _addField() {
        ...
        const newField = { id, label: 'New Field', type: 'single_select' };
    ...
    _removeField(idx) {
        ...
        fields.splice(idx, 1);
        this.criteriaConfig = { ...this.criteriaConfig, fields };
        if (this._expandedFieldIdx === idx)
            this._expandedFieldIdx = -1;
        else if (this._expandedFieldIdx > idx)
            this._expandedFieldIdx--;
        this._emit();
    }
    _updateField(idx, updates) {
        const fields = [...this.criteriaConfig.fields];
        fields[idx] = { ...fields[idx], ...updates };
        this.criteriaConfig = { ...this.criteriaConfig, fields };
        this._emit();
    }
    _updateDateConfig(idx, updates) {
    ...
    _getOptionsMode(field) {
        if (field.optionsSource)
            return 'external';
        if (!field.options?.length && field.dataField)
            return 'data_column';
        return 'manual';
    }
    _updateOptionsSource(idx, updates) {
        const field = this.criteriaConfig.fields[idx];
        const current = field.optionsSource ?? { dataSetId: '', valueField: '' };
        this._updateField(idx, { optionsSource: { ...current, ...updates } });
    }
    _defaultSelectionMode(type) {
        return type === 'single_select' ? 'single' : 'multiple';
    }
    _moveField(idx, direction) {
        const newIdx = idx + direction;
        if (newIdx < 0 || newIdx >= this.criteriaConfig.fields.length)
            return;
        const fields = [...this.criteriaConfig.fields];
        [fields[idx], fields[newIdx]] = [fields[newIdx], fields[idx]];
        this.criteriaConfig = { ...this.criteriaConfig, fields };
        if (this._expandedFieldIdx === idx)
            this._expandedFieldIdx = newIdx;
        else if (this._expandedFieldIdx === newIdx)
            this._expandedFieldIdx = idx;
        this._emit();
    }
    _toggleFieldExpand(idx) {
    ...
    _moveBindingField(idx, direction) {
        const resolved = this._resolvedFields;
        const newIdx = idx + direction;
        if (newIdx < 0 || newIdx >= resolved.length)
            return;
        const orderedIds = resolved.map(r => {
            if (r.binding)
                return r.binding.filterDefinitionId;
    ...
    _addDependency() {
        ...
        if (fields.length < 2)
            return;
        const dep = { parentFieldId: fields[0].id, childFieldId: fields[1].id };
        this.criteriaConfig = {
            ...this.criteriaConfig,
    ...
    _renderFieldCard(field, idx) {
      ...
      <div class="phz-sc-field-card ${expanded ? 'phz-sc-field-card--expanded' : ''}">
        <button class="phz-sc-field-card-header" @click=${() => this._toggleFieldExpand(idx)}>
          <span class="phz-sc-field-card-drag" @click=${(e) => e.stopPropagation()}>${ICONS.drag}</span>
          ...
          <button class="phz-sc-field-card-remove" @click=${(e) => { e.stopPropagation(); this._removeField(idx); }} title="Remove field">${ICONS.remove}</button>
          ...
          <div class="phz-sc-field-card-body">
            <div class="phz-sc-field-form-row">
              ...
              ${field.fieldRole === 'parameter' ? html `
                <input class="phz-sc-input" placeholder="Column name (optional)"
                  ...
                  @change=${(e) => this._updateField(idx, { dataField: e.target.value || undefined })} />
              ` : html `
            ...
            <div class="phz-sc-field-form-row">
              ...
              <input class="phz-sc-input" .value=${field.label}
                @change=${(e) => this._updateField(idx, { label: e.target.value })} />
            </div>

            <div class="phz-sc-field-form-row">
              <label class="phz-sc-field-form-label">Type</label>
              <select class="phz-sc-select"
                @change=${(e) => this._updateField(idx, { type: e.target.value })}>
            ...
            <div class="phz-sc-field-form-row">
              ...
              <input class="phz-sc-input" placeholder="None"
                ...
                @change=${(e) => this._updateField(idx, { defaultValue: e.target.value || null })} />
            </div>
              ...
              <div class="phz-sc-inline-toggle">
                <label class="phz-sc-toggle-switch">
                  <input type="checkbox" ?checked=${!!field.required}
                    @change=${(e) => this._updateField(idx, { required: e.target.checked })} />
      ...
      <div style="border-top:1px solid #F5F5F4; padding-top:8px; margin-top:4px">
        ...
        <div class="phz-sc-field-form-row">
          ...
          <select class="phz-sc-select"
            ...
            this._updateDateConfig(idx, { defaultPresetId: val ? val : undefined });
        }}>
          ...
          <div class="phz-sc-inline-toggle">
            <label class="phz-sc-toggle-switch">
              <input type="checkbox" ?checked=${!!dc.comparisonEnabled}
                @change=${(e) => this._updateDateConfig(idx, { comparisonEnabled: e.target.checked })} />
              <span class="phz-sc-toggle-track"></span>
            </label>
        ...
        <div style="display:flex; gap:8px; margin-top:6px">
          <div class="phz-sc-field-form-row" style="flex:1">
            ...
            <select class="phz-sc-select"
              @change=${(e) => this._updateDateConfig(idx, { weekStartDay: e.target.value })}>
          ...
          <div class="phz-sc-field-form-row" style="flex:1">
            ...
            <select class="phz-sc-select"
              @change=${(e) => this._updateDateConfig(idx, { weekNumbering: e.target.value })}>
    ...
    _renderPresenceConfig(field, idx) {
          ...
          <div class="phz-sc-inline-toggle">
            <label class="phz-sc-toggle-switch">
              <input type="checkbox" ?checked=${!!pc.compact}
                @change=${(e) => this._updatePresenceConfig(idx, { compact: e.target.checked })} />
              <span class="phz-sc-toggle-track"></span>
            </label>
    ...
    _renderOptionsSourceConfig(field, idx) {
      ...
      <div style="border-top:1px solid #F5F5F4; padding-top:8px; margin-top:4px">
        ...
        ${mode === 'external' && src ? html `
          ...
          <div class="phz-sc-field-form-row">
            ...
            <select class="phz-sc-select"
              @change=${(e) => this._updateOptionsSource(idx, { valueField: e.target.value })}>
              ...
              <div style="max-height:200px; overflow-y:auto">
                ${unused.map(col => html `
                  <button style="display:flex; align-items:center; gap:8px; width:100%; padding:7px 10px; border:none; background:none; cursor:pointer; font-family:inherit; font-size:12px; color:#44403C; text-align:left; transition:background 0.1s"
                    @mouseover=${(e) => { e.currentTarget.style.background = '#F5F5F4'; }}
    ...
    _renderDefinitionPicker() {
      ...
      <div style="border:1px solid #E7E5E4; border-radius:10px; overflow:hidden; margin-top:4px">
        ...
        <button style="display:flex; align-items:center; gap:6px; width:100%; padding:8px 10px; border:none; border-top:1px solid #E7E5E4; background:#FAFAF9; cursor:pointer; font-family:inherit; font-size:12px; color:#78716C; text-align:left; transition:background 0.1s"
          @mouseover=${(e) => { e.currentTarget.style.background = '#F5F5F4'; }}
          @mouseout=${(e) => { e.currentTarget.style.background = '#FAFAF9'; }}
    ...
    _renderBindingFieldCard(resolved, idx) {
      ...
      <div class="phz-sc-field-card ${expanded ? 'phz-sc-field-card--expanded' : ''}">
        <button class="phz-sc-field-card-header" @click=${() => this._toggleFieldExpand(idx)}>
          ...
          <button class="phz-sc-field-card-remove" @click=${(e) => { e.stopPropagation(); if (binding)
            this._emitBindingRemove(binding.filterDefinitionId); }} title="Unbind filter">${ICONS.remove}</button>
          <span class="phz-sc-field-card-chevron">${expanded ? ICONS.chevronDown : ICONS.chevronRight}</span>
        </button>
              ...
              <div class="phz-sc-inline-toggle">
                <label class="phz-sc-toggle-switch">
                  <input type="checkbox" ?checked=${binding.requiredOverride ?? definition.required ?? false}
                    @change=${(e) => {
            this._emitBindingUpdate(binding.filterDefinitionId, { requiredOverride: e.target.checked });
        }} />
                  <span class="phz-sc-toggle-track"></span>
                </label>
                <span class="phz-sc-inline-toggle-label">Required</span>
              ...
              <div class="phz-sc-inline-toggle">
                <label class="phz-sc-toggle-switch">
                  <input type="checkbox" ?checked=${binding.visible}
                    @change=${(e) => {
            this._emitBindingUpdate(binding.filterDefinitionId, { visible: e.target.checked });
        }} />
                  <span class="phz-sc-toggle-track"></span>
                </label>
          ...
          <div class="phz-sc-dep-card">
            <div class="phz-sc-dep-card-row">
              ...
              <select class="phz-sc-select" style="flex:1"
                @change=${(e) => this._updateDependency(idx, 'parentFieldId', e.target.value)}>
            ...
            <div class="phz-sc-dep-card-row">
              ...
              <select class="phz-sc-select" style="flex:1"
                @change=${(e) => this._updateDependency(idx, 'childFieldId', e.target.value)}>
    ...
    _renderLayoutTab() {
        ...
        const topMode = ly.barMode ?? 'button';
        ...
        <div class="phz-sc-layout-section">
          <div class="phz-sc-layout-row">
            ...
            <input class="phz-sc-input" .value=${btnLabel}
              @change=${(e) => this._updateLayout({ buttonLabel: e.target.value || 'Filters' })} />
          </div>
        </div>
          ...
          <div class="phz-sc-layout-row">
            ...
            <div class="phz-sc-color-row">
              <input type="color" class="phz-sc-color-input" .value=${btnBg}
                @input=${(e) => this._updateLayout({ buttonBgColor: e.target.value })} />
              <input class="phz-sc-input phz-sc-color-hex" .value=${btnBg}
                @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateLayout({ buttonBgColor: v });
        }} />
            </div>
          </div>
          <div class="phz-sc-layout-row">
            ...
            <div class="phz-sc-color-row">
              <input type="color" class="phz-sc-color-input" .value=${btnText}
                @input=${(e) => this._updateLayout({ buttonTextColor: e.target.value })} />
              <input class="phz-sc-input phz-sc-color-hex" .value=${btnText}
                @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateLayout({ buttonTextColor: v });
        }} />
            </div>
          </div>
        </div>
          ...
          <div class="phz-sc-admin-toggle" style="padding-top:0">
            <label class="phz-sc-toggle-switch">
              <input type="checkbox" ?checked=${buttonOnly}
                @change=${(e) => this._updateLayout({ buttonOnly: e.target.checked })} />
              <span class="phz-sc-toggle-track"></span>
            </label>
            ...
            <div class="phz-sc-layout-row">
              ...
              <div class="phz-sc-color-row">
                <input type="color" class="phz-sc-color-input" .value=${containerBg}
                  @input=${(e) => this._updateLayout({ containerBgColor: e.target.value })} />
                <input class="phz-sc-input phz-sc-color-hex" .value=${containerBg}
                  @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateLayout({ containerBgColor: v });
        }} />
              </div>
            </div>
            <div class="phz-sc-layout-row">
              ...
              <div class="phz-sc-color-row">
                <input type="color" class="phz-sc-color-input" .value=${containerBorder || '#E7E5E4'}
                  @input=${(e) => this._updateLayout({ containerBorderColor: e.target.value })} />
                <input class="phz-sc-input phz-sc-color-hex" .value=${containerBorder}
                  @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v) || v === '')
                this._updateLayout({ containerBorderColor: v });
        }} />
              </div>
            </div>
            <div class="phz-sc-layout-row">
              ...
              <div style="display:flex; align-items:center; gap:8px">
                <input type="range" min="0" max="24" step="1" .value=${String(containerRadius)}
                  ...
                  @input=${(e) => this._updateLayout({ containerBorderRadius: Number(e.target.value) })} />
        ...
        <div class="phz-sc-layout-section">
          <div class="phz-sc-layout-row">
            ...
            <input class="phz-sc-input" .value=${summaryPlaceholder}
              @change=${(e) => this._updateLayout({ summaryPlaceholder: e.target.value || 'No filters applied' })} />
          </div>
          <div style="font-size:10px; color:#A8A29E; margin-top:4px; line-height:1.4">
        ...
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${defBg}
              @input=${(e) => this._updateSummaryLayout({ bgColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${defBg}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ bgColor: v });
        }} />
          </div>
        </div>
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${defText}
              @input=${(e) => this._updateSummaryLayout({ textColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${defText}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ textColor: v });
        }} />
          </div>
        </div>
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${defBorder}
              @input=${(e) => this._updateSummaryLayout({ borderColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${defBorder}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ borderColor: v });
        }} />
          </div>
        </div>
        ...
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${actBg}
              @input=${(e) => this._updateSummaryLayout({ activeBgColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${actBg}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ activeBgColor: v });
        }} />
          </div>
        </div>
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${actText}
              @input=${(e) => this._updateSummaryLayout({ activeTextColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${actText}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ activeTextColor: v });
        }} />
          </div>
        </div>
        <div class="phz-sc-layout-row">
          ...
          <div class="phz-sc-color-row">
            <input type="color" class="phz-sc-color-input" .value=${actBorder}
              @input=${(e) => this._updateSummaryLayout({ activeBorderColor: e.target.value })} />
            <input class="phz-sc-input phz-sc-color-hex" .value=${actBorder}
              @change=${(e) => {
            const v = e.target.value;
            if (/^#[0-9A-Fa-f]{6}$/.test(v))
                this._updateSummaryLayout({ activeBorderColor: v });
        }} />
          </div>
        </div>
        ...
        <div class="phz-sc-layout-row" style="margin-top:4px">
          ...
          <div style="display:flex; align-items:center; gap:8px">
            <input type="range" min="0" max="24" step="1" .value=${String(radius)}
              style="flex:1; accent-color:#1C1917"
              @input=${(e) => this._updateSummaryLayout({ borderRadius: Number(e.target.value) })} />
    ...
    _renderSettingsTab() {
      ...
      <div class="phz-sc-admin-toggle">
        <label class="phz-sc-toggle-switch">
          <input type="checkbox" ?checked=${b.autoApply}
            @change=${(e) => this._updateBehavior({ autoApply: e.target.checked })} />
          <span class="phz-sc-toggle-track"></span>
        </label>
        <div class="phz-sc-toggle-text">
      ...
      <div class="phz-sc-admin-toggle">
        <label class="phz-sc-toggle-switch">
          <input type="checkbox" ?checked=${b.showPresetManager !== false}
            @change=${(e) => this._updateBehavior({ showPresetManager: e.target.checked })} />
          <span class="phz-sc-toggle-track"></span>
        </label>
        <div class="phz-sc-toggle-text">
      ...
      <div class="phz-sc-admin-toggle">
        <label class="phz-sc-toggle-switch">
          <input type="checkbox" ?checked=${b.showResetButton !== false}
            @change=${(e) => this._updateBehavior({ showResetButton: e.target.checked })} />
          <span class="phz-sc-toggle-track"></span>
        </label>
        <div class="phz-sc-toggle-text">
      ...
      <div class="phz-sc-style-group">
        ...
        <select class="phz-sc-select" style="width:100%"
          @change=${(e) => this._updateBehavior({ panelStyle: e.target.value })}>
      ...
      ${this._studioOpen ? html `
        ...
        >
          <phz-filter-studio
            ...
            @filter-studio-save=${(e) => {
            this._studioOpen = false;
            const def = e.detail?.definition;
            if (def)
                this._emitDefinitionCreate(def);
...
], PhzCriteriaAdmin.prototype, "criteriaConfig", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "availableFields", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "columns", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "data", void 0);
__decorate([
    property({ type: Object })
], PhzCriteriaAdmin.prototype, "dataSources", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "filterDefinitions", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "filterBindings", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "filterRules", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "sharedPresets", void 0);
__decorate([
    property({ type: Array })
], PhzCriteriaAdmin.prototype, "userPresets", void 0);
__decorate([
    property({ type: String })
], PhzCriteriaAdmin.prototype, "artefactId", void 0);
__decorate([
    property({ type: Boolean })
], PhzCriteriaAdmin.prototype, "resizable", void 0);
__decorate([
    property({ type: Number })
], PhzCriteriaAdmin.prototype, "minWidth", void 0);
__decorate([
    property({ type: Number })
], PhzCriteriaAdmin.prototype, "maxWidth", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_activeTab", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_expandedFieldIdx", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_showColumnPicker", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_showDefinitionPicker", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_resizing", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_studioOpen", void 0);
__decorate([
    state()
], PhzCriteriaAdmin.prototype, "_editingDef", void 0);
PhzCriteriaAdmin = __decorate([
    customElement('phz-criteria-admin')
], PhzCriteriaAdmin);
export { PhzCriteriaAdmin };
//# sourceMappingURL=phz-criteria-admin.js.map