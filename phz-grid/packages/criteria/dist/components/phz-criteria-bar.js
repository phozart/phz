...
let PhzCriteriaBar = class PhzCriteriaBar extends LitElement {
    constructor() {
        super(...arguments);
        this.config = { fields: [] };
  ...
  `]; }
    ...
    _getSL() {
        const sl = this._ly.summaryStrip;
        return {
      ...
      <div class="phz-sc-summary-bar" style=${barStyle} @click=${this._openDrawer} role="button" tabindex="0"
        ...
        ${hasActive ? html `
          <button class="phz-sc-bar-clear" style="flex-shrink:0; margin-left:auto"
            @click=${(e) => { e.stopPropagation(); this._clearAll(); }}>Clear all</button>
        ` : nothing}
      </div>
    `;
    }
    _renderFilterButton(activeCount) {
        const ly = this._ly;
...
], PhzCriteriaBar.prototype, "config", void 0);
__decorate([
    property({ type: Object })
], PhzCriteriaBar.prototype, "selectionContext", void 0);
__decorate([
    property({ type: Object })
], PhzCriteriaBar.prototype, "layout", void 0);
PhzCriteriaBar = __decorate([
    customElement('phz-criteria-bar')
], PhzCriteriaBar);
export { PhzCriteriaBar };
//# sourceMappingURL=phz-criteria-bar.js.map