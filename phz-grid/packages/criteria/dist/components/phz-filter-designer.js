...
const FIELD_TYPE_OPTIONS = FIELD_TYPES.map(t => ({ value: t.value, label: t.label }));
...
export function buildDefContextItems(def) {
    if (def.deprecated) {
        return [
...
export function buildRuleContextItems(rule, isFirst, isLast) {
    return [
...
export function buildPresetContextItems(preset, scope) {
    if (scope === 'shared') {
        return [
...
export function buildBgContextItems(tab, helpOpen) {
...
let PhzFilterDesigner = class PhzFilterDesigner extends LitElement {
    constructor() {
        super(...arguments);
        // -- Public properties --
        this.definitions = [];
        ...
        this.data = [];
        this.rulePreviewResults = {};
        this.dataSources = [];
        this.filterPresets = [];
        // -- Internal state --
        this._tab = 'definitions';
        this._searchTerm = '';
        this._drawerOpen = false;
        this._drawerMode = 'inline';
        this._editingDef = null;
        this._hasStudio = false;
        this._studioChecked = false;
  ...
  `]; }
      ...
      <div class="phz-fd-list">
        ${filtered.length === 0
            ? html `<div class="phz-fd-empty">${this._searchTerm
                ...
                : 'No filter definitions yet. Click \u201C+ New Definition\u201D above or right-click for options.'}</div>`
            : nothing}
    ...
    _renderDefinitionCard(def) {
      ...
      <div class="phz-fd-card ${def.deprecated ? 'phz-fd-card--deprecated' : ''}"
           @contextmenu=${(e) => this._handleDefContextMenu(e, def)}>
      ...
      <div class="phz-fd-form">
        ...
        <div class="phz-fd-form-row">
          ...
          <phz-combobox
            ...
            .value=${this._newType}
            .allowEmpty=${false}
            @combobox-change=${(e) => this._newType = e.detail.value}
          ></phz-combobox>
        ...
        <div class="phz-fd-form-row">
          ...
          <phz-combobox
            ...
            .allowEmpty=${false}
            @combobox-change=${(e) => this._newSession = e.detail.value}
          ></phz-combobox>
        ...
        ${(this.dataSources ?? []).length > 0 ? html `
          <div class="phz-fd-form-row">
            ...
            <phz-combobox
              ...
              allow-empty
              empty-label="\u2014 None \u2014"
              @combobox-change=${(e) => this._handleNewDataSourceChange(e.detail.value)}
            ></phz-combobox>
          ...
          ${this._newDataSourceId ? html `
            <div class="phz-fd-form-row">
              ...
              <phz-combobox
                ...
                allow-empty
                empty-label="\u2014 Select \u2014"
                @combobox-change=${(e) => this._newValueField = e.detail.value}
              ></phz-combobox>
            ...
            <div class="phz-fd-form-row">
              ...
              <input class="phz-sc-input" .value=${this._newLabelTemplate}
                     @input=${(e) => this._newLabelTemplate = e.target.value}
      ...
      <div style="display:flex;flex-direction:column;gap:12px;padding:0 16px">
        <div class="phz-fd-form-row">
          ...
          <input class="phz-sc-input" .value=${def.label}
                 @input=${(e) => this._handleEditorChange('label', e.target.value)}>
          <span class="phz-fd-form-hint">The name users will see for this filter</span>
        </div>
        <div class="phz-fd-form-row">
          <label class="phz-sc-field-label">Type</label>
          <phz-combobox
            ...
            .value=${def.type}
            .allowEmpty=${false}
            @combobox-change=${(e) => this._handleEditorChange('type', e.detail.value)}
          ></phz-combobox>
        ...
        <div class="phz-fd-form-row">
          ...
          <phz-combobox
            ...
            .allowEmpty=${false}
            @combobox-change=${(e) => this._handleEditorChange('sessionBehavior', e.detail.value)}
          ></phz-combobox>
          <span class="phz-fd-form-hint">Should this filter remember the user's last selection?</span>
        </div>
        ${(this.dataSources ?? []).length > 0 ? html `
    ...
    _renderContextMenu() {
      ...
      <div class="phz-fd-ctx-backdrop" @click=${this._closeContextMenu}
           @contextmenu=${(e) => { e.preventDefault(); this._closeContextMenu(); }}></div>
      <div class="phz-fd-ctx" role="menu" aria-label="Context menu"
        ...
        ${this._ctxItems.map((item, idx) => {
            if (item.separator)
                return html `<div class="phz-fd-ctx-sep" role="separator"></div>`;
    ...
    _handleCtxKeydown(e) {
        ...
        switch (e.key) {
            ...
            case ' ':
                e.preventDefault();
                if (this._ctxFocusIdx >= 0)
                    this._selectCtxItem(this._ctxItems[this._ctxFocusIdx]);
                break;
        }
    }
    // -- Context Menu: Open / Close --
    _openContextMenu(x, y, items, targetId, targetData) {
        this._ctxX = x;
        this._ctxY = y;
    ...
    _handleRootContextMenu(e) {
        ...
        if (isCard || isForm || isDrawer)
            return;
        e.preventDefault();
        this._openContextMenu(e.clientX, e.clientY, buildBgContextItems('definitions', this._helpOpen));
    }
    _handleDefContextMenu(e, def) {
        e.preventDefault();
        e.stopPropagation();
        this._openContextMenu(e.clientX, e.clientY, buildDefContextItems(def), def.id, def);
    }
    _handleRuleContextMenu(e) {
        const { ruleId, rule, x, y } = e.detail;
        const sorted = [...this.rules].sort((a, b) => a.priority - b.priority);
        const idx = sorted.findIndex(r => r.id === ruleId);
        this._openContextMenu(x, y, buildRuleContextItems(rule, idx === 0, idx === sorted.length - 1), ruleId, rule);
    }
    _handleRulesBgContextMenu(e) {
        const { x, y } = e.detail;
        this._openContextMenu(x, y, buildBgContextItems('rules', this._helpOpen));
    }
    _handlePresetContextMenu(e) {
        const { presetId, preset, scope, x, y } = e.detail;
        this._openContextMenu(x, y, buildPresetContextItems(preset, scope), presetId, preset);
    }
    _handlePresetsBgContextMenu(e) {
        const { scope, x, y } = e.detail;
        this._openContextMenu(x, y, buildBgContextItems('presets', this._helpOpen), undefined, { scope });
    }
    _handleFilterPresetContextMenu(e) {
        const { presetId, preset, x, y } = e.detail;
        const items = buildFilterPresetContextItems(preset);
        this._openContextMenu(x, y, items, presetId, preset);
    }
    _redispatch(e) {
        this._dispatchEvent(e.type, e.detail);
    }
        ...
        switch (item.id) {
            ...
            case 'edit-rule':
                if (this._ctxTargetId && this._ctxTargetData) {
                    this._openRuleEditor(this._ctxTargetData, 'edit');
                }
                break;
            case 'copy-rule':
                if (this._ctxTargetId && this._ctxTargetData) {
                    this._openRuleEditor(this._ctxTargetData, 'copy');
                }
                break;
            case 'toggle-rule':
                if (this._ctxTargetId && this._ctxTargetData) {
                    const enabled = this._ctxTargetData.enabled;
            ...
            // -- Presets --
            case 'new-preset': {
                const scope = this._ctxTargetData?.scope ?? 'shared';
                this._dispatchEvent('preset-create', { scope });
                break;
            }
            case 'edit-preset':
                if (this._ctxTargetId)
                    this._dispatchEvent('preset-update', { presetId: this._ctxTargetId });
                break;
            case 'set-default-preset':
                if (this._ctxTargetId && this._ctxTargetData) {
                    const isDefault = this._ctxTargetData.isDefault;
            ...
            case 'edit-filter-preset':
                if (this._ctxTargetId && this._ctxTargetData) {
                    const presetAdmin = this.shadowRoot?.querySelector('phz-preset-admin');
                    if (presetAdmin) {
                        const fp = this._ctxTargetData;
    ...
    _openRuleEditor(rule, mode) {
        const ruleAdmin = this.shadowRoot?.querySelector('phz-rule-admin');
        if (ruleAdmin) {
            if (rule) {
                ruleAdmin.openEditor(rule, mode);
            }
            else {
                ruleAdmin.openEditor(null, 'add');
            }
        }
    }
    // -- Toast --
    ...
    // -- Data Source helpers --
    _handleNewDataSourceChange(dsId) {
        ...
        if (dsId && !this._newLabel) {
            ...
            if (ds)
                this._newLabel = ds.name;
        }
    }
    _renderNewTemplatePreview() {
        if (!this._newLabelTemplate || !this._newDataSourceId)
            return nothing;
    ...
    _closeDrawer() {
        ...
        if (this._drawerMode === 'inline' && this._autoSaveTimer)
            this._flushAutoSave();
        this._drawerOpen = false;
        this._editingDef = null;
        this._saveStatus = 'idle';
...
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "definitions", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "rules", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "sharedPresets", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "userPresets", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "availableColumns", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "data", void 0);
__decorate([
    property({ type: Object })
], PhzFilterDesigner.prototype, "rulePreviewResults", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "dataSources", void 0);
__decorate([
    property({ type: Array })
], PhzFilterDesigner.prototype, "filterPresets", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_tab", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_searchTerm", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_drawerOpen", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_drawerMode", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_editingDef", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_hasStudio", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_studioChecked", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newLabel", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newType", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newSession", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_showNewForm", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newDataSourceId", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newValueField", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_newLabelTemplate", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_editingId", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_editingLabel", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxOpen", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxX", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxY", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxItems", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxFocusIdx", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxTargetId", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_ctxTargetData", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_helpOpen", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_saveStatus", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_toastVisible", void 0);
__decorate([
    state()
], PhzFilterDesigner.prototype, "_toastMessage", void 0);
PhzFilterDesigner = __decorate([
    customElement('phz-filter-designer')
], PhzFilterDesigner);
export { PhzFilterDesigner };
