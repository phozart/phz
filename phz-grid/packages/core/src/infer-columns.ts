 ...
 */
export function formatFieldAsHeader(field: string): string {
  if (!field) return '';
 ...
 */
export function inferColumns(data: unknown[], options?: InferColumnsOptions): ColumnDefinition[] {
  ...
  const sample = data.slice(0, sampleSize) as Record<string, unknown>[];
