...
import type { QueryBackend, LocalQuery, LocalQueryResult, QueryBackendCapabilities } from './types/query-backend.js';
import type { FilterOperator } from './types/state.js';
import { parseData, buildCoreRowModel, filterRows, sortRows } from './row-model.js';

export function createJSArrayQueryBackend(
  data: unknown[],
  ...
  return {
    async execute(query: LocalQuery): Promise<LocalQueryResult> {
      ...
      if (query.fields && query.fields.length > 0) {
        ...
        outputRows = resultRows.map(row => {
          const { __id, ...rest } = row as Record<string, unknown>;
          return rest;
        });
      }
