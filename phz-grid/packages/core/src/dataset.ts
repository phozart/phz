...
import type { DataSet, DataSetColumn, DataSetColumnType, DataSetMeta } from './types/dataset.js';
 ...
 */
function normalizeType(dsType: DataSetColumnType): ColumnType {
  switch (dsType) {
 ...
 */
export function toColumnDefinitions(columns: DataSetColumn[]): ColumnDefinition[] {
  return columns
 ...
 */
export function createDataSet<T = Record<string, unknown>>(
 ...
 * - `string` otherwise
 */
export function inferDataSetColumns(rows: Record<string, unknown>[]): DataSetColumn[] {
  ...
  return fields.map(field => {
    ...
    let type: DataSetColumnType = 'string';
    if (nonNullCount > 0) {
      if (boolCount === nonNullCount) type = 'boolean';
      else if (numCount === nonNullCount) type = 'number';
      else if (dateCount === nonNullCount) type = 'date';
    }
