...
// --- Filter evaluation ---
function matchesFilter(row, rule) {
    ...
    switch (rule.operator) {
        ...
        case 'between': {
            if (typeof val !== 'number' || typeof cmp !== 'number' || typeof rule.value2 !== 'number')
                return false;
...
// --- Grouping ---
function groupByCategory(rows, categoryField) {
...
// --- Aggregation ---
function aggregateGroup(rows, measures) {
    const result = {};
...
// --- Main pipeline ---
export function processWidgetData(rows, dataConfig) {
    ...
    if (bindings.type === 'chart') {
        ...
        let processedRows = Array.from(groups.entries()).map(([label, groupRows]) => ({
            label,
    ...
    return {
        rows: filtered.map((row, i) => ({
            label: String(i),
            values: Object.fromEntries(Object.entries(row).filter(([, v]) => typeof v === 'number').map(([k, v]) => [k, v])),
        })),
    };
}
//# sourceMappingURL=widget-data-processor.js.map