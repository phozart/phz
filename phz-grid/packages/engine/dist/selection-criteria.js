...
function resolveAnchor(anchor, now) {
    switch (anchor) {
        case 'start_of_month':
...
function subtractUnits(d, unit, count) {
...
export function resolveDynamicPreset(preset, now) {
...
export const BUILTIN_DATE_PRESETS = [
...
export const DATE_PRESET_GROUP_LABELS = {
    relative: 'Relative',
    rolling: 'Rolling',
    to_date: 'To Date',
...
function getFiscalYearStart(d, fiscalStartMonth) {
...
export function getFiscalQuarter(d, fiscalStartMonth = 1) {
...
export function getFiscalQuarterBounds(year, quarter, fiscalStartMonth = 1) {
    const fsm = fiscalStartMonth - 1;
...
export function getWeekStart(d, weekStartDay = 'monday') {
...
export function getWeekEnd(d, weekStartDay = 'monday') {
...
export function getSequentialWeekNumber(d) {
...
export function getCalendarQuarterBounds(year, quarter) {
...
export function resolveBuiltinPreset(presetId, now, options) {
    const ref = now ?? new Date();
...
export function resolveComparisonPeriod(primary, type) {
...
export function getAvailablePresets(config) {
...
export function formatDateRangeDisplay(value, locale) {
    ...
    const formatDate = (d) => fmtDay.format(d);
    const formatMonth = (d) => fmtMonth.format(d);
    const wrap = (rangeStr) => value.presetLabel ? `${value.presetLabel} (${rangeStr})` : rangeStr;
    // Same day
...
export function resolveDynamicDefaults(config, now) {
    const values = {};
...
export function filterTreeByParent(nodes, parentValue) {
    if (!parentValue)
        return nodes;
...
export function resolveDependencies(config, currentValues) {
...
export function validateCriteria(config, values) {
    const errors = [];
    for (const field of config.fields) {
...
export function formatCriteriaValue(field, value, locale) {
    if (value == null || value === '')
        return '(All)';
    if (field.type === 'date_range' && typeof value === 'string') {
        try {
            ...
            if (range.comparisonStartDate && range.comparisonEndDate) {
                const compRange = { startDate: range.comparisonStartDate, endDate: range.comparisonEndDate };
...
export function buildExportMetadata(config, values) {
    const entries = [];
    for (const field of config.fields) {
        const val = values[field.id];
        entries.push({
...
export function serializeCriteria(values, config) {
...
export function deserializeCriteria(params, config) {
    const values = {};
    for (const field of config.fields) {
...
export function inferCriteriaType(columnType, distinctCount) {
    switch (columnType) {
        case 'date':
            return 'date_range';
        case 'number':
...
export function deriveOptionsFromData(data, dataField) {
    const seen = new Set();
    const options = [];
...
export function resolveOptionsSource(source, dataSources) {
...
export function resolveFieldOptions(field, dataSources, currentData) {
...
export function applyCriteriaToData(data, config, values) {
    let filtered = data;
    for (const field of config.fields) {
        // selectionMode: 'none' — skip filtering entirely
        if (field.selectionMode === 'none')
            continue;
        const val = values[field.id];
        if (val == null || val === '' || (Array.isArray(val) && val.length === 0))
            continue;
        ...
        switch (field.type) {
            ...
            case 'field_presence': {
                if (typeof val === 'string') {
                    try {
                        const presenceMap = JSON.parse(val);
                        ...
                        if (active.length > 0) {
                            filtered = filtered.filter(row => {
                                for (const [f, state] of active) {
                                    ...
                                    if (state === 'has_value' && !hasValue)
                                        return false;
                                    if (state === 'empty' && hasValue)
                                        return false;
                                }
                                return true;
                            });
...
export function applyPresenceFilter(data, filters) {
