...
// --- Factory ---
export function createFilterRuleEngine() {
    const rules = new Map();
    const customEvaluators = new Map();
    return {
        addRule(rule) {
        ...
        getRulesForFilter(filterDefId) {
            return Array.from(rules.values())
                .filter(r => r.filterDefinitionId === filterDefId)
                .sort((a, b) => a.priority - b.priority);
        },
        registerCustomEvaluator(key, evaluator) {
            customEvaluators.set(key, evaluator);
        },
        evaluate(filterDefId, options, treeNodes, context) {
...
export function evaluateRule(rule, options, treeNodes, customEvaluators, context) {
...
function evaluateExcludePattern(config, options) {
...
function evaluateIncludePattern(config, options) {
    const regex = safeRegExp(config.pattern, config.flags);
    if (!regex) {
        return { included: options.map(o => o.value), excluded: [] };
    }
...
function evaluateValueSet(config, options) {
...
function evaluateTreeGroupCompare(config, options, treeNodes) {
    if (!treeNodes)
        return { included: options.map(o => o.value), excluded: [] };
...
function collectMatchingTreeValues(nodes, config, result) {
...
function evaluateCustom(config, options, treeNodes, customEvaluators, context) {
...
function evaluateCrossFilter(config, options, context) {
    if (!context) {
    ...
    switch (cond.operator) {
        ...
        case 'equals':
            if (typeof contextValue === 'string')
                return contextValue === cond.values?.[0];
            return false;
        case 'not_equals':
            if (typeof contextValue === 'string')
                return contextValue !== cond.values?.[0];
        ...
        case 'in':
            if (typeof contextValue === 'string')
                return (cond.values ?? []).includes(contextValue);
            return false;
        case 'not_in':
            if (typeof contextValue === 'string')
                return !(cond.values ?? []).includes(contextValue);
...
function executeAction(action, options, context) {
    let allowedSet;
    switch (action.type) {
        case 'include_values':
...
function applyElseAction(elseAction, options) {
    if (elseAction === 'block') {
...
export function previewRule(rule, options, treeNodes, context) {
