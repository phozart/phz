...
export function computePivot(rows, config) {
    ...
    function computeCellValue(cellRows) {
        if (cellRows.length === 0)
            return null;
        if (multiMeasure) {
            return config.valueFields.map(vf => computeAggregation(cellRows, vf.field, vf.aggregation));
        }
        return computeAggregation(cellRows, config.valueFields[0].field, config.valueFields[0].aggregation);
    }
    // Compute cells
    const cells = rowKeys.map(rowKey => colKeys.map(colKey => {
        const key = `${rowKey}###${colKey}`;
 ...
 */
export function pivotResultToFlatRows(result, rowFieldNames, colFieldNames) {
    const flatRows = [];
