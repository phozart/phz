    ...
    while (i < input.length) {
        ...
        if (ch === '[') {
            ...
            if (i < input.length)
                ...
                errors.push({ message: 'Unterminated field reference', pos: start });
        ...
        if (ch === '"' || ch === "'") {
            ...
            if (i < input.length)
                ...
                errors.push({ message: 'Unterminated string literal', pos: start });
...
class Parser {
    ...
    parseAddition() {
        ...
        while (this.peek().type === 'op' && (this.peek().value === '+' || this.peek().value === '-')) {
            ...
            if (!right)
                return left;
            left = { kind: 'binary_op', operator: opToken.value, left, right, pos: { start: opToken.pos, end: this.tokens[this.pos - 1]?.pos ?? opToken.pos } };
        }
        return left;
    }
    parseMultiplication() {
    ...
    parsePower() {
        ...
        if (this.peek().type === 'op' && this.peek().value === '^') {
            ...
            if (!right)
                return left;
            left = { kind: 'binary_op', operator: '^', left, right, pos: { start: opToken.pos, end: this.tokens[this.pos - 1]?.pos ?? opToken.pos } };
        }
        return left;
    }
    parseUnary() {
    ...
    switch (node.kind) {
        ...
        case 'unary_op':
            if (node.operator === 'negate')
                return `-${wrapIfNeeded(node.operand)}`;
            if (node.operator === 'not')
                return `NOT ${wrapIfNeeded(node.operand)}`;
...
function wrapIfNeeded(node, parent) {
