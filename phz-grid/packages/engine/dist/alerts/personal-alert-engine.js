...
function extractThreshold(alert) {
...
function extractSeverity(alert) {
    ...
    if (alert.severityFilter && alert.severityFilter.length > 0) {
        ...
        for (const s of priority) {
            if (alert.severityFilter.includes(s))
                return s;
        }
    }
    return 'warning';
...
// Grace period check
// ========================================================================
function isWithinGracePeriod(gracePeriod, lastTriggeredAt) {
 ...
 */
export function evaluateAlert(alert, currentValue, lastTriggeredAt) {
 ...
 */
export function evaluateAllAlerts(alerts, values) {
 ...
 */
export function shouldNotify(result, preference) {
    if (!result.triggered)
        return false;
 ...
 */
export function formatAlertMessage(result) {
    if (!result.triggered) {
...
//# sourceMappingURL=personal-alert-engine.js.map