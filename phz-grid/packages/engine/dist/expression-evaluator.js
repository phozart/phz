 ...
 */
// --- Row-level evaluator ---
export function evaluateRowExpression(node, ctx) {
    return evalNode(node, ctx, 'row');
}
// --- Metric-level evaluator ---
export function evaluateMetricExpression(node, ctx) {
...
function evalNode(node, ctx, level) {
    switch (node.kind) {
        case 'literal':
            return node.value;
        case 'field_ref': {
...
// --- Binary Operator ---
function evalBinaryOp(op, leftNode, rightNode, ctx, level) {
...
// --- Built-in Functions ---
function evalFunction(name, argNodes, ctx, level) {
