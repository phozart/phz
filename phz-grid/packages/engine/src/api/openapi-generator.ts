...
import type { ApiSpec, ApiEndpoint, ApiParam, APISpecConfig, HttpMethod } from '@phozart/phz-shared/types';
...
function paramTypeToSchema(type: ApiParam['type']): Record<string, unknown> {
  switch (type) {
    case 'string':
      return { type: 'string' };
    case 'number':
...
function apiParamToOpenAPIParam(
  param: ApiParam,
  ...
  if (param.defaultValue !== undefined) {
    (result.schema as Record<string, unknown>).default = param.defaultValue;
  }
  return result;
}
...
export function endpointToOperation(endpoint: ApiEndpoint): Record<string, unknown> {
  ...
  if (endpoint.requestBody) {
    ...
    if (endpoint.requestBody.description) {
      (operation.requestBody as Record<string, unknown>).description = endpoint.requestBody.description;
    }
  }
