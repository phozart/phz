...
function checkDepth(depth: number): void {
  if (depth > MAX_AST_DEPTH) {
    throw new Error(
      `Expression AST exceeds maximum depth of ${MAX_AST_DEPTH}. ` +
...
function compileNode(node: ExpressionNode, depth = 0): (ctx: RowExpressionContext) => unknown {
  checkDepth(depth);
  switch (node.kind) {
    case 'literal': {
      const v = node.value;
...
function compileBinaryOp(
  ...
  depth = 0,
): (ctx: RowExpressionContext) => unknown {
  ...
  switch (name) {
    ...
    case 'LOG':
      return (ctx) => {
        ...
        if (base === null || val === null || base <= 0 || val <= 0 || base === 1) return null;
    ...
    case 'DATE_ADD':
      return (ctx) => {
        ...
        if (!unit || amount === null || !date) return null;
    ...
    case 'RANK':
      return (ctx) => {
        if (argFns.length < 2) return null;
        const val = toNumber(argFns[0](ctx));
        if (val === null) return null;
        const population = argFns.slice(1).map(fn => toNumber(fn(ctx))).filter((v): v is number => v !== null);
    ...
    case 'LAG':
      return (ctx) => {
        ...
        if (!Array.isArray(arr) || offset === null) return defaultVal;
    ...
    case 'NTILE':
      return (ctx) => {
        ...
        const val = toNumber(argFns[1](ctx));
        if (n === null || val === null || n <= 0) return null;
...
function compileMetricNode(node: ExpressionNode, depth = 0): (ctx: MetricExpressionContext) => unknown {
  checkDepth(depth);
  switch (node.kind) {
    case 'literal': {
      const v = node.value;
...
function compileMetricBinaryOp(
  op: string,
  leftNode: ExpressionNode,
  rightNode: ExpressionNode,
  depth = 0,
): (ctx: MetricExpressionContext) => unknown {
  const leftFn = compileMetricNode(leftNode, depth);
  const rightFn = compileMetricNode(rightNode, depth);

  if (op === 'and') {
    return (ctx) => {
      if (!toBool(leftFn(ctx))) return false;
      return toBool(rightFn(ctx));
    };
  }
  ...
  if (op === 'or') {
    return (ctx) => {
      if (toBool(leftFn(ctx))) return true;
      return toBool(rightFn(ctx));
    };
  }
  ...
  return (ctx) => {
    ...
    switch (op) {
      ...
      case '*': return leftNum !== null && rightNum !== null ? leftNum * rightNum : null;
      case '/':
        if (leftNum !== null && rightNum !== null) return rightNum === 0 ? null : leftNum / rightNum;
        return null;
      case '%':
        if (leftNum !== null && rightNum !== null) return rightNum === 0 ? null : leftNum % rightNum;
        return null;
...
function compileMetricFunction(
  name: BuiltinFunction,
  argNodes: ExpressionNode[],
  depth = 0,
): (ctx: MetricExpressionContext) => unknown {
  ...
  switch (name) {
    ...
    case 'ROUND': return (ctx) => { const v = toNumber(argFns[0](ctx)); const d = argFns[1] ? toNumber(argFns[1](ctx)) ?? 0 : 0; if (v === null) return null; const f = Math.pow(10, d); return Math.round(v * f) / f; };
    ...
    case 'SUBSTR': return (ctx) => { const a = argFns[0](ctx); if (a === null || a === undefined) return null; const s = String(a); const st = toNumber(argFns[1](ctx)) ?? 0; const l = argFns[2] ? toNumber(argFns[2](ctx)) : null; return l !== null ? s.substring(st, st + l) : s.substring(st); };
    ...
    case 'COALESCE': return (ctx) => { for (const fn of argFns) { const a = fn(ctx); if (a !== null && a !== undefined) return a; } return null; };
    case 'IF': return (ctx) => toBool(argFns[0](ctx)) ? argFns[1](ctx) : (argFns[2] ? argFns[2](ctx) : null);
    case 'CLAMP': return (ctx) => { const v = toNumber(argFns[0](ctx)); const mn = toNumber(argFns[1](ctx)); const mx = toNumber(argFns[2](ctx)); if (v === null || mn === null || mx === null) return null; return Math.max(mn, Math.min(mx, v)); };
    // New math
    case 'SQRT': return (ctx) => { const v = toNumber(argFns[0](ctx)); if (v === null || v < 0) return null; return Math.sqrt(v); };
    case 'POWER': return (ctx) => { const b = toNumber(argFns[0](ctx)); const e = toNumber(argFns[1](ctx)); if (b === null || e === null) return null; return Math.pow(b, e); };
    case 'MOD': return (ctx) => { const a = toNumber(argFns[0](ctx)); const b = toNumber(argFns[1](ctx)); if (a === null || b === null || b === 0) return null; return a % b; };
    case 'LOG': return (ctx) => { if (argFns.length === 1) { const v = toNumber(argFns[0](ctx)); if (v === null || v <= 0) return null; return Math.log(v); } const base = toNumber(argFns[0](ctx)); const val = toNumber(argFns[1](ctx)); if (base === null || val === null || base <= 0 || val <= 0 || base === 1) return null; return Math.log(val) / Math.log(base); };
    case 'EXP': return (ctx) => { const v = toNumber(argFns[0](ctx)); if (v === null) return null; return Math.exp(v); };
    // New string
    case 'LEFT': return (ctx) => { const s = argFns[0](ctx); if (s === null || s === undefined) return null; const n = toNumber(argFns[1](ctx)); if (n === null) return null; return String(s).substring(0, n); };
    case 'RIGHT': return (ctx) => { const s = argFns[0](ctx); if (s === null || s === undefined) return null; const n = toNumber(argFns[1](ctx)); if (n === null) return null; return String(s).slice(-n); };
    case 'REPLACE': return (ctx) => { const s = argFns[0](ctx); if (s === null || s === undefined) return null; const search = argFns[1](ctx); const repl = argFns[2](ctx); if (search === null || search === undefined) return null; return String(s).replaceAll(String(search), String(repl ?? '')); };
    case 'REPEAT': return (ctx) => { const s = argFns[0](ctx); if (s === null || s === undefined) return null; const n = toNumber(argFns[1](ctx)); if (n === null || n < 0) return null; return String(s).repeat(n); };
    // New date
    case 'DATE_DIFF': return (ctx) => { const unit = argFns[0](ctx); const start = toDate(argFns[1](ctx)); const end = toDate(argFns[2](ctx)); if (!unit || !start || !end) return null; const ms = end.getTime() - start.getTime(); const u = String(unit).toLowerCase(); switch (u) { case 'day': return Math.floor(ms / 86400000); case 'hour': return Math.floor(ms / 3600000); case 'minute': return Math.floor(ms / 60000); case 'month': return (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth()); case 'year': return end.getFullYear() - start.getFullYear(); default: return null; } };
    case 'DATE_ADD': return (ctx) => { const unit = argFns[0](ctx); const amount = toNumber(argFns[1](ctx)); const date = toDate(argFns[2](ctx)); if (!unit || amount === null || !date) return null; const r = new Date(date.getTime()); const u = String(unit).toLowerCase(); switch (u) { case 'day': r.setDate(r.getDate() + amount); break; case 'month': r.setMonth(r.getMonth() + amount); break; case 'year': r.setFullYear(r.getFullYear() + amount); break; case 'hour': r.setHours(r.getHours() + amount); break; case 'minute': r.setMinutes(r.getMinutes() + amount); break; default: return null; } return r; };
    case 'FORMAT_DATE': return (ctx) => { const fmt = argFns[0](ctx); const date = toDate(argFns[1](ctx)); if (!fmt || !date) return null; let r = String(fmt); r = r.replace('YYYY', String(date.getFullYear())); r = r.replace('MM', String(date.getMonth() + 1).padStart(2, '0')); r = r.replace('DD', String(date.getDate()).padStart(2, '0')); r = r.replace('HH', String(date.getHours()).padStart(2, '0')); r = r.replace('mm', String(date.getMinutes()).padStart(2, '0')); r = r.replace('ss', String(date.getSeconds()).padStart(2, '0')); return r; };
    ...
    // Window
    case 'RANK': return (ctx) => { if (argFns.length < 2) return null; const val = toNumber(argFns[0](ctx)); if (val === null) return null; const pop = argFns.slice(1).map(fn => toNumber(fn(ctx))).filter((v): v is number => v !== null); const sorted = [...pop].sort((a, b) => b - a); const idx = sorted.indexOf(val); return idx === -1 ? null : idx + 1; };
    case 'DENSE_RANK': return (ctx) => { if (argFns.length < 2) return null; const val = toNumber(argFns[0](ctx)); if (val === null) return null; const pop = argFns.slice(1).map(fn => toNumber(fn(ctx))).filter((v): v is number => v !== null); const unique = [...new Set(pop)].sort((a, b) => b - a); const idx = unique.indexOf(val); return idx === -1 ? null : idx + 1; };
    case 'LAG': return (ctx) => { const arr = argFns[0](ctx); const offset = toNumber(argFns[1](ctx)); const def = argFns[2] ? argFns[2](ctx) : null; if (!Array.isArray(arr) || offset === null) return def; const idx = arr.length - 1 - offset; return idx >= 0 && idx < arr.length ? arr[idx] : def; };
    case 'LEAD': return (ctx) => { const arr = argFns[0](ctx); const offset = toNumber(argFns[1](ctx)); const def = argFns[2] ? argFns[2](ctx) : null; if (!Array.isArray(arr) || offset === null) return def; const idx = arr.length - 1 + offset; return idx >= 0 && idx < arr.length ? arr[idx] : def; };
    case 'RUNNING_SUM': return (ctx) => { let sum = 0; for (const fn of argFns) { const v = toNumber(fn(ctx)); if (v !== null) sum += v; } return sum; };
    case 'NTILE': return (ctx) => { const n = toNumber(argFns[0](ctx)); const val = toNumber(argFns[1](ctx)); if (n === null || val === null || n <= 0) return null; const bucket = Math.ceil(val * n); return Math.max(1, Math.min(n, bucket)); };
    default: return () => null;
  }
}
 ...
 */
export function compileRowExpression(ast: ExpressionNode): CompiledRowExpression {
 ...
 */
export function compileMetricExpression(ast: ExpressionNode): CompiledMetricExpression {
  const fn = compileMetricNode(ast);
  return (ctx: MetricExpressionContext): number | null => {
    const result = fn(ctx);
