...
function classifySeverity(deviation: number, threshold: number): AnomalySeverity {
  // Critical if deviation exceeds 1.5x the threshold
  return deviation >= threshold * 1.5 ? 'critical' : 'warning';
}

function filterValid(series: number[]): { values: number[]; indices: number[] } {
  const values: number[] = [];
...
function detectZScore(series: number[], sigma: number): AnomalyResult[] {
...
function detectIQR(series: number[], multiplier: number): AnomalyResult[] {
  const { values, indices } = filterValid(series);
  if (values.length < 3) return [];
...
function detectMovingAvgDeviation(series: number[], windowSize: number, threshold: number): AnomalyResult[] {
  const { values, indices } = filterValid(series);
  if (values.length < 3) return [];
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (window.length < 2 && i > 0) continue;
 ...
 */
export function detectAnomalies(series: number[], config: AnomalyConfig): AnomalyResult[] {
 ...
 */
export function detectTrendChange(series: number[]): TrendChangeResult {
  if (series.length < 2) {
  ...
  function segmentTrend(start: number, end: number): TrendDirection {
    if (end - start < 1) return 'flat';
    let up = 0;
    let down = 0;
  ...
  let prevTrend: TrendDirection | null = null;
