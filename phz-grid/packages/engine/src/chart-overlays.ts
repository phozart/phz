...
import type { AlertRule, ThresholdBreachConfig, AlertSeverity } from './kpi-alerting.js';
 ...
 *   r2 = 1 - SS_res / SS_tot
 */
export function computeLinearRegression(data: { x: number; y: number }[]): LinearRegressionResult {
  const n = data.length;
  if (n === 0) return { slope: 0, intercept: 0, r2: 0 };
  if (n === 1) return { slope: 0, intercept: data[0].y, r2: 0 };
 ...
 */
export function computeMovingAverage(values: number[], period: number): (number | null)[] {
 ...
 */
export function computeExponentialRegression(
  data: { x: number; y: number }[],
): ExponentialRegressionResult {
  if (data.length === 0) return { a: 0, b: 0, r2: 0 };
  if (data.length === 1) return { a: data[0].y > 0 ? data[0].y : 0, b: 0, r2: 0 };
 ...
 */
export function resolveTargetForCategory(
 ...
 * an empty array.
 */
export function alertRuleToThresholdBands(alertRule: AlertRule): ChartThresholdBand[] {
  if (alertRule.type !== 'threshold_breach') return [];

  const config = alertRule.config as ThresholdBreachConfig;
