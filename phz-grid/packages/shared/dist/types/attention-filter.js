 ...
 */
export function filterAttentionItems(items, filters) {
    return items.filter(item => {
        ...
        if (filters.artifactId && filters.artifactId.length > 0) {
            if (!item.artifactId || !filters.artifactId.includes(item.artifactId))
                return false;
        }
        // Acknowledged: defaults to false (hide acknowledged)
        const ack = filters.acknowledged ?? false;
        if (item.acknowledged !== ack)
            return false;
        // Date range
 ...
 */
function filterExcludingFacet(items, filters, excludeField) {
 ...
 */
export function computeAttentionFacets(items, currentFilters) {
    const facets = [];
    // --- Priority facet (always present) ---
    const priorityItems = filterExcludingFacet(items, currentFilters, 'priority');
    const priorityValues = ['critical', 'warning', 'info'];
    const priorityFacetValues = priorityValues.map(p => ({
        value: p,
    ...
    const sourceFacetValues = allSources
        .map(s => ({
        value: s,
    ...
    if (artifactMap.size > 3) {
        const artifactFacetValues = Array.from(artifactMap.entries()).map(([id, { name, count }]) => ({
            value: id,
            count,
        }));
...
//# sourceMappingURL=attention-filter.js.map