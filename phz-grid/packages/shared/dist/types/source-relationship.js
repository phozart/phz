 ...
 */
export function resolveEffectiveJoin(relationships, fromSourceId, toSourceId) {
    const direct = relationships.find(r => (r.leftSourceId === fromSourceId && r.rightSourceId === toSourceId) ||
 ...
 */
export function flipJoinType(joinType) {
    if (joinType === 'left')
        return 'right';
    if (joinType === 'right')
        return 'left';
 ...
 */
export function isJoinPropagationAllowed(relationships, fromSourceId, toSourceId) {
 ...
 */
export function findReachableSources(relationships, fromSourceId, allSourceIds) {
    const visited = new Set([fromSourceId]);
 ...
 */
export function getJoinKeysForPair(relationships, sourceA, sourceB) {
    const rel = relationships.find(r => (r.leftSourceId === sourceA && r.rightSourceId === sourceB) ||
...
//# sourceMappingURL=source-relationship.js.map