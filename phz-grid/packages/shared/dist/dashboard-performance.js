 ...
 */
export function getSourceAssessment(estimatedRows, engine, hasArrowIPC) {
 ...
 */
export function getOverallAssessment(preloadMs, fullLoadMs) {
    ...
    if (worst < FULL_LOAD_FAST_MS)
        return 'fast';
    if (worst < FULL_LOAD_MODERATE_MS)
        return 'moderate';
    if (worst < FULL_LOAD_SLOW_MS)
        return 'slow';
    return 'warning';
}
// ========================================================================
 ...
 */
export function estimateDashboardPerformance(config, widgets, filters, sourceHints) {
    const warnings = [];
    const sources = [];
...
//# sourceMappingURL=dashboard-performance.js.map