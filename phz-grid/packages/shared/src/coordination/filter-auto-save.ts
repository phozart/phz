 ...
 */
export function createDefaultAutoSaveConfig(
  overrides?: Partial<FilterAutoSaveConfig>,
): FilterAutoSaveConfig {
  return {
    enabled: true,
    debounceMs: 500,
...
export function createFilterSnapshot(
  filters: Record<string, unknown>,
  context?: { artifactId?: string; userId?: string },
): FilterStateSnapshot {
  return {
    filters: { ...filters },
    timestamp: Date.now(),
 ...
 * a positive number.
 */
export function shouldAutoSave(config: FilterAutoSaveConfig): boolean {
...
export function pruneHistory(
  history: readonly FilterStateSnapshot[],
  config: FilterAutoSaveConfig,
): FilterStateSnapshot[] {
