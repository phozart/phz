...
function aggFnToSQL(fn: AggregationFunction, col: string): string {
  switch (fn) {
...
function buildHavingClause(having: HavingInput[]): { clause: string; params: unknown[] } {
  if (having.length === 0) return { clause: '', params: [] };
...
function buildWhereClause(filters: FilterInput[]): { clause: string; params: unknown[] } {
...
function filterToSQL(col: string, operator: FilterOperator, value: unknown): { sql: string; values: unknown[] } {
  switch (operator) {
    case 'equals':
...
export function buildGridQuery(input: GridQueryInput): SqlResult {
  ...
  if (input.sort.length > 0) {
    const sorts = input.sort.map(s =>
      `"${sanitizeIdentifier(s.field)}" ${s.direction === 'desc' ? 'DESC' : 'ASC'}`
    ).join(', ');
    sql += ` ORDER BY ${sorts}`;
  }
...
export function buildCountQuery(input: { tableName: string; filters: FilterInput[]; groupBy?: string[] }): SqlResult {
  const table = `"${sanitizeIdentifier(input.tableName)}"`;
  const params: unknown[] = [];
