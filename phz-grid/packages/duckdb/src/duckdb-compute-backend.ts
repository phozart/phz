...
export class DuckDBComputeBackend implements ComputeBackend {
  ...
  async aggregate(
    ...
    const fields = config.fields.map(f => ({
      field: f.field,
      functions: f.functions as string[] as import('./duckdb-aggregation.js').DuckDBAggregationFunction[],
    }));
  ...
  async computeCalculatedFields(
    _data: Record<string, unknown>[],
    fields: CalculatedFieldInput[],
  ): Promise<Record<string, unknown>[]> {
...
export function createDuckDBComputeBackend(executor: DuckDBQueryExecutor): ComputeBackend {
