...
## Packages Overview (19 packages)

phz-grid is a monorepo with 19 active packages. You only need to install the
ones relevant to your use case.

### Shell Packages (pick one or more based on your deployment)

| Package                  | Purpose                                                    |
| ------------------------ | ---------------------------------------------------------- |
| `@phozart/workspace` | Full admin shell — authoring, governance, configuration    |
| `@phozart/editor`    | Author shell — constrained authoring with curated measures |
| `@phozart/viewer`    | Analyst shell — read-only dashboards, reports, explorer    |
| `@phozart/shared`    | Shared types and infrastructure used by all shells         |

### Core Infrastructure

| Package                    | Purpose                                                |
| -------------------------- | ------------------------------------------------------ |
| `@phozart/core`        | Headless grid engine (data model, state, events)       |
| `@phozart/engine`      | BI engine (reports, dashboards, KPIs, filters, pivots) |
| `@phozart/definitions` | Serializable grid blueprints, stores, converters       |
| `@phozart/duckdb`      | DuckDB-WASM data source adapter                        |
| `@phozart/ai`          | AI toolkit (schema-as-contract, NL query)              |
| `@phozart/collab`      | Real-time collaboration (Yjs CRDTs)                    |
| `@phozart/local`       | Tier 2 local server (native DuckDB, filesystem)        |

### UI Packages

| Package                 | Purpose                                                   |
| ----------------------- | --------------------------------------------------------- |
| `@phozart/grid`     | Web Components grid (Lit rendering, virtualization, a11y) |
| `@phozart/criteria` | Selection criteria and filter UI components               |
| `@phozart/widgets`  | Dashboard widgets (bar-chart, KPI card, trend-line, etc.) |

### Framework Adapters

| Package                | Purpose                                               |
| ---------------------- | ----------------------------------------------------- |
| `@phozart/react`   | React wrappers for grid, viewer, editor, and criteria |
| `@phozart/vue`     | Vue wrappers                                          |
| `@phozart/angular` | Angular wrappers                                      |

### Additional

| Package               | Purpose                                                  |
| --------------------- | -------------------------------------------------------- |
| `@phozart/python` | Python package (pip install) for Jupyter/Panel/Streamlit |
| `@phozart/demo`   | Standalone demo app with sample datasets                 |
...
```bash
npm install @phozart/viewer @phozart/shared
```

### Author + Viewer (self-service authoring)

```bash
npm install @phozart/viewer @phozart/editor @phozart/shared
```

### Full platform (admin + author + viewer)

```bash
npm install @phozart/workspace @phozart/core @phozart/engine
```
...
import { MemoryDataAdapter } from '@phozart/shared/adapters/memory-data-adapter';
...
import { MemoryWorkspaceAdapter } from '@phozart/workspace/adapters/memory-adapter';
...
```typescript
import { analyzeSchema } from '@phozart/workspace/templates/schema-analyzer';
...
```typescript
import { matchTemplates } from '@phozart/workspace/templates/template-matcher';
import { DEFAULT_TEMPLATES } from '@phozart/workspace/templates/default-templates';
...
Use `@phozart/local` to start a persistent local server backed by the
...
```bash
npm install @phozart/local
```

### 2. Start the server programmatically

```typescript
import { createLocalServer } from '@phozart/local';
