...
graph TB
    ...
    subgraph "phz-grid Ecosystem"
        CORE[@phozart/core<br/>Headless Engine]
        GRID[@phozart/grid<br/>Web Components]
        REACT[@phozart/react]
        VUE[@phozart/vue]
        ANGULAR[@phozart/angular]
        DUCKDB[@phozart/duckdb]
        AI[@phozart/ai]
        COLLAB[@phozart/collab]
    end
...
graph TB
    subgraph "Browser Runtime"
        subgraph "@phozart/core - Headless Engine"
        ...
        subgraph "@phozart/grid - Rendering Layer"
...
graph TB
    subgraph "Layer 0: Core Engine (No Dependencies)"
        CORE[@phozart/core<br/>Headless State & Logic]
    end

    subgraph "Layer 1: Rendering (Depends on Core)"
        GRID[@phozart/grid<br/>Lit Web Components]
    end
    ...
    subgraph "Layer 3: Advanced Extensions (Depend on Core)"
        DUCKDB[@phozart/duckdb<br/>WASM SQL Engine]
        AI[@phozart/ai<br/>LLM Integration]
        COLLAB[@phozart/collab<br/>Yjs CRDTs]
    end

    subgraph "Layer 4: Documentation (Depends on All)"
        DOCS[@phozart/docs<br/>VitePress Site]
    end
...
| ---------------------- | --------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------ |
| `@phozart/core`    | State management, event system, data model, row pipeline, hooks | `createGrid()`, `GridState`, `EventEmitter`, `registerHook()` | None (zero deps)                                 |
| `@phozart/grid`    | Lit components, DOM virtualization, a11y shadow tree, theming   | `<phz-grid>`, `<phz-cell>`, `<phz-header>`                    | `@phozart/core`, `lit@^5.0`                  |
| `@phozart/react`   | React wrapper, hooks, prop mapping                              | `<PhzGrid>`, `useGridState()`, `useGridApi()`                 | `@phozart/grid`, `react@^18`                 |
| `@phozart/vue`     | Vue wrapper, composables, reactive bindings                     | `<PhzGrid>`, `useGrid()`                                      | `@phozart/grid`, `vue@^3`                    |
| `@phozart/angular` | Angular component, RxJS observables, DI integration             | `PhzGridComponent`, `GridService`                             | `@phozart/grid`, `@angular/core@^17`         |
| `@phozart/duckdb`  | DuckDB-WASM data source, SQL query engine, Arrow integration    | `DuckDBDataSource`, `query()`, `loadParquet()`                | `@phozart/core`, `@duckdb/duckdb-wasm@^1.31` |
| `@phozart/ai`      | Schema generation, LLM tool-use, NL query parser                | `generateSchema()`, `queryFromNL()`, `AIAdapter`              | `@phozart/core`, `zod@^3`                    |
| `@phozart/collab`  | Yjs CRDT bindings, awareness, sync adapters                     | `CollabProvider`, `YjsBinding`, `PresenceManager`             | `@phozart/core`, `yjs@^13`, `y-websocket@^2` |
| `@phozart/docs`    | Documentation site, live examples, API reference                | N/A (docs only)                                               | All packages                                     |

### Strict Layering Rules

1. **Core is Zero-Dependency** — `@phozart/core` MUST NOT depend on any rendering library, framework, or browser API (except Web APIs)
2. **Grid Depends Only on Core + Lit** — `@phozart/grid` MUST NOT depend on React/Vue/Angular
3. **Framework Adapters are Thin Wrappers** — React/Vue/Angular adapters MUST NOT implement grid logic, only prop/event mapping
4. **Advanced Modules Extend Core** — DuckDB/AI/Collab MUST depend on `@phozart/core`, NOT on `@phozart/grid`
...
interface GridEvents {
  ...
  selectionChange: {
    selectedRowIds: Set<RowId>;
    addedRowIds: Set<RowId>;
    removedRowIds: Set<RowId>;
  };
...
sequenceDiagram
    participant User
    participant Grid as @phozart/grid<br/>Lit Component
    participant Core as @phozart/core<br/>State Engine
...
class KeyboardNavigationManager {
  ...
  private announceCellFocus(cellId: CellId) {
    ...
    const announcement =
      `Row ${rowIndex + 1} of ${this.gridState.sortedRowIds.length}, ` +
...
class CollabGridBinding {
  ...
  private connectToSyncBackend() {
    ...
    const provider = new WebsocketProvider('wss://sync.phozart.io', this.roomId, this.yDoc);
...
class NLQueryProcessor {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a data grid assistant. Use the provided tools to answer user queries about the grid data.',
        },
        {
          role: 'user',
          content: userQuery,
  ...
  private async executeToolCall(toolCall: ToolCall): Promise<any> {
    switch (toolCall.name) {
      case 'filter_rows':
        return this.gridApi.setColumnFilter(toolCall.arguments.columnId, {
...
| Total bundle (with Lit)   | <70 KB gzipped | Full `@phozart/grid` package   |
...
1. **Zero dependencies for core** — `@phozart/core` has ZERO npm dependencies
...
graph TB
    subgraph "npm Registry"
        NPM_CORE[@phozart/core]
        NPM_GRID[@phozart/grid]
        NPM_REACT[@phozart/react]
...
| --------------------- | ----------- | ----------------------------------------------------------------- |
| **Next.js** (React)   | Full        | Declarative Shadow DOM, no `window` access in `@phozart/core` |
...
## Shared Infrastructure Package (`@phozart/shared`)

Added in v15. `@phozart/shared` is the extracted foundation layer that all shells and higher-level packages depend on. It contains zero Lit or DOM dependencies and exports only pure functions, type definitions, and constants.
...
Before v15, adapter interfaces and shared types were defined in `@phozart/workspace` and re-exported via shims. This caused circular dependency risks and forced viewer-only deployments to bundle the full workspace. Extracting shared infrastructure into its own package:
...
v15 introduces a three-shell architecture separating concerns by persona. Each shell is a standalone package with its own Lit components and headless state machines. All three depend on `@phozart/shared` for adapters, types, and design tokens.
...
Layer 1: Core Engine
  @phozart/core (depends on shared, definitions)

Layer 2: Computation
  @phozart/engine (depends on core, shared)
  @phozart/duckdb (depends on core)

Layer 3: UI Components
  @phozart/grid (depends on core)
  @phozart/criteria (depends on core)
  @phozart/widgets (depends on engine)

Layer 4: Shells
  @phozart/workspace (depends on core, engine, shared, criteria, definitions)
  @phozart/viewer (depends on shared)
  @phozart/editor (depends on shared)

Layer 5: Extensions
  @phozart/ai (depends on core)
  @phozart/collab (depends on core)

Layer 7: Framework Adapters
  @phozart/react (depends on grid, criteria, workspace)
  @phozart/vue (depends on grid)
  @phozart/angular (depends on grid)

Independent:
  @phozart/local (native DuckDB, filesystem persistence)
  @phozart/definitions (serializable grid blueprints)
```
...
Five new subsystems were added to `@phozart/engine`:
...
Generates OpenAPI 3.1.0 documents from the phz-grid API specification types defined in `@phozart/shared/types`.
...
New authoring and shell state machines added to `@phozart/workspace`:
