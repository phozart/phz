...
> Formal requirements for the `@phozart/react` package React wrappers.
> Traced to the underlying `<phz-grid>` and `<phz-workspace>` Web Components.
> Note: `@phozart/grid-admin` has been archived. Admin functionality is now in `@phozart/workspace`.
...
The component MUST dynamically import `@phozart/workspace` on first mount to register the custom element. (Previously `@phozart/grid-admin`, now archived.)
