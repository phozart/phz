...
ADMIN (workspace authoring):
  ...
  │
  ├── Creates and manages FilterRules (conditional business rules)
  │   ("If viewer.country=BE AND direction=origin, restrict country to [BE]")
  │   ("If service_type=Express, restrict weight_class to [0-2kg, 2-5kg]")
  │
...
END USER (consumer side):
  ...
  │
  ├── Sets filter values within the constraints defined by:
  │   · validation rules (from ArtifactFilterContract)
  │   · business rules (from FilterRules — restrict, hide, disable, force)
  │   · security bindings (from FilterDefinition — viewer attribute restrictions)
  │
...
Admins own the filter definitions, the business rules, and the filter
contracts per artifact. Users own personal presets. Users CANNOT create
filter definitions, add filters to dashboards, modify validation rules,
create business rules, or configure transforms. They interact with the
filters the admin provided, within the constraints the admin and the
business rules define. The personal preset is their only customization, and
...
- If an admin creates a new FilterRule that restricts values, existing
  user presets with now-restricted values are automatically pruned to
  the allowed set when the preset is loaded.
...
interface FilterDefinition extends VersionedConfig {
  ...
  // Defaults — static or dynamic (resolved at runtime)
  defaultValue?: FilterDefault
  required: boolean               // cannot be cleared
}

// Filter defaults can be static values or dynamic expressions
// that resolve at runtime. Dynamic defaults are the key feature
// for "last 7 days", "current quarter", "viewer's region", etc.

type FilterDefault =
  | StaticDefault
  | RelativeDateDefault
  | ViewerAttributeDefault
  | ExpressionDefault

interface StaticDefault {
  type: 'static'
  value: unknown                  // concrete value, e.g. 'Europe' or 2025
}

interface RelativeDateDefault {
  type: 'relative-date'
  // Preset relative periods
  preset?: 'today' | 'yesterday' | 'last-7d' | 'last-30d' | 'last-90d' |
           'this-week' | 'last-week' | 'this-month' | 'last-month' |
           'this-quarter' | 'last-quarter' | 'this-year' | 'last-year' |
           'ytd' | 'qtd' | 'trailing-12m'

  // OR: custom relative definition
  custom?: {
    // Anchor: where to start counting from
    anchor: 'today' | 'start-of-week' | 'start-of-month' |
            'start-of-quarter' | 'start-of-year' | 'end-of-month' |
            'end-of-quarter' | 'end-of-year'

    // Offset from anchor: shift the anchor before computing the range
    anchorOffset?: { amount: number; unit: 'day' | 'week' | 'month' | 'quarter' | 'year' }
    // e.g. { amount: -3, unit: 'day' } → "3 days ago" as anchor

    // Range: how far back/forward from the (offset) anchor
    rangeStart: { amount: number; unit: 'day' | 'week' | 'month' | 'quarter' | 'year' }
    rangeEnd?: { amount: number; unit: 'day' | 'week' | 'month' | 'quarter' | 'year' }
    // If rangeEnd is omitted, the range ends at the anchor.
    // e.g. anchor='today', anchorOffset={-3, 'day'}, rangeStart={-7, 'day'}
    //   → "last 7 days starting from 3 days ago" = 10 days ago to 3 days ago
  }

  // Respects fiscal calendar from TimeIntelligenceConfig if available
  useFiscalCalendar?: boolean
}

interface ViewerAttributeDefault {
  type: 'viewer-attribute'
  attribute: string               // e.g. 'region', 'country', 'department'
  // Resolves to viewerContext.attributes[attribute] at runtime.
  // If the attribute is absent, falls back to no default (filter is empty).
}

interface ExpressionDefault {
  type: 'expression'
  // An expression AST that evaluates to the default value at runtime.
  // Can reference: today(), viewer.attribute, other filter values.
  // Example: "QUARTER(TODAY()) - 1" → previous quarter number
  expression: ExpressionAST
}
```

**How dynamic defaults resolve:**

When the filter bar renders, for each filter with a dynamic default:

1. `StaticDefault`: use the value as-is.
2. `RelativeDateDefault`: compute the date range from `preset` or `custom`
   relative to the current date. If `useFiscalCalendar` is true and
   `TimeIntelligenceConfig` is available, fiscal calendar rules apply
   (fiscal year start month, week start day). The result is a concrete
   `{ from: Date, to: Date }` range.
3. `ViewerAttributeDefault`: read `viewerContext.attributes[attribute]`.
   A Belgian user opens the dashboard → the country filter defaults to 'BE'.
4. `ExpressionDefault`: evaluate the expression using the existing expression
   compiler. Available context: `TODAY()`, `YEAR(TODAY())`, viewer attributes
   as variables.

Dynamic defaults resolve on every dashboard open, not once at save time.
"Last 7 days" always means the 7 days ending yesterday, regardless of
when the admin configured it.

**Admin configures dynamic defaults in the workspace:**

In the filter definition editor (FilterDesigner), the "Default Value" field
is not a text input. It's a picker with four tabs:

- **Static**: enter a fixed value (text, number, date)
- **Relative Date**: pick a preset from a dropdown ("Last 7 days",
  "This quarter", etc.) OR configure a custom relative range with
  anchor, offset, and range controls. A live preview shows the
  computed date range for today's date.
- **Viewer Attribute**: pick an attribute from a dropdown of known
  viewer attributes. Preview shows: "Will resolve to the viewer's
  [region] value."
- **Expression**: formula editor (same as MetricBuilder formula bar).
  Preview shows the evaluated result.

The same picker appears in `DashboardFilterRef.defaultValueOverride`
for per-dashboard default overrides.

**How this interacts with presets:**

Published presets can also contain dynamic defaults. An admin creates a
published preset "Rolling Last 7 Days" with:

```typescript
{
  filterDefinitionId: 'time-period',
  value: { type: 'relative-date', preset: 'last-7d' }
}
```

When this preset is applied (manually or as the dashboard default), the
filter resolves to the current "last 7 days" at open time. Personal
presets can also contain dynamic values: a user saves "My View" with
"last 30 days" as a relative date, and it always shows the most recent
30 days.
...
interface DashboardFilterRef {
  ...
  defaultValueOverride?: FilterDefault  // different default than the central definition
                                        // supports static and dynamic (relative dates, viewer attributes)
...
### 1.0.4 Filter business rules (conditional multi-filter constraints)

Security bindings handle simple cases: one viewer attribute restricts one
filter. Cascading handles data-driven parent→child dependencies. But
enterprise filter logic often involves conditional rules that span multiple
filters and viewer attributes simultaneously:

- "If viewer.country = BE and the direction filter = 'origin', restrict
  the country filter to [BE] only"
- "If service_type = 'Express', then weight_class can only be 0-5kg"
- "If viewer.role = 'operator-manager', hide the operator filter entirely
  and lock it to viewer.operator"
- "If date_range spans more than 12 months and granularity = 'daily',
  force granularity to 'monthly' (too much data for daily)"
- "Filters A and B are mutually exclusive: if A has a value, B is disabled"

These are business rules, not data relationships. They encode organizational
policy about what filter combinations are valid, visible, and allowed for
specific viewers. The admin authors them in the workspace.

```typescript
interface FilterRule extends VersionedConfig {
  type: 'filter-rule'
  name: string                    // "BE origin restriction", "Express weight limit"
  description?: string
  enabled: boolean
  ...
  conditionLogic: 'all' | 'any'   // AND / OR across conditions
...
type FilterRuleCondition =
  ...
  | FilterPresenceCondition

interface ViewerCondition {
  type: 'viewer-attribute'
  attribute: string               // e.g. 'country', 'role', 'department'
  operator: 'equals' | 'notEquals' | 'in' | 'notIn' | 'exists'
  value: unknown
}

interface FilterValueCondition {
  type: 'filter-value'
  filterDefinitionId: string      // which filter to check
  operator: 'equals' | 'notEquals' | 'in' | 'notIn' | 'between' |
            'greaterThan' | 'lessThan' | 'isEmpty' | 'isNotEmpty'
  value?: unknown
}

interface FilterPresenceCondition {
  type: 'filter-active'
  filterDefinitionId: string      // is this filter currently set?
  active: boolean                 // true = filter has a value, false = filter is empty
}

type FilterRuleAction =
  | RestrictValuesAction
  | SetValueAction
  | HideFilterAction
  | DisableFilterAction
  | ForceValueAction
...
interface RestrictValuesAction {
  type: 'restrict-values'
  targetFilterId: string          // which filter to restrict
  // Static whitelist
  allowedValues?: unknown[]
  // OR: dynamic — pull allowed values from a lookup based on the condition context
  dynamicLookup?: {
    dataSourceId: string
    filterField: string           // field matching the triggering condition
    targetField: string           // field to pull allowed values from
  }
}
...
interface SetValueAction {
  ...
  value: unknown                  // set the filter to this value
  userCanOverride: boolean        // if true, the value is a default; if false, it's locked
}

interface HideFilterAction {
  type: 'hide'
  targetFilterId: string          // remove this filter from the filter bar entirely
}

interface DisableFilterAction {
  type: 'disable'
  targetFilterId: string          // show the filter but make it non-interactive
  message?: string                // tooltip: "This filter is not available when X = Y"
}

interface ForceValueAction {
  type: 'force-value'
  targetFilterId: string
  // Value comes from a viewer attribute or another filter
  sourceType: 'viewer-attribute' | 'filter-value'
  sourceAttribute?: string        // for viewer-attribute
  sourceFilterId?: string         // for filter-value
  // The filter is locked to this value and cannot be changed
}

interface ShowWarningAction {
  type: 'show-warning'
  message: string                 // displayed as an inline warning in the filter bar
...
When the filter bar renders or any filter value changes, the rule engine
runs:

1. Collect all enabled `FilterRule` artifacts, sorted by priority (highest first)
2. For each rule, evaluate its conditions against the current state:
   - `ViewerCondition`: check `viewerContext.attributes`
   - `FilterValueCondition`: check the current `FilterContextState`
   - `FilterPresenceCondition`: check whether the filter has an active value
3. If conditions match (all/any based on `conditionLogic`), execute the actions
4. Actions modify the filter bar state:
   - `restrict-values`: the filter dropdown only shows allowed values
   - `set-value`: the filter value is pre-filled (overridable or locked)
   - `hide`: the filter is removed from the bar (and its value is cleared)
   - `disable`: the filter is grayed out with a tooltip explanation
   - `force-value`: the filter is locked to a value derived from context
   - `show-warning`: an inline message appears in the filter bar
5. If multiple rules target the same filter, higher-priority rules win.
   For `restrict-values`, intersect the allowed values across rules.

**Rule evaluation is synchronous and client-side.** It runs on the
current filter state and viewer context, not on data. This means it's
instant — no server round-trip when a filter changes to check whether
rules apply.

The `restrict-values` action with `dynamicLookup` is the one exception:
it needs to query the DataAdapter to resolve which values are allowed
based on the triggering condition. This query uses
`dataAdapter.getDistinctValues()` with the condition context as a filter,
same as cascading. The result is cached for the session.

**How the admin configures rules in the workspace:**

The workspace filter catalog includes a "Rules" tab alongside the filter
list:

1. "Create Rule" → name, description, priority
2. "When..." → condition builder:
   - "Viewer attribute [country] [equals] [BE]" (dropdown of known attributes)
   - "AND Filter [Direction] [equals] [origin]" (dropdown of FilterDefinitions)
   - Toggle between ALL / ANY conditions
3. "Then..." → action builder:
   - "Restrict [Country] to values where [origin_country = BE]"
   - "Hide [Operator] and lock to viewer's operator"
   - "Show warning: Daily granularity not available for ranges > 12 months"
4. Test the rule: preview the filter bar with simulated viewer context
   and filter values, see which rules fire and what actions apply

Rules are `VersionedConfig` artifacts with `visibility: 'published'`
(they apply org-wide). Admins can temporarily disable a rule without
deleting it.

**Real-world examples for postal operations:**

| Rule | Conditions | Actions |
|------|-----------|---------|
| BE origin restriction | viewer.country = 'BE' AND filter.direction = 'origin' | Restrict country filter to ['BE'] |
| Express weight limit | filter.service_type = 'Express' | Restrict weight_class to ['0-2kg', '2-5kg'] |
| Operator lock for managers | viewer.role = 'operator-manager' | Force operator filter to viewer.operator, hide operator from bar |
| Daily granularity guard | filter.date_range spans > 365 days AND filter.granularity = 'daily' | Force granularity to 'monthly', show warning |
| Mutual exclusion | filter.origin_country is not empty | Disable destination_country filter with message |

**Interaction with the filter resolution pipeline (section 1.1.2):**

Filter rules execute BEFORE the artifact filter contract validation.
The pipeline becomes:

```
Filter value set by user
  → 1. Rule engine evaluates (restrict, force, hide, disable)
  → 2. Contract check (does this artifact accept this filter?)
  → 3. Validation (is the value valid per the contract?)
  → 4. Transform (convert value for the artifact's data model)
  → 5. Binding resolution (map to actual field name)
  → 6. Merge with other filter levels
  → FilterExpression[] for dataAdapter.execute()
```

Rules can prevent invalid values from ever reaching the validation stage.
If a rule restricts the country filter to ['BE'], the user can only select
BE, so the validation step never sees an invalid country.

### 1.0.5 Shared and personal presets reference central filter IDs
...
interface PresetFilterValue {
  ...
  // Value can be static or dynamic (same FilterDefault type)
  // Static: { type: 'static', value: 'Europe' }
  // Dynamic: { type: 'relative-date', preset: 'last-7d' }
  value: FilterDefault
  label: string                   // human-readable for display
}
```

Presets with dynamic values resolve at load time. An admin creates a
published preset "Rolling Last 7 Days" with:
`{ filterDefinitionId: 'time-period', value: { type: 'relative-date', preset: 'last-7d' } }`

Every time anyone applies this preset, it resolves to the current last
7 days. A user saves a personal preset "My View: Europe, Last 30 Days"
with a mix of static ('Europe') and dynamic ('last-30d') values. Both
resolve correctly when loaded.

```typescript
interface FilterPreset extends VersionedConfig {
  // ... fields above ...

  // Optional presentation overrides (sort, grouping, column order, density)
  // When a user "saves their view", this captures HOW they want to see
  // the data, not just WHAT data they want to see.
  presentationOverrides?: Partial<DefaultPresentation>
}
```

When "Save my view" captures presentation state, the personal preset
stores both filter values and presentation settings. On load:
1. Filter values from the preset are applied (including dynamic resolution)
2. Presentation overrides from the preset are applied on top of the
   admin's `DefaultPresentation`
3. The user sees their grid sorted their way, grouped their way, with
   their column order — on the data slice they saved.
