...
### 8.3 The `npx` launcher

Publish a `phz-playground` package that is a single command:

```bash
npx phz-playground
# → starts a local Vite dev server on port 5173
# → opens the browser to the playground
# → all data stored locally via OPFS
```

This is the CLI equivalent of the hosted demo. It runs entirely locally.
The package contains the pre-built workspace shell, default widgets,
DuckDB-WASM adapter, and the file upload UI. No internet required after
initial install.

### 8.4 Documentation structure

Four persona entry points:
