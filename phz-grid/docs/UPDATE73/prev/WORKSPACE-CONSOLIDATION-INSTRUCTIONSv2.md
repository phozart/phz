...
- A sidebar or top nav with sections: Catalog, Explore, Create New, Templates, Alerts, Placements
...
- `listDataSources()` → list loaded Parquet/CSV files
...
This is the zero-ops getting-started path: load a CSV/Parquet file, the
DataAdapter provides the schema, the suggestion flow proposes templates,
the user picks one, and has a dashboard in under 5 minutes. No server needed.
...
## Phase 8: Getting-Started Experience
...
- Open workspace → Select data source → Pick template → Customize → Save
- Video/GIF walkthrough of the suggestion flow
...
grid-admin → engine-admin → grid-creator → phz-workspace
```

`phz-workspace` is last in the chain. No other package depends on it.
