...
interface WidgetCommonConfig {
  ...
  // 'drill': click triggers the primary NavigationLink (configured in 4.4)
  // 'filter': click applies a cross-filter to other widgets
  // 'navigate': click fires a navigate event (consumer handles routing)
...
interface TableTypeConfig {
  ...
  rowClickAction?: 'select' | 'drill' | 'expand' | 'navigate' | 'none'
}
...
type WidgetEvent =
  ...
  | { type: 'navigate'; link: NavigationLink; resolvedFilters: FilterExpression[]; sourceContext: { widgetId: string; clickedValue: unknown; clickedRow?: Record<string, unknown> } }
...
## Part 6: Workspace Information Architecture

The workspace has grown to 15+ artifact types. Without clear organization,
the admin UI becomes a sprawl. The information architecture groups
concepts by what the admin is thinking about, not by implementation type.

### 6.1 Three top-level concerns

The workspace sidebar organizes into three concern areas. Each has its
own section with sub-navigation.
...
├──────────┬──────────────────────────────────────────────┤
│          │                                              │
│  CONTENT │  Where the admin spends 80% of their time.  │
│  ──────  │  Building and managing the things users see. │
│          │                                              │
│  📊 My    │  (active section content)                   │
│    Catalog│                                              │
│  🔍 Explore                                             │
│  📝 Create│                                              │
│    New    │                                              │
...
│    ates   │                                              │
│          │                                              │
│  DATA    │  Where the data comes from and how it's      │
│  ──────  │  connected. Configure once, use everywhere.  │
│          │                                              │
│  🗄️ Data  │                                              │
│    Sources│                                              │
│  🔗 Conn- │                                              │
│    ections│                                              │
│          │                                              │
│  GOVERN  │  Rules, filters, alerts, publishing.         │
│  ──────  │  How the system behaves and who sees what.   │
│          │                                              │
│  🎯 Filt- │                                              │
│    ers    │                                              │
│  ⚙️ Rules │                                              │
...
### 6.2 Content section (what users see)

This is the primary workspace for building artifacts.

| Navigation item | What it contains | Opens |
|----------------|-----------------|-------|
| Catalog | All artifacts the admin can see (grouped by My Work / Shared / Published) | Catalog browser |
| Explore | Visual query builder on any data source | Data explorer |
| Create New | Artifact creation flow with template suggestions | Suggestion flow → appropriate designer |
| Templates | Template gallery with built-in and custom templates | Template gallery |

When the admin clicks into an artifact from the catalog, the content
area shows the appropriate designer with the workspace shell header
(breadcrumbs, save status, preview toggle). Designers include:

- Dashboard builder (layout canvas + widget picker + filter bar config)
- Report designer (6-step wizard)
- Grid definition editor (grid creator → grid admin)
- KPI designer (6-step wizard)
- Metric builder

### 6.3 Data section (where it comes from)

This section configures data sources and is typically set up once, then
rarely touched.

| Navigation item | What it contains | Opens |
|----------------|-----------------|-------|
| Data Sources | All registered data sources (server-provided + uploaded + connected) | Unified data source panel |
| Connections | Remote URL and API connections | Connection editor |

"Data Sources" shows the unified panel from section 5.8 of the main
instructions. "Connections" is the configuration UI for remote data
connectors (section 5.7). File uploads are initiated from Data Sources.

### 6.4 Govern section (how it behaves)

This section configures system-wide behavior: filters, business rules,
alerts, and publishing. These settings affect all artifacts.

| Navigation item | What it contains | Opens |
|----------------|-----------------|-------|
| Filters | Central filter catalog: FilterDefinitions, bindings, security bindings, dynamic defaults | Filter designer (list + detail) |
| Rules | Filter business rules: conditional multi-filter constraints | Rule editor (list + detail) |
| Alerts | Alert rules and subscriptions, active breaches | Alert rule designer + breach panel |
| Publish | Placement records: which artifacts are published to which routes for which roles | Placement manager |

"Filters" is the most complex section. It shows a list of all
`FilterDefinition` artifacts with: name, type (select/date-range/etc.),
number of bindings, number of dashboards using it, last modified. Clicking
a filter opens its editor (FilterDesigner) with tabs for:
- General (name, type, value source)
- Bindings (per data source field mapping)
- Security (viewer attribute restrictions)
- Defaults (dynamic default configuration)
- Usage (which dashboards/reports reference this filter)

### 6.5 Navigation from content to governance

The admin shouldn't have to leave the dashboard builder to configure
filters. Cross-navigation shortcuts:

- **In the dashboard builder's filter bar config**: a "Manage in catalog"
  link next to each filter opens the filter definition editor in a side
  panel (or navigates to the Govern > Filters section with that filter
  pre-selected).

- **In any widget's config panel**: a "Set alert" link opens the alert
  rule designer pre-populated with the widget's KPIs.

- **In the dashboard builder header**: "Publish" button opens a mini
  placement editor inline (route, roles, label) without leaving the
  builder. For full placement management, link to Govern > Publish.

- **In any designer's filter contract config**: a "Create new filter"
  link opens the filter definition form in a modal, then returns to
  the designer with the new filter referenced.

These shortcuts mean the admin can stay in context while accessing
governance features. The Govern section is for dedicated management,
not for every interaction.

---

## Part 7: Role-Based Workspace Experience

The workspace serves three distinct roles. Each role sees a different
subset of the workspace, not a different product.

### 7.1 The three roles

```typescript
type WorkspaceRole = 'admin' | 'author' | 'viewer'
```

The consumer provides the role as part of `ViewerContext`:
...
const workspace = createWorkspace({
  ...
  workspaceRole: 'author'    // determines what the workspace shows
})
```

### 7.2 What each role sees

**Admin** (full access):
- All three sections: Content, Data, Govern
- Can create/edit/delete any artifact (respecting visibility rules)
- Can manage filter definitions, filter rules, and alert rules
- Can publish artifacts and manage placements
- Can configure filter contracts, validation, and transforms on artifacts
- Can set default presentations and dynamic defaults
- Can create organization templates
- Can manage data source connections
- Sees "Preview as..." in preview mode to simulate other users

**Author** (creates content, no governance):
- Content section: full access (catalog, explore, create, templates)
- Data section: read-only (can see data sources and their schemas,
  can upload files in Tier 1/2, but cannot modify server-provided
  data source configurations)
- Govern section: hidden entirely
- Can create dashboards, reports, grids, KPIs (personal or shared)
- Cannot create filter definitions, filter rules, or alert rules
- Cannot modify filter contracts or validation rules
- Cannot publish artifacts (no PlacementRecord access)
- Can use existing templates but cannot create organization templates
- Can save personal and shared presets
- Can set their own default presentation preferences via personal presets

**Viewer** (end user, consumes only):
- Content section: limited to catalog browser showing published and
  shared-with-them artifacts, plus "My Work" (personal copies)
- Explore section: available (they can explore data sources they have access to)
- No Data or Govern sections
- Can open published dashboards and reports (read-only view)
- Can duplicate a published artifact to "My Work" for personal customization
- Can apply filters within admin-defined constraints and business rules
- Can save personal presets (filter values + presentation overrides)
- Cannot edit any artifact's structure (widgets, layout, columns)
- Cannot access any designer or builder
- Their "workspace" is essentially: catalog of available artifacts,
  personal presets, personal copies, and the explorer

### 7.3 Role determines workspace shell configuration

The workspace shell reads `workspaceRole` and adjusts:

- **Sidebar items**: only sections the role has access to
- **Artifact card actions**: admin sees edit/delete/publish, author sees
  edit/delete, viewer sees open/duplicate
- **Create button**: admin and author see "Create New", viewer sees
  "Duplicate to My Work" on existing artifacts
- **Configuration panels**: admin sees all config tabs on designers,
  author sees content tabs but not filter contract/governance tabs,
  viewer doesn't open designers at all
- **Preview mode**: admin has "Preview as..." simulation, author has
  standard preview, viewer is always in "preview" (they only see
  rendered output)

### 7.4 The viewer's workspace is lightweight

When `workspaceRole: 'viewer'`, the workspace shell simplifies to:

```
┌──────────────────────────────────────────┐
│  My Dashboards          [Explore data]   │
├──────────────────────────────────────────┤
│  Published                               │
...
│  My Work                                 │
│  ├── My Sales View (copy)                │
│  └── Q3 Exploration (saved explore)      │
│                                          │
│  [Filter presets: My View ▾] [Save view] │
└──────────────────────────────────────────┘
```

This is not the full workspace shell with a hidden sidebar. It's a
purpose-built lightweight component that shows the viewer their available
artifacts, their presets, and the ability to explore. Opening an artifact
renders it in place (or in a modal/new page depending on consumer routing).

If the consumer doesn't want viewers to access the workspace shell at all
(they just render dashboards inline in their app), they use
`createWorkspaceClient()` instead of `createWorkspace()`. The workspace
shell is optional. The rendering pipeline works without it.

### 7.5 Role enforcement

phz-grid does NOT enforce roles on the backend. The `workspaceRole` controls
what the UI shows. The consumer's `WorkspaceAdapter` is responsible for
rejecting unauthorized operations:

- If an author tries to save a filter definition (by manipulating the
  API directly), the adapter should reject it
- If a viewer tries to edit an artifact they can only read, the adapter
  should reject it
- The UI role gating prevents accidental misuse; the adapter prevents
  intentional misuse

---

## Part 8: Developer Experience (DX) — Consumer Integration Guide

### 8.1 What the consumer must implement

The consumer integrates phz-grid by implementing adapters and embedding
components. Here's the minimum viable integration path, then the
progressive enhancements.

**Minimum viable (renders a dashboard from config):**

```
Consumer implements:
  1. WorkspaceAdapter    → CRUD for artifact configs (REST API + database)
  2. DataAdapter         → execute queries, return rows (your existing API)

Consumer embeds:
  3. <phz-dashboard>     → renders a dashboard config with data

That's it. Three things. Everything else is optional.
```

**Adding the workspace (authoring UI):**

```
Consumer additionally:
  4. Embeds <phz-workspace> for admin/author users
  5. Sets workspaceRole based on their auth system
  6. Maps ViewerContext from their user/session model
```
...
```
Consumer additionally:
  7. Implements AlertChannelAdapter(s) for notification delivery
  8. Calls evaluateAlerts() on a schedule (cron, data refresh hook, etc.)
  9. Implements BreachStoreExtension on WorkspaceAdapter for breach persistence
```

**Adding the local playground (zero-ops demo):**

```
No consumer implementation needed. Just:
  10. <phz-workspace> with MemoryAdapter + DuckDBDataAdapter
  11. That's the playground. Ship it as a static site.
```

### 8.2 What the consumer does NOT implement

These are handled by phz-grid internally:

- Widget rendering (20+ built-in widgets)
- Widget registry and resolution
- Layout rendering (LayoutIntent → CSS Grid)
- Filter bar rendering and filter context management
- Filter rule evaluation
- Expression compilation and evaluation
- Template matching and suggestion
- Schema analysis for auto-configuration
- Preload/full load data orchestration
- Query coordination and deduplication
- CSS theming (consumer sets tokens, phz-grid handles the rest)
- Responsive widget behavior (CSS Container Queries)
- DuckDB-WASM integration (for Tier 1/2)
- Schema migration on config load
- Undo/redo in designers
- Keyboard shortcuts in the workspace

### 8.3 Integration decision matrix

For each capability, the consumer decides their level of involvement:

| Capability | Use built-in | Customize | Replace entirely |
|-----------|-------------|-----------|-----------------|
| Widget types | 20+ built-in | Register custom via WidgetManifest | Replace the registry |
| Data fetching | DuckDB adapter (Tier 1/2) | Implement DataAdapter | — |
| Config storage | MemoryAdapter (dev) | Implement WorkspaceAdapter | — |
| Theming | 5 built-in themes | Override CSS tokens | Provide full ThemeTokens object |
| Filter UI | Built-in filter types | — | Register custom filter type |
| Layout | Intent-based auto-grid | Configure LayoutIntent tree | — |
| Auth/identity | None (single user) | Provide ViewerContext | — |
| Notifications | None | Implement AlertChannelAdapter | — |
| Routing | None (phz-grid emits events) | Handle navigate events | — |
| i18n | English default | Provide I18nProvider | — |

### 8.4 Adapter implementation checklist

When a consumer implements `WorkspaceAdapter`, they need endpoints for:

| Method | Minimum | With alerts | With history |
|--------|---------|-------------|-------------|
| getArtifact | Required | Required | Required |
| listArtifacts | Required | Required | Required |
| saveArtifact | Required | Required | Required |
| deleteArtifact | Required | Required | Required |
| listTemplates | Required | Required | Required |
| getTemplate | Required | Required | Required |
| getPlacementsForRoute | Required | Required | Required |
| savePlacement | Required | Required | Required |
| getActiveBreaches | — | Required | Required |
| saveBreaches | — | Required | Required |
| acknowledgeBreach | — | Optional | Optional |
| getArtifactHistory | — | — | Required |
| getArtifactVersion | — | — | Required |
| restoreArtifactVersion | — | — | Optional |

When implementing `DataAdapter`:

| Method | Minimum | With filters | With explorer |
|--------|---------|-------------|--------------|
| execute | Required | Required | Required |
| getSchema | Required | Required | Required |
| listDataSources | Required | Required | Required |
| getDistinctValues | — | Required | Required |
| getFieldStats | — | Optional | Required |

### 8.5 The consumer's codebase separation

The consumer's codebase should separate phz-grid concerns cleanly:

```
consumer-app/
  src/
    phz/                          # All phz-grid integration code lives here
      adapters/
        workspace-adapter.ts      # WorkspaceAdapter implementation (REST → your API)
        data-adapter.ts           # DataAdapter implementation (your DB queries)
        alert-channels.ts         # AlertChannelAdapter implementations (email, slack)
      config/
        theme.ts                  # Custom ThemeTokens
        capabilities.ts           # ConsumerCapabilities declaration
        custom-widgets.ts         # Custom widget registrations
      pages/
        workspace.tsx             # <phz-workspace> embedding
        dashboard-viewer.tsx      # Consumer-side dashboard rendering
      utils/
        viewer-context.ts         # Map your auth user → ViewerContext
        filter-sync.ts            # URL filter state ↔ your router
```

Everything phz-grid-related lives in one directory. The adapters are the
boundary. If the consumer decides to replace phz-grid with another tool,
they replace this directory and implement the same adapter interfaces
against the new tool. The rest of the consumer app doesn't know phz-grid
exists.

### 8.6 Testing strategy for the consumer

The consumer tests their integration at three levels:

1. **Adapter unit tests**: Test WorkspaceAdapter and DataAdapter against
   your backend. Mock the HTTP calls, verify the correct queries are
   built from DataQuery objects, verify visibility filtering works.

2. **Integration tests**: Render `<phz-workspace>` with your adapters
   in a test browser (Playwright). Verify: dashboard loads with data,
   filters work, presets save and load, navigation links resolve.

3. **End-to-end**: Full app with real backend. Verify: user opens
   workspace, creates a dashboard, saves it, another user sees it
   (if shared/published), personal presets persist across sessions.

phz-grid ships test utilities for each level:

```typescript
import { createMockWorkspaceAdapter, createMockDataAdapter } from '@phozart/phz-workspace/testing'

// Adapter with in-memory storage, pre-populated with sample data
const mockAdapter = createMockWorkspaceAdapter({
  artifacts: [sampleDashboard, sampleReport],
  templates: defaultTemplates
})

// DataAdapter with sample datasets
const mockDataAdapter = createMockDataAdapter({
  dataSources: [sampleSalesData, sampleHRData]
})
```
