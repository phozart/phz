...
packages/phz-workspace/
  src/
    ...
    testing/
      mock-workspace-adapter.ts  # In-memory adapter with sample data for tests
      mock-data-adapter.ts       # Mock DataAdapter with sample datasets
    index.ts
  package.json
```
...
- A sidebar organized into three sections (see WORKSPACE-CONVENTIONS.md Part 6):
  - **Content**: Catalog, Explore, Create New, Templates
  - **Data**: Data Sources, Connections
  - **Govern**: Filters, Rules, Alerts, Publish
- Sections visible based on `workspaceRole` (see WORKSPACE-CONVENTIONS.md Part 7):
  - Admin: all three sections
  - Author: Content + Data (read-only)
  - Viewer: simplified catalog only
...
export class WorkspaceShell extends LitElement {
  ...
  @property({ type: String }) workspaceRole?: 'admin' | 'author' | 'viewer'
...
- `workspaceRole` controls what the UI shows, not what the backend allows.
  The consumer's WorkspaceAdapter is responsible for rejecting unauthorized
  operations. The UI role gating prevents accidental misuse; the adapter
...
// Testing utilities (import from '@phozart/phz-workspace/testing')
export { createMockWorkspaceAdapter } from './testing/mock-workspace-adapter'
export { createMockDataAdapter } from './testing/mock-data-adapter'
