...
const packages = [
  { name: 'shared', entry: 'packages/shared/src/index.ts', budget: 50 },
  ...
  { name: 'workspace', entry: 'packages/workspace/src/index.ts', budget: 150 },
  { name: 'viewer', entry: 'packages/viewer/src/index.ts', budget: 80 },
  { name: 'editor', entry: 'packages/editor/src/index.ts', budget: 80 },
  ...
  { name: 'local', entry: 'packages/local/src/index.ts', budget: 50 },
];
...
const aliasPlugin = {
  ...
  setup(b) {
    b.onResolve({ filter: /^@phozart\/phz-/ }, (args) => {
      const pkgName = args.path.replace('@phozart/', '');
